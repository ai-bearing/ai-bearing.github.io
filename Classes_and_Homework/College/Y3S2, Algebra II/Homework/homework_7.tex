...
\begin{document}
  \begin{center}
    ...
    {\bf Collaborators:} {Antonio Cabello, Timothy Rainone} \\
  \end{center}
  \section{Problem 1}%
  ...
  First, we have that $D_x(f(x)) = px^{p-1} - 1 = -1$, meaning that $\gcd(f(x),D_x(f(x))) = 1$, so $f$ is separable. Additionally, we have that $x^p - x + a = (x^p - x) + a = 0 + a = a$, so $f$ is irreducible over $\mathbb{F}_p$.
  \section{Problem 4}%
  Let $K$ be a finite extension of $\Q$. We will prove there are only a finite number of roots of unity in $K$.
  \section{Problem 6}%
\end{document}
