...
\end{tikzcd}
  ...
  Let $\tau\in\text{Gal}\left(\Q(\zeta_7)/\Q\right)$, with $\tau \notin \langle \sigma_6\rangle$. We have $\tau(\alpha)$ must be a linear combination of the basis elements $1,\zeta_7,\dots,\zeta_7^6$. If $\tau(\alpha) = \alpha$, then $\tau(\zeta_7)=\sigma(\zeta_7)$ for some $\sigma = 1,\sigma_6$, but $\tau\notin\langle \sigma_6\rangle$. Thus, it must be that $\Q(\zeta_7 + \zeta_7^{-1}) = \Q(\zeta_7)^{\langle \sigma_6\rangle}$.\\
...
\end{tikzcd}
  ...
  \begin{enumerate}[(1)]
    \item $\text{Gal}(x^3 - 2) = \text{Gal}\left(\Q(\zeta_3,\sqrt[2]{3})/\Q\right)\cong S_3$
  ...
  \subsection{Elementary Symmetric Functions and General Polynomial}%
