...
\begin{document}
  ...
  \begin{definition}[Fourier Transform]
    ...
    \end{align*}
    I will use $\mathfrak{F}$ to denote the Fourier transform; $\mathfrak{F}(f(t)) = \hat{f}(k)$.
  \end{definition}
  \begin{proposition}[Properties of the Fourier Transform]
    Let $f\in \mathcal{S}(\R)$. Then:
    \begin{itemize}
      \item $\displaystyle\mathfrak{F}(f(x+h)) = \hat{f}(k)e^{2\pi i h k}$ for $h\in \R$;
      \item $\displaystyle \mathfrak{F}\left(f(x)e^{-2\pi i x h}\right) = \hat{f}(k + h)$ for $h\in \R$;
      \item $\displaystyle \mathfrak{F}\left(f(\delta x)\right) = \delta^{-1}\hat{f}(\delta^{-1}k)$ for $\delta > 0$;
      \item $\mathfrak{F}(f'(x)) = 2\pi i k \hat{f}(k)$;
      \item $\mathfrak{F}(-2\pi i x f(x)) = \frac{d\hat{f}}{dk}$.
    \end{itemize}
  \end{proposition}
