...
\end{definition}
  ...
  \begin{solution}\hfill
    \begin{enumerate}[(i)]
      ...
      \item Let $M_X = M\cap X$ and $M_Y = M\cap Y$. Notice that $M = M_X\sqcup M_Y$.\newline
        ...
        Thus, $|M| \leq |N(M)|$. For future reference, the condition $|M| \leq |N(M)|$ is known as the Hall condition.
  ...
  \begin{definition}[Hall's Marriage Condition]
    For a bipartite graph $\left(X,Y,E,\phi\right)$, there is an $X$-perfect matching\footnote{A matching on $\left(X,Y,E,\phi\right)$ that satisfies every vertex of $X$.} if and only if for all $S\subseteq $

    Equivalently, for a finite collection of (not necessarily distinct) finite sets $\mathcal{G} = \set{X_i}_{i=1}^{n}$, there is a system of distinct representatives\footnote{A set $\set{x_i}_{i=1}^{n}$ such that $x_i\in X_i$ and $x_{i}\notin X_j$ for $j\neq i$.} for $\mathcal{G}$ if and only if for all subcollections $\mathcal{Y}_k = \set{X_{i_k}}_{k=1}^{m}$, $\left\vert \mathcal{Y}_k \right\vert\leq \left\vert \bigcup_{k=1}^{m}X_{i_k} \right\vert$.
  \end{definition}
  \begin{remark}
    These two formulations of the Hall condition are equivalent regarding a $X$-perfect matching.\newline
  ...
  \end{theorem}
 % \begin{proof}
 %   We define an equivalence relation on $V$ by saying that two vertices are equal if there is a path between them.\newline

 %   Each equivalence class under this relation on $(X,Y,E,\phi)$\footnote{These are known as connected components.} is a $k$-regular bipartite graph that has countably many edges and vertices. We may thus restrict our attention to one of these equivalence classes.\newline

 %   If the equivalence class is finite, we have proven in the exercise that by Hall's theorem that there is a perfect matching. We may thus restrict our attention to connected components with countably infinite $X$, $Y$, and $E$.\newline

 %   For $x\in X$ and $y\in Y$, define $Z_1 = \set{x,y}\cup N\left(\set{x,y}\right)$, and $Z_{n} = N\left(Z_{n-1}\right)$. Define $X_n = Z_n\cap X$ and $Y_n = Z_n\cap Y$, with $E_n\subseteq E$ such that $\left(X_n,Y_n,E_n,\phi\bigr\vert_{E_n}\right)$ is a graph.\newline

 %   We can extend $\left(X_n,E_n,\phi\bigr\vert{E_n}\right)$ to a $k$-regular bipartite graph, $\left(X_n', Y_n', E_n',\phi\bigr\vert_{E_n'}\right)$ by 
 % \end{proof}
