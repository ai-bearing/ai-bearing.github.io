...
\begin{proof}
  ...
  Similarly, if $x = \sum_{i=1}^{n}\alpha_iv_i$ with at least one $\alpha_i\neq 0$, then
  \begin{align*}
    \left(\varphi_{1} + \cdots + \varphi_{n}\right)\left(x\right) &\neq 0
  \end{align*}
  so $\set{\varphi_i}_{i=1}^{n}$ is a basis for $X^{\ast}$.
