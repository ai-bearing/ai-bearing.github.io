...
\begin{example}
  ...
  If $\left(\Omega,\mathcal{M}\right)$ is a measurable space, then $B_{\infty}\left(\Omega,\mathcal{M}\right)$, the space of bounded measurable functions, is a $\ast$-subalgebra of $\ell_{\infty}\left(\Omega\right)$ and of $L_0\left(\Omega,\mathcal{M}\right)$.\newline
...
Let $\Gamma$ be a group, and let $\C\left[\Gamma\right]$ be the free vector space on $\Gamma$. For each $f,g\in \C\left[\Gamma\right]$, we define $f\times g$ by convolution:
\begin{align*}
  f\times g \left(s\right) &= \sum_{t\in \Gamma}f\left(t\right)g\left(t^{-1}s\right)\\
