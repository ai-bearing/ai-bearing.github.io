...
\begin{theorem}\label{thm:fundamental_thm_abelian_gps}
  Let $G$ be a finitely generated (see Definition \ref{def:generating_sets}) abelian group. Then, there is some $d\in \Z_{\geq 0}$ and some $k_1,\dots,k_n$ such that
  \begin{align*}
...
\end{theorem}
\begin{remark}
  Krull's Theorem can be proven using Zorn's Lemma (Lemma \ref{thm:zorns_lemma}) applied to the partially ordered set of proper ideals ordered by inclusion.
\end{remark}
\begin{definition}
...
\begin{definition}\label{def:algebra_homomorphisms_and_characters}
  ...
  \end{enumerate}
\end{definition}
\begin{fact}[{\cite[Exercise F.4.17]{rainone_analysis}}]\label{fact:positivity_homomorphisms}
...
\begin{corollary}[{\cite[Corollary F.6.11]{rainone_analysis}}]\label{cor:characters_algebras}
  ...
  \end{align*}
\end{corollary}
\begin{fact}[{\cite[Fact F.6.12]{rainone_analysis}}]\label{fact:spectrum_commutative}
