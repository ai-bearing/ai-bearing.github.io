Much of this appendix pulls information from \cite{rainone_analysis}, \cite{taste_of_topology}, and \cite{aliprantis_infinite_dimensional_analysis}.
\section{Ordering, the Axiom of Choice, and Zorn's Lemma}%
\begin{definition}[Preorders, Partial Orders, Total Orders, and Well-Orders]\label{def:ordered_sets}
Let $X$ be a set, and $\preceq $ be a relation on $X$. We say a relation is a \textit{preorder} if it is reflexive and transitive:
\begin{itemize}
  \item $a\preceq a$
  \item $a\preceq b \wedge b\preceq c\Rightarrow a\preceq c$.
\end{itemize}
...
\begin{example}\hfill
  ...
  \end{itemize}
\end{example}
\begin{example}\label{ex:ordered_vector_space}
  ...
  \end{itemize}
  then we say $V$ is an \textit{ordered vector space}.\newline
  ...
  Note that if we are given a vector space $V$, and a cone $C\subseteq V$ such that $C\cap \left( -C \right) = \set{0}$, then we may define an ordering on $V$ by taking $x\geq y$ if and only if $x-y\in C$.
\end{example}
...
\begin{theorem}[Zorn's Lemma]\label{thm:zorns_lemma}
...
\begin{example}\label{ex:results_follow_from_axiom_of_choice}\hfill
  \begin{itemize}
    ...
    \item If $\varphi$ is a continuous linear functional defined on a subspace $W\subseteq V$, there is an extension $\Phi$ such that $\Phi|_{W} = \varphi$ --- this is one of the Hahn--Banach theorems (Theorem \ref{thm:hb_continuous_extension}). %See: Hahn--Banach Theorems
...
\begin{definition}[Open and Closed Sets]
  ...
  \begin{enumerate}[(1)]
    \item For $x\in X$ and $\delta > 0$, we define
      \begin{enumerate}[(a)]
        \item the \textit{open ball} at $x$ with radius $\delta > 0$
          \begin{align*}
            U\left(x,\delta\right) &\coloneq \set{y\in X\mid d\left(y,x\right) < \delta};
          \end{align*}
        \item the \textit{closed ball} centered at $x$ with radius $\delta > 0$
          \begin{align*}
            B\left(x,\delta\right) &\coloneq \set{y\in X\mid d\left(y,x\right)\leq \delta};
          \end{align*}
        \item the \textit{sphere} centered at $x$ with radius $\delta > 0$
          \begin{align*}
            S\left(x,\delta\right) &\coloneq \set{y\in X\mid d\left(y,x\right) = \delta}.
          \end{align*}
      \end{enumerate}
    ...
    \item If $A\subseteq X$ is any subset, the interior of $A$ is
      \begin{align*}
        A^{\circ} &\coloneq \bigcup\set{V\mid V\text{ is open, }V\subseteq A},
      \end{align*}
      the \textit{closure} of $A$ is
      \begin{align*}
        \overline{A} &\coloneq \bigcap\set{C\mid C\text{ is closed, }A\subseteq C},
      \end{align*}
      and the \textit{boundary} of $A$ is
      \begin{align*}
        \partial A &\coloneq \overline{A} \setminus A^{\circ}.
      \end{align*}
  \end{enumerate}
\end{definition}
...
\end{fact}
\begin{fact}
  If $M$ and $N$ are metric spaces with $N$ complete, and $A\subseteq M$ is dense, then if $f\colon A\rightarrow N$ is uniformly continuous, there is a unique uniformly continuous map $\tilde{f}\colon M\rightarrow N$.
\end{fact}
\begin{definition}
...
\begin{definition}\label{def:nowhere_dense}
  ...
  \begin{enumerate}[(1)]
    %\item The \textit{interior} of $A$ is the open set $A^{\circ} = \bigcup\set{V | V\in\tau,~V\subseteq A}$.
    %\item The \textit{closure} of $A$ is the closed set $\overline{A} = \bigcap\set{C| C\text{ closed, }A\subseteq C}$.
...
\end{fact}
\begin{definition}\label{def:neighborhoods_and_bases}
...
\end{fact}
\begin{definition}[Separation Axioms]\label{def:separation_axioms}
Let $\left(X,\tau\right)$ be a topological space.
\begin{itemize}
  \item We say $X$ is T1 if $\set{x}$ is closed for every $x\in X$.
  \item We say $X$ is T2 (or \textit{Hausdorff}) if, for every $x,y\in X$ with $x\neq y$, there are $U,V\in \tau$ with $x\in U$, $y\in V$, and $U\cap V = \emptyset$.
  \item We say $X$ is T3 if, for every $x\in X$ and $B\subseteq X$ closed with $x\notin B$, there are $U,V\in \tau$ with $x\in U$, $B\subseteq V$, and $U\cap V = \emptyset$. If $X$ is T1 and T3, we say $X$ is \textit{regular}.
  \item We say $X$ is T3.5 if, for every $x_0\in X$ and closed $B\subseteq X$ with $x_0\notin B$, there is a continuous function $f\colon X\rightarrow [0,1]$ with $f\left(x_0\right) = 0$ and $f\left(B\right) = 1$. If $X$ is T1 and T3.5, we say $X$ is \textit{completely regular}.
  \item We say $X$ is T4 if, for every pair of closed subsets $A,B\subseteq X$ with $A\cap B = \emptyset$, there are $U,V\in \tau$ with $A\subseteq U$, $B\subseteq V$, and $U\cap V = \emptyset$. If $X$ is T1 and T4, then we say $X$ is \textit{normal}.
\end{itemize}
\end{definition}
...
\begin{theorem}[Urysohn's Lemma]\label{thm:urysohn_lemma}
...
\end{enumerate}
\end{example}
\begin{definition}\label{def:cluster_points}
...
\begin{fact}[Characterizations Using Nets]
  ...
  \end{itemize}
\end{fact}
We define subnets in a similar fashion to how we define subsequences, albeit with a little bit more care due to the fact that our domain is not $\N$, but any directed set.
\begin{definition}
...
\begin{theorem}[Stone--Weierstrass Theorem]\label{thm:stone_weierstrass}
