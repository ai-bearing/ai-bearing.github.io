...
Much of this chapter pulls information from \cite{rainone_analysis}.
...
\end{corollary}
To provide some context for the Hahn--Banach separation results, consider two open, disjoint, convex subsets $A,B\subseteq \R^n$. The hyperplane separation theorem from convex optimization (see \cite[Chapter 2.6]{convex_optimization}) states that there is a nonzero vector $m\in \R^n$ and some $b\in \R$ such that the map $\varphi\colon \R^n\rightarrow \R$, defined by $\varphi(\mathbf{x}) = \mathbf{m}^{T}\mathbf{x} - b$, is strictly negative for all $\mathbf{x}\in A$ and is strictly positive for all $\mathbf{x}\in B$. The affine hyperplane (Definition \ref{def:hyperplane}) defined by $\set{\mathbf{x}\in \R^n | \varphi(\mathbf{x}) = b}$ is known as a separating hyperplane for $A$ and $B$.\newline
...
\begin{definition}\label{def:adjoint_properties}
  ...
  \begin{itemize}
    ...
    \item $\norm{T^{\ast}}_{\op} = \norm{T}_{\op}$;
    \item $\norm{T^{\ast}T}_{\op} = \norm{T}_{\op}^2$ (known as the $\mathrm{C}^{\ast}$-property).
  \end{itemize}
\end{definition}
