...
\end{proof}
\begin{definition}
  A \textbf{state} on $\ell_{\infty}\left(G\right)$ is a continuous linear functional $\mu\in \left(\ell_{\infty}\left(G\right)\right)^{\ast}$ such that the following are true:
  \begin{itemize}
    \item $\mu$ is positive;
...
\begin{definition}
  A group is said to satisfy the \textbf{Følner condition} if, for every $\ve > 0$ and $E\subseteq G$, there is a nonempty finite subset $F\subseteq G$ such that for all $t\in E$,
  \begin{align*}
...
\end{proof}
\begin{definition}
  For a countable group $G$, a sequence $\left(f_k\right)_k$ is called an \textbf{approximate mean} if, for all $s\in G$,
  \begin{align*}
    \left\vert f_k - \lambda_s\left(f_k\right) \right\vert_{1} &\xrightarrow{k\rightarrow \infty}0.
  \end{align*}
\end{definition}
\begin{proposition}
