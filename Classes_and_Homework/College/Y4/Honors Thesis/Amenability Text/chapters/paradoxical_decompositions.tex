...
\end{restatable}
The Banach--Tarski paradox throws a wrench into a common belief that we have about $\R^3$ --- specifically, that every subset of $\R^3$ has a \textit{finitely additive} ``volume'' that is invariant under isometry.\footnote{Note that if we desire countable additivity, the axiom of choice shows that there does not exist a countably additive measure on $P\left(\R\right)$ that is also translation-invariant. Finite additivity is a weaker condition than countable additivity that allows for the existence of well-behaved measures on $P\left(\R\right)$ and $P\left(\R^2\right)$, but even this fails in $\R^3$ and above.} Later, we will see that this property \textit{does} exist for $\R$ and $\R^2$, as we will show that their isometry groups are abelian, and (as we will see in Chapter 5) thus contain a property known as amenability.\newline
...
%
%We will not be using this method, though, instead we will be using the construction in \cite{delaHarpe_topics_in_geometric_group_theory}. For the sake of completeness, we will state the universal property here before moving towards the more detailed exposition.
%\begin{definition}
...
%\end{definition}
%To start the construction of the free group, we begin by defining the free \textit{monoid}, which will them become a free product, of which the free group is a special example.
...
\section{Introducing Paradoxical Decompositions}%
With knowledge of free groups in mind, we can turn our attention to ``paradoxical'' actions that seem to recreate a set by using some of its disjoint proper subsets.
...
\end{remark}
\section{Paradoxical Decompositions in $\R^3$: the Weak Banach--Tarski Paradox}%
...
\end{proof}
\section{The Full Banach--Tarski Paradox}%
