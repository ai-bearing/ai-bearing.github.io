...
\begin{definition}[Faithful Action]
  An action is \textbf{faithful} if the kernel of the action is the identity, $e_G$. Equivalently, the permutation representation $\varphi\colon G\rightarrow \sym(A)$ is injective.
\end{definition}
...
\begin{definition}
  Let $S$ be a set. A group $F$ containing $S$ is said to be \textbf{freely generated} if, for every group $G$, and every map $\phi\colon S\rightarrow G$, there is a unique group homomorphism $\varphi\colon F\rightarrow G$ that extends $\varphi$. The following diagram, where $\iota$ denotes the inclusion of $S$ into $F$, commutes:
\begin{center}
...
\begin{proof}
  ...
  Since $S^{2}$ is $\text{SO}(3)$-paradoxical, there exists pairwise disjoint $A_1,\dots,A_n,B_1,\dots,B_m\subset S^2$ and elements $g_1,\dots,g_n,h_1,\dots,h_m\in \text{SO}(3)$ such that
  \begin{align*}
