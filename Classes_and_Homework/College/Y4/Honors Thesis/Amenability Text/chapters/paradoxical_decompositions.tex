...
\section{Prelude: Group Action Essentials}
...
\end{theorem}
\begin{proof}
  Let $A$ and $B$ be the rotations in $SO(3)$ that serve as the generators of the subgroup isomorphic to $F(a,b)$. Since $A$ and $B$ are rotations, so too is any reduced word over $\set{A,A^{-1},B,B^{-1}}$. Thus, any such non-empty word contains two fixed points.\newline
...
\end{proof}
To prove the Banach--Tarski paradox, we need a slightly larger group than $SO(3)$ --- one that includes translations in addition to the traditional rotations. This is the Euclidean group.
