...
The exposition in this chapter follows some notes compiled by professor Rainone, as well as pulling from \cite[Appendix A]{juschenko_amenability}.
\section{Inheritance Properties of Amenability}\label{sec:amenability_subgroups_quotients}%
...
This section will begin by establishing some useful inheritance properties.
\begin{definition}
...
\begin{proof}
  ...
  Thus, $G$ is amenable.
\end{proof}
\section{From Means to States}\label{sec:functional_analysis_and_amenability}%
...
\begin{definition}
  ...
  \begin{enumerate}[(1)]
    \item The space $\mathcal{F}\left(G\right)$ is defined by
      ...
      \end{align*}
    \item A function $f\in \mathcal{F}\left(G\right)$ is called \textit{positive} if $f(x) \geq 0$ for all $x\in G$.
    \item A function $f\in \mathcal{F}\left(G\right)$ is called \textit{simple} if $\ran(f)$ is finite. We let
      \begin{align*}
...
\end{definition}
\begin{fact}
  The space of simple functions, $\Sigma \subseteq \mathcal{F}\left(G\right)$, is a linear subspace.
\end{fact}
\begin{definition}
...
\begin{definition}
  ...
  \end{itemize}
  A state is called \textit{left-invariant} if
  \begin{align*}
...
%  \end{enumerate}
%\end{theorem}
\begin{corollary}\label{cor:state_space_compact}
...
This is the substance of the von Neumann conjecture --- and as it turns out, it is false. There are some groups that are not amenable, but do not contain a subgroup isomorphic to $F(a,b)$. In \cite{piecewise_projective_homeomorphisms}, Nicolas Monod showed that groups of piecewise projective homeomorphisms of the projective real line, $\R P^1$, are not amenable and do not contain any freely generated subgroups.\newline

Yet, in \cite{free_subgroups_of_linear_groups}, Jacques Tits proved that in any subgroup of $\text{GL}_n\left(\F\right)$ (where $\F$ is any field with characteristic zero), a subgroup either admits a solvable subgroup of finite index (hence amenable by Corollary \ref{cor:solvable_groups_amenable} and Proposition \ref{prop:finite_index_amenable_subgroup}) or contains a non-abelian freely generated subgroup (which is not amenable by Theorem \ref{thm:tarski}). This is known as the Tits alternative, and its proof relies heavily on results from algebraic geometry.\newline

In other words, the von Neumann conjecture \textit{is} true for linear groups, so it necessarily means that we cannot represent the counterexample groups to the von Neumann conjecture as linear groups. Oddly, Monod's proof relied on properties of the group $\text{PSL}_2(\R)$ represented as matrices, though none of the groups that he constructed are able to be represented by linear groups.\newline
