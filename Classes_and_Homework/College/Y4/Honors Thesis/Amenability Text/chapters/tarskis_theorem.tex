Ultimately, the reason the Banach--Tarski paradox ``works'' is because the paradoxical group $F(a,b)$, lacks a property known as amenability --- specifically, that a group admitting a paradoxical decomposition is not amenable. Before we go further into the characterizations of amenability discussed in chapters 5 and 6, we will show that this statement reverses. Indeed, every amenable group is \textit{non}-paradoxical.
\begin{restatable}[Tarski's Theorem]{theorem}{tarski}
  Let $G$ be a group that acts on a set $X$, and let $E \subseteq X$ be nonempty. There is a finitely additive translation-invariant measure $\mu \colon P(X) \to [0, \infty]$ with $\mu(E) \in (0, \infty)$ if and only if $E$ is not $G$-paradoxical.
\label{thm:tarski}
\end{restatable}
