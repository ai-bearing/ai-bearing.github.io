Ultimately, the reason the Banach--Tarski paradox ``works'' is because the paradoxical group $F(a,b)$, lacks a property known as amenability --- specifically, that a group admitting a paradoxical decomposition is not amenable. Before we go further into the characterizations of amenability that will be discussed in Chapter 3, we will start by proving Tarski's theorem, which will establish the equivalence between amenability and non-paradoxicality.
\begin{theorem}[Tarski's Theorem]
  Let $G$ be a group that acts on a set $X$, and let $E\subseteq X$ be nonempty. There is a finitely additive translation-invariant measure $\mu\colon P(X)\rightarrow [0,\infty]$ with $\mu(E)\in (0,\infty)$ if and only if $E$ is not $G$-paradoxical.\label{thm:tarski}
\end{theorem}
...
\end{fact}
\begin{proof}[Proof of Fact \ref{fact:type_semigroup_paradoxicality}:]
...
\begin{lemma}
  ...
  \begin{enumerate}[(i)]
    ...
    \item for $\alpha_1,\dots,\alpha_n,\beta_1,\dots,\beta_m\in \mathcal{S}_0$ with $\alpha_1+\cdots+\alpha_n\leq \beta_1+\cdots\beta_m$,
      ...
      \end{align*}
  \end{enumerate}\label{lemma:set_function_existence}
\end{lemma}
\begin{proof}
...
\begin{theorem}
  ...
  \end{enumerate}\label{thm:homomorphism_existence}
\end{theorem}
\begin{proof}
...
\begin{proof}[Proof of Theorem \ref{thm:tarski}:]
