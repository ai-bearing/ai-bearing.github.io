...
\end{proof}
\begin{proposition}
  If $\mu\in \left(\ell_{\infty}\left(G\right)\right)^{\ast}$ is a state, then $m: P(G) \rightarrow [0,1]$ defined by $m(E) = \mu\left(\1_{E}\right)$ is a finitely additive probability measure on $G$. Moreover, if $\mu$ is invariant, then $m$ is a mean.
\end{proposition}
\begin{proof}
  We have
  \begin{align*}
...
\begin{proof}
  ...
  Since $\overline{\Sigma} = \ell_{\infty}(G)$, we see that $\mu_0$ extends to a continuous linear functional $\mu: \ell_{\infty}\left(G\right) \rightarrow \R$, with $\mu\left(\1_{G}\right) = \mu_{0}\left(\1_{G}\right) = 1$.\newline
...
\end{proposition}
\begin{proof}
  We know that $\lambda_{1}: \ell_{\infty}\left(\Z\right) \rightarrow \ell_{\infty}\left(\Z\right)$, defined by
  \begin{align*}
