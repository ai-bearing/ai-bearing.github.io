...
\end{align*}
where $h\left(m_1,\dots,m_k,0\right) = f\left(m_1,\dots,m_k\right)$. Here, $f\left(m_1,\dots,m_k\right)$ is written in terms of $Z,S,P_{i}^{(k)}$. We say $h$ is obtained from $f$ and $g$ by primitive recursion.
\end{definition}
\begin{definition}[Primitive Recursive Function]
  Any function obtained from $Z,S,P_{i}^{(k)}$, composition, and primitive recursion, is called a primitive recursive function.
\end{definition}
