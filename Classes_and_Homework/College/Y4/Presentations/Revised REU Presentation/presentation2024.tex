...
\end{itemize}
\begin{definition}[Bergman Projection]
...
\begin{frame}{Literature Review on Previous Results II}
    ...
    \item Substituted Toeplitz operators for derivatives in Excess Area identity \cite{AshleyAdenCelikDanielLuke2024}
    \end{itemize}
\end{frame}
...
\begin{enumerate}
 \item[1.] Results and Observations influenced by the Excess Area identity: 
   \begin{itemize}
...
%  The area is integrated with respect to $dA = dx \wedge dy$. When $a,b$, and $c$ are $1$-forms, the wedge product has the following properties:
%  \begin{align*}
%    a\wedge \left(b + c\right) &= a\wedge b + a\wedge c\\
%    a\wedge b &= -b\wedge a\\
%    a\wedge a &= 0.
%  \end{align*}
...
\begin{frame}{Excess Area on Fock Spaces}
    ...
    \begin{block}{Excess Area on Fock Space}
      Given $h\in \mathcal{F}^2 = A^{2}\left(\C,e^{-|z|^2}\right) $ with $\frac{\partial h}{\partial z}\in \mathcal{F}^2$,
    \begin{align*}
        &A_{\mathcal{F}^2}\left(zh\right) - A_{\mathcal{F}^2}\left(h\right) \\
        &= \pi \norm{M_zT^{\mathcal{F}^2}_{\ol{z}}\left(h\right)}_{\mathcal{F}^2}^2+ \pi\norm{M_zh}_{\mathcal{F}^2}^2
...
%    \end{block}
%\end{frame}
\begin{frame}{Contraction and Dilation from $A^2(D(0,r))$ to $A^2(\mathbb{D})$}
    \begin{block}{Contraction from $A^2(\mathbb{D})$ to $A^2(D(0,r))$}
    Given some $h\in A^{1, 2}(\mathbb{D})$ and taking $h_r = h(rz)$ for some \(0 < r < 1\),
    \begin{align*}
    ...
    \begin{block}{Dilation from $A^2(D(0,r))$ to $A^2(\mathbb{D})$}
    Given some $h \in A^{1,2}(D(0,r))$ and taking $h_{\frac{1}{r}}=h(\frac{z}{r})$ for some $0 < r < 1$
    \begin{align*}
...
%\end{frame}
\section{REU Experience}
\begin{frame}{What was Research Like?}
  \begin{itemize}
    ...
    \item However, the content was still difficult, and we spent a lot of time trying to understand what it was we were doing --- our mentor was always available to help us improve our understanding.
  \end{itemize}
\end{frame}
\begin{frame}{Activities and Life Outside Research}
  \begin{itemize}
    ...
    \item Overall, it was an intensely rewarding experience --- if you enjoy math and want to help push the boundaries of the subject just a little bit further, apply to some REUs!
%    \item There were four project cohorts at the REU, but ours was the coolest since it wasn't applied.
  \end{itemize}
\end{frame}
\section{Acknowledgements and References}
    ...
    This research is based upon work supported by the National Science Foundation under Grant DMS-2243991.\break
