...
\end{solution}
\begin{exercise}[Exercise 2.45]
Show that ideals $I,J\subseteq k\left[ x_1,\dots,x_n \right]$ with $k$ algebraically closed are comaximal if and only if $V(I)\cap V(J) = \emptyset$.
\end{exercise}
\begin{solution}
  If $I$ and $J$ are comaximal, then $ I + J = k\left[ x_1,\dots,x_n \right] $, so since $I + J$ is an ideal that contains both $I$ and $J$,
  \begin{align*}
    \emptyset &= V\left( I + J \right)\\
              &\supseteq V\left( I \right)\cap V\left( J \right).
  \end{align*}
  Now, if $V(I)\cap V(J) = \emptyset$, then $V(I+J) = \emptyset$, so $ I + J = k\left[ x_1,\dots,x_n \right] $.
\end{solution}
...
\begin{proof}
  ...
  By the Nullstellensatz, $\operatorname{rad}\left( I \right) = \bigcap_{i=1}^{n}I_i$, so $\left( \bigcap I_i \right)^d\subseteq I$ for some $d$. Since $\bigcap_{j\neq i}I_j$ and $I_i$ are comaximal we have $\bigcap I_j^d = \left( I_1\cdots I_N \right)^d \subseteq I$.\newline

  Let $F_i\in k\left[ x_1,\dots,x_n \right]$ be such that $f_i\left( P_j \right) = \delta_{ij}$. Let $E_i = 1-\left( 1-F_i^{d} \right)^{d}$. Note that $E_i = F_i^{d}D_i$ for some $D_i$, so $E_i\in I_j^{d}$ for $i\neq j$. Furthermore,
  \begin{align*}
    1- \sum_{i} E_i &= \left( 1-E_j \right) - \sum_{i\neq j}E_j\\
                    &\in \bigcap I_j^{d}\\
                    &\subseteq I.
  \end{align*}
  Furthermore, $E_i-E_i^2 = E_i\left( 1-F_i^{d} \right)^{d}$ is in $\left( \bigcap_{j\neq i} I_j^{d} \right) I_i^{d} \subseteq I$. If we define $e_i$ to be the residue of $E_i$ in $R$, we have $e_i^2 = e_i$, $e_ie_j = 0$ for $i\neq j$, and $\sum e_i = 1$.\newline

  We claim that if $g\in k\left[ x_1,\dots,x_n \right]$, and $g\left( P_i \right)\neq 0$, there is $t\in R$ such that $tg^{\ast} = e_i$, where $g^{\ast}$ is the residue of $g$ in $R/I$.\newline

  We may assume that $g\left(P_i\right) = 1$. Let $h = 1-g$. It follows that $\left( 1-h \right)\left( E_i + h E_i + \cdots + h^{d-1}E_i \right) = E_i - h^{d}E_i$, meaning $h\in I_i$, and $h^{d}E_i\in I$, so $g^{\ast}\left( e_i + h^{\ast}e_i + \cdots + \left( h^{\ast} \right)^{d-1}e_i \right) = e_i$.\newline

  Now, we may show that $\varphi$ is an isomorphism. If $\varphi\left( f \right) = 0$, then for each $i$ there is $g_i$ with $g_i\left( P_i \right) \neq 0$, and $g_i f\in I$, where $f$ is the residue of $F$ in $I$. Let $t_ig_i = e_i$ (where $g_i$ is the residue of $G_i$ in $I$). Then, $f = \sum e_i f = \sum t_ig_i f = 0$.\newline

  Now, since $E_i\left( P_i \right)=1$, $\varphi_i\left( e_i \right)$ is a unit in $R_i$, and since $\varphi_i\left( e_i \right)\varphi_i\left( e_j \right) = \varphi_i\left( e_ie_j \right) = 0$ if $i\neq j$, then $\varphi_i\left( e_j \right) = 0$ for all $i\neq j$. Thus, $\varphi\left( e_i \right) = \varphi_i\left( \sum e_j \right) = 1$. Now, if $z = \left( a_1/s_1,\dots,a_N/s_N \right)\in \prod R_i$, we may find $t_i$ such that $t_is_i = e_i$ by our earlier claim. Therefore, $a_i/s_i = a_it_i$, so $\varphi_i\left( \sum t_ja_je_j \right) = \varphi_i\left( t_ia_i \right) = a_i/s_i$, and $\varphi\left( \sum t_ja_je_j \right) = z$.
\end{proof}
\begin{corollary}
  \begin{align*}
    \Dim_{k}\left( k\left[ x_1,\dots,x_n \right]/I \right) &= \sum_{i=1}^{n}\Dim_{k}\left( \mathcal{O}_i/I\mathcal{O}_i \right).
  \end{align*}
\end{corollary}
\begin{corollary}
  If $V(I) = \set{P}$, then $k\left[ x_1,\dots,x_n \right]/I$ is isomorphic to $\mathcal{O}_P\left( \A^n \right)/I\mathcal{O}_P\left( \A^n \right)$.
\end{corollary}
\begin{exercise}[Exercise 2.47]
...
\end{exercise}
\begin{solution}
  Since $R$ is a finite-dimensional $k$-vector space that is also ring-finite over $k$, we may consider $R$ to be of the form $k\left[ a_1,\dots,a_n \right]$, which is isomorphic to $k\left[ x_1,\dots,x_n \right]/\left\langle x_1-a_1,\dots,x_n-a_n \right\rangle$, so it is isomorphic to the product established in the proposition.
\end{solution}
\subsection{Quotient Modules and Exact Sequences}%
Let $R$ be a ring, with $M,M'$ $R$-modules A homomorphism of abelian groups $\varphi\colon M\rightarrow M'$ of abelian groups is called an $R$-module homomorphism if $\varphi\left( am \right) = a\varphi\left( m \right)$. If $\varphi$ is bijective, then we say it is an $R$-module isomorphism.\newline
...
If $\psi\colon M'\rightarrow M$ and $\varphi\colon M\rightarrow M''$ are $R$-module homomorphisms, we say the sequence
\begin{align*}
  M'\xrightarrow{\psi}M\xrightarrow{\varphi}M''
\end{align*}
is \textit{exact} at $M$ if $\img\left( \psi \right) = \ker\left( \varphi \right)$. Note that since there are unique $R$-module homomorphisms from $0$ to $M$ and $M$ to $0$, $\varphi$ is surjective if and only if $M\xrightarrow{\varphi}M''\rightarrow 0$ is exact, and $\psi$ is injective if and only if $0\rightarrow M'\xrightarrow{\psi}M$ is exact.\newline

If $\varphi_i\colon M_i\rightarrow M_{i+1}$ are $R$-module homomorphisms, we say
\begin{align*}
  M_1\xrightarrow{\varphi_1}M_2\xrightarrow{\varphi_2}\cdots\xrightarrow{\varphi_n}M_{n+1}
\end{align*}
is exact if $\img\left( \varphi_i \right) = \ker\left( \varphi_{i+1} \right)$ for all $i$.
\begin{exercise}[Exercise 2.49]\hfill
  \begin{enumerate}[(a)]
    \item Let $N$ be a submodule of $M$, $\pi\colon M\rightarrow M/N$ the natural homomorphism. Suppose $\varphi\colon M\rightarrow M'$ is a homomorphism of $R$-modules, and $\varphi\left( N \right)=0$. Show that there is a unique homomorphism $ \overline{\varphi}\colon M/N\rightarrow M' $ such that $ \overline{\varphi}\circ\pi = \varphi $.
    \item If $N$ and $P$ are submodules of $M$ with $P\subseteq N$, then there are natural homomorphisms from $M/P$ to $M/N$ and from $N/P$ to $M/P$. Show that the resulting sequence,
      \begin{align*}
        0\rightarrow N/P \rightarrow M/P \rightarrow M/N \rightarrow 0
      \end{align*}
      is exact.
    \item Let $U\subseteq W \subseteq V$ be vector spaces, with $V/U$ finite dimensional. Then, $\Dim \left( V/U \right) = \Dim\left( V/W \right) + \Dim\left( W/U \right)$.
    \item If $J\subseteq I$ are ideals in $R$, then there is a natural exact sequence of $R$-modules,
      \begin{align*}
        0\rightarrow I/J \rightarrow R/J \rightarrow R/I \rightarrow 0.
      \end{align*}
    \item If $\mathcal{O}$ is a local ring with maximal ideal $\mathfrak{m}$, then there is a natural exact sequence of $\mathcal{O}$-modules
      \begin{align*}
...
\begin{solution}\hfill
\begin{enumerate}[(a)]
  \item Let $ \overline{\varphi}\left( a + N \right) = \varphi\left( a \right) $. This definition is independent of representatives of $\left[ a \right]_M$ as $a + N = b + N$ differ by a factor of $m\in N$, which is evaluated to $0$. The definition is unique as it is defined solely via $\varphi$.
  \item The inclusion map $N/P\hookrightarrow M/P$ is injective, and the map $M/P\rightarrow M/N$ given by $a + P \mapsto a + N$ is surjective with kernel $N/P$, so the sequence is exact.
  \item This follows from rank-nullity on the exact sequence $0\rightarrow W/U \rightarrow V/U \rightarrow V/W \rightarrow 0$.
  \item We define the first map to be inclusion, and the second map to be $a + J \mapsto a + I$. Inclusion is injective, and the map $a + J \mapsto a + I$ has kernel $I/J$.
  \item Since $\mathfrak{m}^{n+1}\subseteq \mathfrak{m}^{n}$, this follows from (d).
\end{enumerate}
\end{solution}
\begin{exercise}[Exercise 2.50]
  Let $R$ be a DVR. Then, $\mathfrak{m}^n/\mathfrak{m}^{n+1}$ is an $R$-module that is also a $k$-module.
  \begin{enumerate}[(a)]
    \item Show that $\Dim_{k}\left( \mathfrak{m}^{n}/\mathfrak{m}^{n+1} \right) = 1$ for all $n\geq 0$.
    \item Show that $\Dim_{k}\left( R/\mathfrak{m}^{n} \right) = n$ for all $n > 0$.
    \item Let $z\in R$. Show that $\operatorname{ord}(z) = n$ if $\left\langle z \right\rangle = \mathfrak{m}^{n}$, hence $\operatorname{ord}\left( z \right) = \Dim_{k}\left( R/\left\langle z \right\rangle \right)$.
  \end{enumerate}
\end{exercise}
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    \item We note that
      \begin{align*}
        \mathfrak{m}^{n}/\mathfrak{m}^{n+1} &\cong \left( R/\mathfrak{m} \right)\mathfrak{m}^{n}\\
                                            &\cong k \mathfrak{m}^{n}\\
                                            &= k.
      \end{align*}
    \item Assuming the inductive hypothesis, we construct the exact sequence $0\rightarrow \mathfrak{m}^n/\mathfrak{m}^{n+1}\rightarrow R/\mathfrak{m}^{n+1}\rightarrow R/\mathfrak{m}^{n}\rightarrow 0$. Rank-nullity gives $\Dim_k\left( R/\mathfrak{m}^{n+1} \right) = 1 + \Dim_{k}\left( R/\mathfrak{m}^{n} \right)$.
    \item If $\left\langle z \right\rangle = \mathfrak{m}^{n}$, then $\left\langle z \right\rangle = \left\langle t^{n} \right\rangle$, where $t$ is the uniformizing parameter of $R$. Thus, $\operatorname{ord}\left( z \right) = n$.
  \end{enumerate}
\end{solution}
\section{Local Properties of Plane Curves}%

\subsection{Multiple Points and Tangent Lines}%
Affine plane curves correspond to nonconstant polynomials $F\in k\left[ x,y \right]$ without multiple factors, where $F$ is determined up to multiplication by a nonzero constant.\newline

We modify this slightly. We say $F$ and $G$ are equivalent if $F = \lambda G$ for some nonzero $\lambda\in k$. An \textit{affine plane curve} is an equivalence class of nonconstant polynomials under this equivalence relation. The degree of a curve is the degree of the defining polynomial for the curve.\newline
...
If $F$ is irreducible, then $V(F)$ is a variety in $\A^2$. We will write $\Gamma(F),k(F),\mathcal{O}_P(F)$ instead of $\Gamma\left( V(F) \right),k\left( V(F) \right),\mathcal{O}_P\left( V(F) \right)$.\newline

The point $P = (a,b)\in F$, where $F$ is a curve, is called a simple point if either $F_X(P) \neq 0$ or $F_Y(P)\neq 0$, with the line $F_X(P)\left( X-a \right) + F_Y(P)\left( Y-b \right) = 0$ as the tangent line to $F$ at $P$. A curve with only simple points is called a nonsingular curve.\newline
...
Since $F_m$ is a form in two variables, we may write $F_m = \prod_{i}L_i^{r_i}$, where the $L_i$ are distinct lines. The $L_i$ are called tangent lines to $F$ at $P = (0,0)$, and $r_i$ is the multiplicity of the tangent.
\subsection{Multiplicities and Local Rings}%
Let $F$ be an irreducible plane curve with $P\in F$. We find the multiplicity of $P$ in $F$ in terms of the local ring $\mathcal{O}_P(F)$. We will denote the residue of any polynomial $G\in k\left[ X,Y \right]$ in $\Gamma(F)$ by $g$.
\begin{theorem}
  The point $P$ is a simple point of $F$ if and only if $\mathcal{O}_P(F)$ is a discrete valuation ring. In this case, if $L = aX + bY + C$ is any line through $P$ that is not tangent to $F$ at $P$, the image $l$ of $L$ in $\mathcal{O}_P(F)$ is a uniformizing parameter for $\mathcal{O}_P(F)$.
\end{theorem}
\begin{proof}
  Let $P$ be a simple point on $F$, and let $L$ be a line through $P$ not tangent to $F$ at $P$. Via an affine transformation, we may assume $P = (0,0)$, the $Y$-axis is the tangent line to $P$, and $L$ is the $X$-axis. It suffices to show that $\mathfrak{m}_P(F)$ is generated by $x$.\newline

  Note that $\mathfrak{m}_P(F) = \left\langle x,y \right\rangle$, whether $P$ is simple or not, and by assumption $F = Y + \text{higher terms}$. Grouping higher terms with factors of $Y$ together, we may write $F = YG - X^2 H$,\footnote{Note that $Y$ is a $1$-form, so everything else is a $2$-form and above, and we have grouped all the terms with $Y$.} where $G = 1 + \text{higher terms}$ and $H\in k\left[ X \right]$.\newline

  Then, $yg = x^2h\in \Gamma(F)$, so $y = x^2hg^{-1}\in \left\langle x \right\rangle$, since $g(P)\neq 0$. Therefore, $\mathfrak{m}_P(F) = \left\langle x,y \right\rangle = \left\langle x \right\rangle$ as desired.
\end{proof}
