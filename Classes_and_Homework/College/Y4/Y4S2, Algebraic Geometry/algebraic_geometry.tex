...
A \textit{change of coordinates} on $\A^n$ is a polynomial map $T\colon \A^n\rightarrow \A^n$ such that each $T_i$ is a polynomial of degree $1$ and $T$ is bijective. If $T_i = \sum a_{ij}x_j + a_0$, then $T = T'' \circ T'$, where $T'$ is a linear map and $T''$ is a translation. Since translations are invertible, it follows that $T$ is bijective if and only if $T'$ is invertible.\newline
