  ...
  \begin{description}
    ...
    \item[The Roy Model:] Understanding teacher supply based on skills and outside options. 
      \begin{itemize}
        ...
        \item Teaching has structure $w_T= \alpha_T + \beta_T s$ and other careers has structure $w_O = \alpha_O + \beta_O s$
      \end{itemize}
      We can see some potential changes in the $w$/$s$ relationship.
      \begin{itemize}
        \item Alternative labor market options increase $\alpha_O$ for people at all skill levels.
        \item Teacher unionization compresses the wage scale, meaning $s$ is not rewarded, so $\beta_T$ reduces.
      \end{itemize}
  \end{description}
\end{document}
