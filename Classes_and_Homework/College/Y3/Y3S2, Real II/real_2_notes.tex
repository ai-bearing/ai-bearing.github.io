...
\end{enumerate}
  ...
  \begin{description}
    \item[Proof:] We are aware of the fact that $C([0,1])$ under $\norm{\cdot}_{u}$ is complete with respect to the induced metric. We will show that the set of differentiable functions at any point $x\in [0,1]$ is meager in $C([0,1])$, thus proving that differentiable nowhere functions are residual.\\

      Let $A_n\subseteq C([0,1])$ denote the set of functions where there exists $x\in (0,1)$ with
      \begin{align*}
        \left|\frac{f(x)-f(y)}{x-y}\right| \leq n
      \end{align*}
      for all $y$ with $0 < |x-y| \leq 1/n$. All differentiable functions must be an element of $A_n$ for some $n$.\\

      To show $A_n$ is nowhere dense, we show that $A_n$ is closed and contains no open ball.\\

      Let $(f_j)_j$ be a sequence in $A_n$ with $(f_j)_j \rightarrow f$. For each $j$, there exists an $x_j$ such that
      \begin{align*}
        \left|\frac{f_j(x_j)-f_j(y)}{x_j-y}\right| \leq n
      \end{align*}
      for $0 < |x_j - y| \leq 1/n$. By the Bolzano--Weierstrass theorem, there is a subsequence $\left(x_{j_k}\right)_k \rightarrow x$. Suppose $0 < |x-y| \leq 1/n$. Then, for all $k$ sufficiently large, $\left|x_{j_k} - y \right| \leq 1/n$, so by continuity and the pointwise convergence of $f$,
      \begin{align*}
        \left|\frac{f(x)-f(y)}{x-y}\right| &= \lim_{k\rightarrow\infty}\left|\frac{f_{j_k}\left(x_{j_k}\right)-f(y)}{x_{j_k}-y}\right|\\
                                           &\leq n.
      \end{align*}
      Since $f$ is continuous (as $C([0,1])$ is complete), the same is true for $|x-y| = 1/n$.\\

      To show that $A_n$ has no open ball, recall from Real Analysis I that for all $f\in C([0,1])$, there exists a piecewise linear function $p_{\delta}$ with $\norm{p_{\delta}-f}_{u} < \delta$ for all $\delta > 0$.\\

      It follows that, if $A_n$ contains a ball $U_{\varepsilon}(f)$, then $A_n$ contains some ball $U_{\varepsilon/2}(p)$ for said piecewise linear function. Set $M$ to be the maximum value of the slopes that $p$ takes.\\
      ...
      \begin{align*}
        ...
        \end{cases},
      \end{align*}
      with $g$ linear between $[j/M',(j+1)/M']$.\\

      The function $k = f + \frac{1}{2}\varepsilon g$ defines $\norm{f-k} < \varepsilon$, but its slope near any point has magnitude at least $\frac{1}{2}\varepsilon M' - n$. If $M'$ is large enough, this value is larger than $M$.\\

      Thus, there exists $k\in U_{\varepsilon}(f)$ with $k\notin A_n$ for any $A_n$, implying that $A_n^{\circ} = \emptyset$.
  \end{description}
  \section{Compactness}%
  ...
  \end{itemize}
  \subsection{Riesz--Markov--Kakutani Representation Theorem}%
