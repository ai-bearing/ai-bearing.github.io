  ...
  \begin{description}
    \item[Proof:] We know that $C_0(\R)\subseteq C_b(\R)$, meaning we need show $C_0(\R)$ is closed under the uniform norm.\\
      ...
      \begin{align*}
        |f(x)| &= |f(x) - f_n(x) + f_n(x)|\\
               &\leq |f_n(x) - f(x)| + |f_n(x)|.
       \intertext{By the definition of uniform convergence, for all $n\geq N_{\varepsilon}$ it is the case that $|f_n(x)-f(x)| < \varepsilon/2$, and by the definition of vanishing at $\pm \infty$, for all $|x| > M$, $|f_n(x)| < \varepsilon/2$. Thus,}
               &< \varepsilon,
      \end{align*}
