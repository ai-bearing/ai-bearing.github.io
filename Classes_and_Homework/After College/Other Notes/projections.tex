...
\begin{proof}\hfill
  \begin{enumerate}[(i)]
    ...
    \item If each $p_i$ is abelian with pairwise orthogonal central supports, then
      \begin{align*}
        pMp &= \sum_{i\in I} z\left( p_i \right) p_i M p_i
      \end{align*}
      is a direct sum of abelian von Neumann algebras, and so is abelian. By Zorn's lemma, we may find a maximal family of these pairwise orthogonal projections. If there were some abelian projection $q\leq 1-z(p)$, then the collection would not be maximal, meaning that $z = c(p)$ dominates every abelian projection and is the smallest central projection with this property.

      Now, if each $p_i$ is finite, and $q\leq p$ with $q\sim p$, then we have $q_i \coloneq qz\left( p_i \right)\leq p_i$ and $q_i\sim p_i$. Since each $i$ is finite, it follows that $q_i\sim p_i$ for all $i\in I$, so $q = p$ and $p$ is finite. Similarly to the abelian case, we could take a maximal family of finite projections with pairwise orthogonal central supports, so that $z = c(p)$ majorizes every finite projection $q$, else $q\left( 1-z \right)$ would be finite. Since $p$ is finite, it follows that $z$ is the smallest central projection with this property.

      Now, let $\left( z_i \right)_i$ be a maximal family of pairwise orthogonal finite central projections. Then, $z = \sum_{i\in I}z_i$ is finite and central. If there were some finite central projection $p$ such that $p\nleq z$, then $\left( 1-z \right)p$ would be a nonzero finite central projection orthogonal to $z$, contradicting the maximality of our family. Thus, $z$ is the largest finite central projection.
    \item We suppose that $e$ and $f$ are abelian projections with $z(e) = z(f) = 1$. By the comparison theorem, there is a central projection $w$ such that $we \sim wf_0\leq wf$ and $ \left( 1-w \right)f \sim \left( 1-w \right)e_0\leq \left( 1-w \right)e $. Note that $z\left( we \right) = z\left( wf_0 \right) = z\left( wf \right) = w$. If it were the case that $wf_0 < wf$, then there is some $a\in M$ such that
      \begin{align*}
        0 &\neq w\left( f-f_0 \right)awf_0\\
          &= w\left( f-f_0 \right)\left( wfawf \right)wf_0\\
          &= w\left( f-f_0 \right)wf_0\left( wfawf \right)\\
          &= 0,
      \end{align*}
      where we use the fact that $wf$ is an abelian projection. Therefore, we have $wf_0 = wf$, and similarly, $\left( 1-w \right)e_0 = \left( 1-w \right)e$. Therefore, $e = \left( 1-w \right)e + we \sim wf + \left( 1-w \right)f = f$.
  \end{enumerate}
\end{proof}
\begin{corollary}
  Let $p$ be an abelian projection in a von Neumann algebra $M$. Then, $z(p)Z(m)\cong pMp$ via the map $z(p)w \mapsto pw$.
\end{corollary}
\begin{proof}
  We have that $M'p = pMp$, and since $p$ is an abelian projection, we have
  \begin{align*}
    pMp &= Z\left( pMp \right)\\
        &= pMp\cap M'p\\
        &= p\left( M\cap M' \right)p\\
        &= pZ\left( M \right).
  \end{align*}
  Since $w\mapsto pw$ is a $\ast$-homomorphism with $\left( 1-z(p) \right)Z(m)$, we have an isomorphism with $z(p)Z(M)$.
\end{proof}
\begin{definition}
  A von Neumann algebra is called 
  \begin{itemize}
    \item type I if every nonzero projection majorizes a nonzero abelian projection;
    \item type II\textsubscript{1} if it is finite and there are no nonzero abelian projections;
    \item type II\textsubscript{$\infty$} if it is semifinite, properly infinite, and there are no nonzero abelian projections;
    \item type III if it is purely infinite.
  \end{itemize}
  Furthermore, we call a von Neumann algebra atomic if every nonzero projection majorizes a minimal projection. A von Neumann algebra with no minimal projections is called diffuse.
\end{definition}
\begin{theorem}[Type Decomposition]
  Let $M$ be a von Neumann algebra. Then, there are unique central projections $z_1,z_2,z_3,z_4$ such that $1 = z_1 + z_2 + z_3 + z_4$, $Mz_1$ is type I, $Mz_2$ is type II\textsubscript{1}, $Mz_3$ is type II\textsubscript{$\infty$}, and $Mz_4$ is type III respectively.
\end{theorem}
Note that all abelian von Neumann algebras are type I, as well as $B(H)$, the former since the identity is an abelian projection and the latter since every projection majorizes a rank $1$, projection, which is a minimal (and hence abelian) projection. Furthermore, a factor is only one of these types (since its center is trivial).
\subsection{Tensor Products of Hilbert Spaces and von Neumann Algebras}%
Before we can discuss different von Neumann algebras of the various types, we need to discuss the tensor product of von Neumann algebras.
\begin{definition}
  Let $H$ and $K$ be Hilbert spaces. Then, there is an inner product on the algebraic tensor product $H\odot K$ given by
  \begin{align*}
...
\end{definition}
\begin{proposition}
  Let $H$ and $K$ be Hilbert spaces with orthonormal bases $\left( e_i \right)_i$ and $\left( f_j \right)_j$. Then,
  \begin{enumerate}[(i)]
    \item $\set{e_i\otimes f_j}_{i\in I,j\in J}$ is an orthonormal basis for $H\otimes K$;
    \item if $ \left\vert J \right\vert = \alpha $, then $H\otimes K \cong H^{(\alpha)}\cong \bigoplus_{j\in J}H$;
    \item if $H = L_2\left( \mu \right)$ for some $\sigma$-finite regular Borel measure space $\left( X,\mu \right)$, and $K$ is separable, then if $L_2\left( X,\mu,K \right)$ denotes the space of square-integrable Borel functions, we have $H\otimes K\cong L_2\left( X,\mu,K \right)$.
  \end{enumerate}
\end{proposition}
\begin{proof}\hfill
  \begin{enumerate}[(i)]
    \item We observe that the set $\set{e_i\otimes f_j}_{i\in I,j\in J}$ is an orthonormal set; the spans of these elementary tensors are all vectors of the form $x\otimes y$, provided $x$ is a finite linear combination of the $e_i$ and $y$ is a finite linear combination of the $f_j$. Therefore, the completion is equal to the completion of $H\otimes K$, meaning the set if an orthonormal basis.
    \item Considering
      \begin{align*}
        H\otimes K &\cong \bigoplus_{j\in J}H\otimes \C f_j,
      \end{align*}
      we find that this is an $\ell_2$ direct sum of $\left\vert J \right\vert$ copies of $H$.
    \item Define maps $Y_j\colon L_2\left( X,\mu,K \right)\rightarrow L_2\left( \mu \right)$ by
      \begin{align*}
        \left( Y_jf \right)(x) &= \iprod{f(x)}{f_j}
      \end{align*}
      modulo $\mu$. Then, we may define
      \begin{align*}
        Y\colon L_2\left( X,\mu,K \right) &\rightarrow \bigoplus_{j\in J} H\otimes \C f_j\\
        f &\mapsto \sum_{j\in J}Y_jf.
      \end{align*}
      Using Tonelli's theorem and Parseval's identity, we may then compute
      \begin{align*}
        \norm{Yf}^2 &= \sum_{j\in J}\norm{Y_jf}^2\\
                    ...
                    &= \norm{f}^2.
      \end{align*}
      Therefore, $Y$ is an isometry. The range is necessarily dense, since if $h\in L_2\left( \mu \right)$, then $f = hf_j$ is mapped to the vector with $h$ in position $j$ and $0$ elsewhere. Thus, $Y$ is a unitary map of $L_2\left( X,\mu,K \right)$ onto $H\otimes K$.
  \end{enumerate}
\end{proof}
\begin{proposition}
  Let $A\in B(H)$ and $B\in B(K)$. Then, there is a unique operator $A\otimes B\in B\left( H\otimes K \right)$ such that $\left( A\otimes B \right)\left( \xi\otimes \eta \right) = A\xi\otimes B\eta$. Moreover, $\norm{A\otimes B} = \norm{A}\norm{B}$.
\end{proposition}
\begin{proof}
  Consider the amplification $A\otimes I_{K}$. Fixing an orthonormal basis $\set{\eta_j}_{j\in J}$ for $K$, then any vector $\omega\in H\otimes K$ can be expressed as
  \begin{align*}
    \omega &= \left( \zeta_j\otimes \eta_j \right)_{j}
  \end{align*}
  where $\zeta_j\in H$ and 
  \begin{align*}
  ...
  Then, we may rewrite
  \begin{align*}
    \left( A\otimes I \right)\omega &= \left( A\zeta_j\otimes \eta_j \right)_{j},
  \end{align*}
  with
  \begin{align*}
    \norm{\left( A\otimes I \right)\omega}^2 &= \norm{ \left( A\zeta_j\otimes \eta_j \right)_{j} }^2\\
                                             &= \sum_{j\in J}\norm{A\zeta_j}^2\\
                                             &\leq \norm{A}^2 \sum_{j\in J}\norm{\zeta_j}^2\\
                                             &= \left( \norm{A}\norm{\omega} \right)^2.
  \end{align*}
...
\subsection{Type II\textsubscript{1} von Neumann Algebras}%
