...
\end{align*}
where $ \iprod{\cdot}{\cdot} $ is the canonical duality for $X$.\footnote{I.e., if $\varphi\in X^{\ast}$ and $x\in X$, then $ \iprod{x}{\varphi} = \hat{x}(\varphi) = \varphi(x) $.} In particular, from the Hahn--Banach Theorems, we get the expression for the projective norm
\begin{align*}
...
We now focus on the special case of the tensor product $L_1\left( \mu \right)\hat\otimes X$, which is a Banach space of integrable vector-valued functions. To understand this space further, we need to develop an integration theory for functions with values in a Banach space.

Let $\left( \Omega,\Sigma,\mu \right)$ be a measure space. Recall that a function $f\colon \Omega\rightarrow X$ is called simple there are a finite number of distinct values, where we may partition $\Omega$ into $E_1,\dots,E_n$ and distinct nonzero $x_1,\dots,x_n$ in $X$ such that $f(\omega) = x_i$ whenever $\omega\in E_i$. We may write
\begin{align*}
  f &= \sum_{i=1}^{n}\chi_{E_i}x,
\end{align*}
where $\chi_{E_i}$ denotes the characteristic function of the set $E_i$. If $E_i\in \Sigma$ for each $i$, then $f$ is also a measurable function.

We will fire off a bunch of useful notions, most of which originate in measure theory.
\begin{itemize}
  \item An arbitrary function $f\colon \Omega\rightarrow X$ is called measurable if there is a sequence $\left( f_n \right)_n$ of simple measurable functions converging almost everywhere to $f$.
  \item If $f$ is measurable, then so too is $\norm{f}$ (by the reverse triangle inequality).
  \item The function $f\colon \Omega\rightarrow X$ is called \textit{weakly} measurable if the scalar-valued function $\varphi(f)$ is measurable for each $\varphi\in X^{\ast}$.
  \item If $f^{-1}(S)$ is measurable for every open subset $S\subseteq X$, then we say $f$ is Borel measurable.
  \item The function $f\colon \Omega\rightarrow X$ is called essentially separably valued if there is $E\subseteq \Omega$ with $\mu\left( \Omega\setminus E \right) = 0$ and $f(E)$ is contained in a separable subspace $Y$ of $X$.
  \item If $\mu$ is $\sigma$-finite, then $f$ is $\mu$-measurable if and only if $\chi_E f$ is $\mu$-measurable for every $E\in \Sigma$ with finite measure.
\end{itemize}
There is a mild distinction between weakly measurable, measurable, and Borel measurable as we will show below.
\begin{proposition}[Pettis Measurability Theorem]
  Let $\left( \Omega,\Sigma,\mu \right)$ be a $\sigma$-finite measure space. If $f\colon \Omega\rightarrow X$ is a function, then the following are equivalent:
  \begin{enumerate}[(i)]
    \item $f$ is measurable;
    \item $f$ is weakly measurable and essentially separably valued;
    \item $f$ is Borel measurable and essentially separably valued.
  \end{enumerate}
\end{proposition}
\begin{proof}
  We start by showing that (i) implies (ii). If $f$ is measurable, and $\left( f_n \right)_n\rightarrow f$ converges $\mu$-a.e., then for any $\varphi\in X^{\ast}$, we have that $\left( \varphi\left( f_n \right) \right)_n$ is a sequence of scalar-valued measurable functions that converge almost everywhere to $\varphi(f)$, so $\varphi(f)$ is measurable. Letting $Y$ be the subspace generated by the ranges of the $f_n$, then $Y$ is separable and $f(\Omega)$ is contained in $Y$.

  We show that (ii) implies (iii). If $f$ is weakly measurable, and $f(\Omega)$ is essentially contained in a separable subspace $Y$ of $X$, then we may assume without loss of generality that $f(\Omega)\subseteq Y$. Since the embedding of $Y$ into $X$ is continuous, it suffices to prove that $f$ is Borel measurable as a function from $\Omega$ into $Y$. Since $\varphi(f)$ is measurable for every $\varphi\in X^{\ast}$, then $f$ is Borel measurable in the weak topology of $Y$; that is, $f^{-1}(U)\in \Sigma$ for every weakly open subset $U$ of $Y$. Thus, this holds for weakly closed subsets of $Y$. Any norm-closed ball in $Y$ is convex, hence weakly closed, and since $Y$ is separable, every open $U\subseteq Y$ is the union of a countable number of closed balls, so $f^{-1}(U)\in \Sigma$, meaning $f$ is Borel measurable.

  Finally, we show that (iii) implies (i). Assume that $f$ is Borel measurable, with $f(\Omega)$ contained in a separable subspace $Y$ of $X$ (without loss of generality). It suffices to assume that $\mu$ is finite. Let $\set{y_k}_{k\geq 1}$ be a countable dense subset of $Y$; then, for any $n$, the union of the open balls $U\left( y_k,1/n \right)$ is $Y$. By the Borel measurability of $f$, $E_{k}^n\coloneq f^{-1}\left( U\left( y_k,1/n \right) \right)$ are measurable and cover $\Omega$ for each $n$. Convert this to a disjoint cover by setting $F_1^n = E_1^n$ and $F_k^n = E_k^n\setminus \left( E_1^n\cup\cdots\cup E_{k-1}^n \right)$ for each $k$. For each $n$, let 
  \begin{align*}
    g_n &= \sum_{k=1}^{\infty}\chi_{F_k^n}y_k.
  \end{align*}
  Then, $\norm{f\left( \omega \right)-g\left( \omega \right)} < 1/n$ for each $\omega\in \Omega$. Therefore, the sequence $\left( g_n \right)_n$ of countably valued functions converges uniformly to $f$. We will truncate these expressions to get a sequence of measurable simple functions that converge $\mu$-a.e. to $f$. Choose $m_n$ such that
  \begin{align*}
    \mu\left( \bigcup_{k=m_n+1}^{\infty}F_k^n \right) &\leq 2^{-n},
  \end{align*}
  and let
  \begin{align*}
    h_n &= \bigcup_{k=1}^{m_n} \chi_{F_k^n}y_k,
  \end{align*}
  so that $\norm{f(\omega)-h_n(\omega)} < 1/n$ for each $\omega\in \Omega\setminus C_n$, where we let
  \begin{align*}
    C_n &= \bigcup_{k=m_n+1}^{\infty}F_k^n.
  \end{align*}
  Let $C = \limsup_{k\rightarrow\infty}C_k$; then by Borel--Cantelli, $C$ is a null set, and for any $\omega\notin C$, there is $n$  such that $\omega\notin C_k$ for any $k\geq n$, meaning that $\norm{f(\omega)-h_n(\omega)} < 1/k \leq 1/n$ for every $k\geq n$.
\end{proof}
We may now define the integral here. Let $f$ be a measurable simple function with representation
\begin{align*}
  f &= \sum_{i=1}^{n}\chi_{E_i}x_i,
\end{align*}
and suppose each $A_i$ has finite measure. We define
\begin{align*}
  \int_{E}^{} f\:d\mu &= \sum_{i=1}^{n} \mu\left( A_i\cap E \right)x_i.
\end{align*}
A measurable function $f\colon \Omega\rightarrow X$ is called \textit{Bochner integrable} if there exists a sequence of simple functions $\left( f_n \right)_n\rightarrow f$ $\mu$-a.e. and
\begin{align*}
  \lim_{n\rightarrow\infty}\int_{\Omega}^{} \norm{f_n-f}\:d\mu &= 0.
\end{align*}
The Bochner integral of $f$ over $E\in \Sigma$ is then given by
\begin{align*}
  \int_{E}^{} f\:d\mu &= \lim_{n\rightarrow\infty} \int_{E}^{} f_n\:d\mu.
\end{align*}
We can actually characterize Bochner integrability as follows.
\begin{theorem}[Bochner's Theorem]
  If $f\colon \Omega\rightarrow X$ is a measurable function, then $f$ is Bochner-integrable if and only if $\norm{f}$ is integrable.
\end{theorem}
\begin{proof}
  Suppose $f$ is Bochner integrable. Let $\left( f_n \right)_n\rightarrow f$ be a sequence of simple functions converging $\mu$-a.e. as in the definition of the Bochner integral. Then,
  \begin{align*}
    \int_{\Omega}^{} \norm{f}\:d\mu &= \int_{\Omega}^{} \norm{\left( f-f_n \right) + f_n}\:d\mu\\
                                    &\leq \int_{\Omega}^{} \norm{f_n-f}\:d\mu + \int_{\Omega}^{} \norm{f_n}\:d\mu,
  \end{align*}
  for each $n$, so the integral of $\norm{f}$ is finite. Conversely, if $\norm{f}$ is integrable, and $\left( f_n \right)_n$ converges $\mu$-a.e. to $f$, we fix $\delta > 0$, and define
  \begin{align*}
    g_n(\omega) &\coloneq \begin{cases}
      f_n(\omega) & \norm{f_n(\omega)} \leq \left( 1+\delta \right)\norm{f(\omega)}\\
      0 & \text{else}
    \end{cases}.
  \end{align*}
  Then, $\left( g_n \right)_n$ is a sequence of measurable simple functions that converges $\mu$-a.e. to $f$. The sequence of scalar functions $\left( \norm{f-g_n} \right)_n$ is dominated by the integrable function $\left( 2+\delta \right)\norm{f}$, so by dominated convergence, we have
  \begin{align*}
    \int_{\Omega}^{} \norm{f-g_n}\:d\mu &\rightarrow 0.
  \end{align*}
\end{proof}
There is a version of the triangle inequality for the Bochner integral.
\begin{corollary}
  If $f\colon \Omega\rightarrow X$ is Bochner-integrable, then
  \begin{align*}
    \norm{ \int_{\Omega}^{} f\:d\mu } &\leq \int_{\Omega}^{} \norm{f}\:d\mu.
  \end{align*}
\end{corollary}
\begin{proof}
  Let $\left( f_n \right)_n\rightarrow f$ be a sequence of measurable simple functions converging $\mu$-a.e. to $f$, and
  \begin{align*}
    \lim_{n\rightarrow\infty} \int_{\Omega}^{} f_n\:d\mu &= \int_{\Omega}^{} f\:d\mu.
  \end{align*}
  Then, $\left( \norm{f_n} \right)_n\rightarrow \norm{f}$, so that
  \begin{align*}
    \norm{ \int_{\Omega}^{} f\:d\mu } &= \norm{\lim_{n\rightarrow\infty} \int_{\Omega}^{} f_n\:d\mu}\\
                                      &\leq \lim_{n\rightarrow\infty} \int_{\Omega}^{} \norm{f_n}\:d\mu\\
                                      &= \int_{\Omega}^{} \norm{f}\:d\mu.
  \end{align*}
\end{proof}
Additionally, operators respect the Bochner integral.
\begin{proposition}
  Let $f\colon \Omega\rightarrow X$ be Bochner integrable. If $T\in B\left( X,Y \right)$, then $Tf\colon \Omega\rightarrow Y$ is Bochner integrable, with
  \begin{align*}
    \int_{\Omega}^{} Tf\:d\mu &= T\left( \int_{\Omega}^{} f\:d\mu \right).
  \end{align*}
\end{proposition}
