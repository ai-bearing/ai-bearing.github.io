...
\end{problem}
\begin{solution}
By using Fatou's Lemma, and assuming without loss of generality that $\left( f_n \right)_n\rightarrow f$ pointwise everywhere, we get
\begin{align*}
...
\begin{enumerate}[(a)]
  \item Since the given limit is equal to zero, it holds for both the real and imaginary components; thus, we see that
    ...
    \begin{align*}
      \int_{\R}^{} \left( \pi x \right)^2\:d\mu(x) &= \int_{\R}^{} \lim_{\xi\rightarrow 0}\left( \frac{\sin\left( \pi x \xi \right)}{\xi} \right)^2\:d\mu\\
                                                   &= \int_{\R}^{} \liminf_{\xi\rightarrow 0} \left( \frac{\sin\left( \pi x \xi \right)}{\xi} \right)^2\:d\mu(x)\\
                                                   &\leq \liminf_{\xi\rightarrow 0} \int_{\R}^{} \left( \frac{\sin\left( \pi x \xi \right)}{\xi} \right)^2\:d\mu(x)\\
                                                   &= \lim_{\xi\rightarrow 0} \int_{\R}^{} \left( \frac{\sin\left( \pi x \xi \right)}{\xi} \right)^2\:d\mu(x)\\
                                                   &= 0,
    \end{align*}
    so that
    \begin{align*}
