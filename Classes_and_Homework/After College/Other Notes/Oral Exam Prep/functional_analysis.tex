...
\end{proof}
\begin{corollary}
  If $X$ is a reflexive Banach space, and $M\leq X$ is a closed subspace, then $M$ is a reflexive Banach space.
\end{corollary}
\begin{proof}
  Notice that the unit ball of $M$ is the intersection of $M$ with the unit ball of $X$, so the unit ball of $M$ is $\sigma\left( X,X^{\ast} \right)$-compact. Since $\sigma\left( X,X^{\ast} \right)|_{M} = \sigma\left( M,M^{\ast} \right)$, it follows that the unit ball of $M$ is weakly compact with respect to $\sigma\left( M,M^{\ast} \right)$.
\end{proof}
\begin{corollary}
  If $X$ is reflexive, then $X$ is weakly sequentially complete.
\end{corollary}
\begin{proof}
  Let $\left( x_n \right)_n$ be weakly Cauchy. Then, $ \left( \iprod{x_n}{\varphi} \right)_n $ is Cauchy for each $\varphi\in X^{\ast}$. So, by the principle of uniform boundedness, $\norm{x_n} \leq M$ for some constant $M$.

  Since $X$ is reflexive, $\set{x\in X : \norm{x}\leq M}$ is weakly compact. So, there is some $x\in X$ such that $\left( x_{n_k} \right)_{n_k}\rightarrow x$ weakly for some subsequence of $\left( x_n \right)_n$. Since $\left( \iprod{x_n}{\varphi} \right)_n$ converges for all $\varphi\in X^{\ast}$, it follows that $ \iprod{x_n}{\varphi}\rightarrow \iprod{x}{\varphi} $ for all $\varphi\in X^{\ast}$, whence $\left( x_n \right)_n\rightarrow x$ weakly.
\end{proof}
\begin{corollary}
  If $X$ is a reflexive Banach space, $M\leq X$ is a closed subspace, and $x_0\in X\setminus M$, then there is some $y_0\in M$ such that $\norm{x_0-y_0} = \dist_M\left(x_0\right)$.
\end{corollary}
\begin{proof}
  The map $x\mapsto \norm{x-x_0}$ is weakly lower semicontinuous. If $d = \dist_M\left( x_0 \right)$, then the set $M\cap \set{x : \norm{x-x_0} \leq 2d}$ is weakly compact. Since lower semicontinuous functions attain their minima on compact sets, the result follows.
\end{proof}
Closed subspaces satisfying the property that for any $x_0\in X\setminus M$, there exists $y_0\in M$ such that $\norm{x_0-y_0} = \dist_M\left( x_0 \right)$ are known as proximinal subspaces. Not every subspace of a normed vector space is proximinal.
\begin{proposition}
  Let $X$ be a Banach space, $\varphi\in X^{\ast}$. Then, $\ker\left( \varphi \right)$ is proximinal if and only if there is some $x\in X$ with $\norm{x} = 1$ such that $ \iprod{x}{\varphi} = \norm{\varphi} $.
\end{proposition}
\begin{proof}
  Suppose $M = \ker\left( \varphi \right)$ is proximinal. If $f\colon X/M\rightarrow F$ is given by $f\left( x + M \right) = \iprod{x}{\varphi}$, then $f$ is a linear functional with $\norm{f} = \norm{\varphi}$. Since $X/M$ is one-dimensional, there is $x\in X$ such that $\norm{x + M} = 1$ and $f\left( x + M \right) = \norm{f}$. Since $M$ is proximinal, there is $y\in M$ such that $1 = \norm{x + M} = \norm{x + y}$. Therefore,
  \begin{align*}
    \iprod{x + y}{\varphi} &= \iprod{x}{\varphi}\\
                           &= f\left( x + M \right)\\
                           &= \norm{f}\\
                           &= \norm{\varphi}.
  \end{align*}
  Now, suppose there is $x_0\in X$ with $\norm{x_0} = 1$ and $ \iprod{x_0}{\varphi} = \norm{\varphi} $. If $x\in X$ with $\norm{x + M} = \alpha > 0$, then $\norm{\alpha^{-1}x + M} = 1$; since $\norm{x_0 + M} = 1$ and $X/M$ is one-dimensional, it follows that there is $\beta\in \C$ with $\left\vert \beta \right\vert = 1$ such that $\alpha^{-1}x + M = \beta x_0 + M$, whence $\alpha^{-1}x - \beta x_0\in M$. Equivalently, we have $x-\alpha\beta x_0 \in M$. Since
  \begin{align*}
    \norm{x-\left( x-\alpha\beta x_0 \right)} &= \norm{\alpha\beta x_0}\\
                                              &= \alpha\\
                                              &= \dist_M\left( x \right),
  \end{align*}
  it follows that the distance from $x$ to $M$ is attained at the element $x-\alpha\beta x_0\in M$.
\end{proof}
\subsection{Separability and Metrizability}%
\begin{theorem}
  If $X$ is a Banach space, then $B_{X^{\ast}}$ with the weak* topology is metrizable if and only if $X$ is separable.
\end{theorem}
\begin{proof}
  Let $X$ be separable, $\left( x_n \right)_n$ a countable dense subset of $B_X$. Let $D = \prod_{n=1}^{\infty} B_{\C}\left( 0,1 \right)$. Since $D$ is a compact metric space, it follows that if $B_{X^{\ast}}$ with the weak* topology is homeomorphic to a subset of $D$, then $B_{X^{\ast}}$ is weak* metrizable.

  Let $\tau\colon B_{X^{\ast}}\rightarrow X$ be defined by $\tau\left( \varphi \right) = \left( \iprod{x_n}{\varphi} \right)_{n}\in D$. If $\left( \varphi_i \right)_i$ is a net in $B_{X^{\ast}}$ converging weak* to $\varphi\in X^{\ast}$, it follows that for each $n\geq 1$, we have that $\tau\left( \varphi_i \right)\rightarrow \tau\left( \varphi \right)$, so $\tau$ is weak* continuous. Furthermore, $\tau(\varphi) = 0$ if and only if $\varphi\left( x_n \right) = 0$ for all $n$; since $\varphi$ is then zero on a dense subset, it follows that $\varphi$ is the zero functional, so $\tau$ is injective. Since $B_{X^{\ast}}$ is compact, $\tau$ is a homeomorphism onto its image, so $B_{X^{\ast}}$ is weak* metrizable.

  Suppose $B_{X^{\ast}}$ is weak* metrizable. Let $\set{U_n : n\geq 1}$ be a family of weak*-open neighborhoods of $0$ whose intersection is $0$. Then, for each $n$, there is some finite $F_n\subseteq X$ such that
  \begin{align*}
    \set{\varphi\in B_{X^{\ast}} : \left\vert \iprod{x}{\varphi} \right\vert < 1\text{ for all }x\in F_n} \subseteq U_n.
  \end{align*}
  Let $F = \bigcup_{n=1}^{\infty}F_n$. Then, $F$ is countable. The pre-annihilator of $F^{\perp}$ is the closed linear span of $F$, which is separable. Yet, if $\varphi\in F^{\perp}$, then for each $n\geq 1$ and each $x\in F_n$, we have
  \begin{align*}
    \left\vert \iprod{x}{\frac{\varphi}{\norm{\varphi}}} \right\vert &= 0\\
                                                                     &< 1,
  \end{align*}
  so $\frac{\varphi}{\norm{\varphi}}\in U_n$ for all $n\geq 1$, whence $\varphi = 0$. Therefore, $F^{\perp} = \set{0}$, meaning its pre-annihilator is $X$. Thus, $X$ is separable.
\end{proof}
We may be curious about an analogous result for the weak topology. That is, if $X^{\ast}$ is separable, is the weak topology on $B_X$ metrizable? This follows immediately from the above theorem when we consider the embedding of $X$ into $X^{\ast\ast}$. However, this isn't very useful unless we're considering a separable reflexive space.

However, in general, if $X$ is separable, it is not necessarily the case that $B_X$ equipped with the weak topology is metrizable.
\begin{theorem}
  If a sequence in $\ell^1$ converges weakly, then the sequence converges in norm.
\end{theorem}
\begin{proof}
  Since $\ell^1$ is separable, we have that $B_{\ell^{\infty}}$ is weak* metrizable. Since $B_{\ell^{\infty}}$ is weak* compact (i.e., it is a compact metrizable space), we may use the Baire Category Theorem.

  Let $\left( f_n \right)_n\rightarrow 0$ weakly in $\ell^1$, and let $\ve > 0$. For each $m\geq 0$, let
  \begin{align*}
    F_m &= \set{\phi\in B_{\ell^{\infty}} : \left\vert \iprod{f_n}{\phi} \right\vert \leq \ve/3\text{ for all }n\geq m}.
  \end{align*}
  Then, $F_m$ is weak*-closed in $B_{\ell^{\infty}}$. Since $\left( f_n \right)_n\rightarrow 0$ weakly, it follows that the union of all such $F_m$ is equal to $B_{\ell^{\infty}}$.

  By the Baire Category Theorem, there is some $F_m$ with nonempty interior. A metric on $B_{\ell^{\infty}}$ inducing the weak* topology is given by
  \begin{align*}
    d\left( \phi,\psi \right) &= \sum_{j=1}^{\infty} 2^{-j} \left\vert \phi(j)-\psi(j) \right\vert.
  \end{align*}
  Since $F_m$ has nonempty weak* interior, there is $\phi\in F_m$ and $\delta > 0$ such that $U\left( \phi,\delta \right)\subseteq F_m$, where we recall that
  \begin{align*}
    U\left( \phi,\delta \right) &= \set{\psi\in B_{\ell^{\infty}} : d\left( \phi,\psi \right) < \delta}.
  \end{align*}
  Let $J$ be such that $2^{-(J-1)} < \delta$. Let $n\geq m$, and let $\psi\in \ell^{\infty}$ be defined by $\psi(j) = \phi(j)$ for each $1\leq j \leq J$ and $\psi(j) = \sgn\left( f_n(j) \right)$ for each $j > j$. Then, $\psi(j)f_n(j) = \left\vert f_n(j) \right\vert$ for each $j > J$. We have that $\psi\in B_{\ell^{\infty}}$.

  Furthermore, we have
  \begin{align*}
    d\left( \phi,\psi \right) &= \sum_{j=J+1}^{\infty}2^{-j} \left\vert \phi(j)-\psi(j) \right\vert\\
                              &\leq 2^{-\left( J-1 \right)}\\
                              &< \delta.
  \end{align*}
  Thus, $\psi\in F_m$, so $\left\vert \iprod{f_n}{\psi} \right\vert \leq \ve/3$ for all $n\geq m$. In particular, we have
  \begin{align*}
    \left\vert \sum_{j=1}^{J}\phi(j)f_n(j) + \sum_{j=J+1}^{\infty}\left\vert f_n(j) \right\vert \right\vert \leq \frac{\ve}{3}
  \end{align*}
  for all $n\geq m$.

  Yet, since $\left( f_n \right)_{n}\rightarrow 0$ in $\ell^1$, we have some $m_1\geq m$ such that for all $n\geq m_1$, we have
  \begin{align*}
    \sum_{j=1}^{J} \left\vert f_n(j) \right\vert &< \ve/3.
  \end{align*}
  Therefore, we have
  \begin{align*}
    \norm{f_n} &= \sum_{j=1}^{\infty} \left\vert f_n(j) \right\vert\\
               &< \frac{\ve}{3} + \left\vert \sum_{j=J+1}^{\infty}\left\vert f_n(j) \right\vert + \sum_{j=1}^{J}\phi(j)f_n(j) \right\vert + \left\vert \sum_{j=1}^{J}\phi(j)f_n(j) \right\vert\\
               &< \frac{2\ve}{3} + \sum_{j=1}^{J} \left\vert f_n(j) \right\vert\\
               &< \ve
  \end{align*}
  whenever $n\geq m_1$. This gives norm convergence.
\end{proof}
In particular, if it were the case that $B_{\ell^1}$ with the weak topology was metrizable, then we would have that the weak and norm topologies on $\ell^1$ agreed. Yet, this is not the case.
\subsection{Extremal Structure}%
\begin{definition}
  Let $X$ be a vector space, and let $K\subseteq X$ be a convex set. We call a subset $F\subseteq K$ a \textit{face} if $F$ is a convex subset satisfying the following criterion: if $0 < t < 1$, and $x,y\in K$ with $\left( 1-t \right)x + ty\in F$ then $x,y\in F$.

  A singleton set $\set{z}\in K$ that is a face is called an extreme point. We write $\operatorname{ext}\left( K \right)$ for the set of extreme points of $K$.
\end{definition}
In particular, $z\in K$ is an extreme point if and only if, for any $0 < t < 1$, if there are $x,y\in K$ with $\left( 1-t \right)x + ty = z$, then $x = y = z$. In fact, we can take $t = 1/2$ --- that is, we only need to concern ourselves with the midpoint of any such line segment. To see this, we assume without loss of generality that $t < 1/2$. Then, the vector $w = \left( 1-2t \right)x + 2ty$ lies in $C$ since $C$ is convex, and satisfies $\frac{1}{2}\left( x+w \right) = z$. In particular, this means that $x = z = w$, so that $y = z$.

There are convex subsets not admitting extreme points.
\begin{example}
  Let $X = c_0$. We will show that the unit ball $B_{c_0}$ has no extreme points.

  Let $z\in \operatorname{ext}\left( B_{c_0} \right)$. Notice that for any $0 < \norm{z} < 1$, we have
  \begin{align*}
    z &= \left( 1- \norm{z} \right)0 + \norm{z} \frac{z}{\norm{z}},
  \end{align*}
  meaning in particular that $z$ cannot be an extreme point if $\norm{z} < 1$. In particular, this means that $\norm{z} = 1$.

  Since $z\in c_0$, there is $n_0\in \N$ with $\left\vert z\left( n_0 \right) \right\vert < 1$. Write $z\left( n_0 \right) = \left( 1-t \right)\lambda + t\mu$ for some $0 < t < 1$ and $\left\vert \lambda \right\vert = \left\vert \mu \right\vert = 1$. Consider the sequences defined by
  \begin{align*}
    x(n) &= \begin{cases}
      z(n) & n\neq n_0\\
      \lambda & n = n_0
    \end{cases}\\
      ...
      \end{cases}
  \end{align*}
  Then, $\norm{x},\norm{y}\leq 1$, but $\left( 1-t \right)x + ty = z$ with $x\neq z$ and $y\neq z$, meaning that $z$ cannot be extreme.
\end{example}
\begin{proposition}
  Let $\Omega$ be a locally compact Hausdorff space.
  \begin{enumerate}[(i)]
    \item If $\Omega$ is not compact, then $\operatorname{ext}\left( B_{C_0(\Omega)} \right) = \emptyset$.
    \item If $\Omega$ is compact, then 
      \begin{align*}
        \operatorname{ext}\left( B_{C(\Omega)} \right) &= \set{u\in C(\Omega) : \left\vert u(x) \right\vert = 1\text{ for all }x\in \Omega}.
      \end{align*}
  \end{enumerate}
\end{proposition}
\begin{proof}
  Let $f\in \operatorname{ext}\left( B_{C_0(\Omega)} \right)$. Then, $\norm{f} = 1$. Suppose there is some $x_0$ with $ \left\vert f\left( x_0 \right) \right\vert < 1 $. Choose $\delta$ such that $\left\vert f\left( x_0 \right) \right\vert < \delta < 1$. By continuity, there is some open neighborhood $U$ of $x_0$ with $\left\vert f \right\vert < \delta$ on $U$. By Urysohn's Lemma, there is a continuous function $g\colon \Omega\rightarrow [0,1]$ with $\supp\left( g \right)\subseteq U$ and $g\left( x_0 \right) = 1$.

  Then, we may write
  \begin{align*}
    f &= \frac{1}{2}\left( \left( f + \left( 1-\delta \right)g \right) + \left( f-\left( 1-\delta \right)g \right) \right).
  \end{align*}
  First, we observe that on $U^{c}$, we have that $\left\vert f \pm \left( 1-\delta \right)g \right\vert = \left\vert f \right\vert \leq 1$, while on $U$ we have
  \begin{align*}
    \left\vert f \pm \left( 1-\delta \right)g \right\vert &\leq \left\vert f \right\vert + \left( 1-\delta \right)\\
                                                          &\leq 1.
  \end{align*}
  In particular, since $f\pm \left( 1-\delta \right)g \neq f$, this contradicts the fact that $f$ is extreme, so in particular, we must have $\left\vert f(x) \right\vert = 1$ for all $x\in \Omega$, yielding that $\Omega$ is compact. This gives (i).

  Having seen that (i) holds, we have that $\operatorname{ext}\left( B_{C(\Omega)} \right)$ is contained in the set of all $u\in C(\Omega)$ with $\left\vert u \right\vert = 1$ on $\Omega$. Our next task is to show that any such $u$ is extreme.

  Write $u = \frac{1}{2}\left( f+g \right)$ with $f,g\in B_{C(\Omega)}$. Then, $u(x) = \frac{1}{2}\left( f(x) + g(x) \right)$ with $\left\vert f(x) \right\vert,\left\vert g(x) \right\vert\leq 1$ and $\left\vert u(x) \right\vert = 1$. Since the extreme points of the closed unit ball of $\C$ is $S^1$, it follows that $f(x) = g(x) = u(x)$ for all $x\in \Omega$.
\end{proof}
