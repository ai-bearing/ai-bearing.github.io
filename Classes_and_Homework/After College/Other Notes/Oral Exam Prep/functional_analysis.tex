...
\begin{proof}
  ...
  \end{align*}
  Viewing $B_{\infty}(K)\subseteq L^2\left( \mu \right)$, we have that $U\phi = \phi(N)e_0$ defines an isometry from $B_{\infty}(K)$ onto $\set{\phi(N)e_0 : \phi\in B_{\infty}(K)}$.

  Since $e_0$ is star-cyclic, it follows that the range of $U$ is dense in $H$, so $U$ extends to an isomorphism $U\colon L^2\left( \mu \right)\rightarrow H$.
  ...
  \begin{align*}
    UN_{\mu}U^{-1} \left( \phi(N)e_0 \right) &= UN_{\mu}\left( \phi \right)\\
                                             ...
                                             &= N\phi(N)e_0,
  \end{align*}
  so that $UN_{\mu}U^{-1} = N$ on the dense subset $\set{\phi(N)e_0 : \phi\in B_{\infty}(K)}$ of $L^2\left( \mu \right)$. Therefore $UN_{\mu}U^{-1} = N$, and $V = U^{-1}$ is our desired selection.
\end{proof}
