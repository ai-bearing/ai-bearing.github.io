...
\end{proof}
We will take the following result as a black box.
\begin{theorem}[Malcev's Theorem]
  Let $G$ be a finitely generated torsion-free nilpotent group. Then, there exists $n\geq 1$ such that $G$ embeds into $\operatorname{UT}\left( n,\Z \right)$, the group of upper unitriangular matrices over $\Z$.
\end{theorem}
\begin{theorem}
  Let $G$ be a finitely generated nilpotent group. There exists a finite descending normal series
  \begin{align*}
    G\geq N_1\geq\cdots\geq N_s\geq N_{s+1}= \set{e}
  \end{align*}
  such that $N_k/N_{k+1}$ is cyclic and $\left[ N_k,G \right]\subseteq N_{k+1}$ for all $k=1,\dots,s$.
\end{theorem}
\begin{proof}
  Let $\left( \gamma_i \right)_{i\in I}$ be the lower central series of $G$. Since $\gamma_i/\gamma_{i+1}$ is a finitely generated abelian group, there are normal subgroups
  \begin{align*}
    \gamma_i\geq N_{i,1}\geq N_{i,2}\geq\cdots\geq N_{i,t_i} = \gamma_{i+1}
  \end{align*}
  with $N_{i,j}/N_{i,j+1}$ cyclic for each $j$. Additionally, we have 
  \begin{align*}
    \left[ N_{i,j},G \right]&\subseteq \left[ \gamma_i,\gamma_1 \right]\\
                            &= \gamma_{i+1}\\
                            &= N_{i,t_i}\\
                            &\subseteq N_{i,j+1}.
  \end{align*}
  Renaming the normal subgroups yields the desired result.
\end{proof}
The next focus will be on torsion in finitely generated nilpotent groups.
\begin{lemma}
  Every subgroup of a finitely generated nilpotent group is finitely generated.
\end{lemma}
\begin{proof}
  Let $G$ be a finitely generated nilpotent group with
  \begin{align*}
    G = N_1\geq N_2\geq\cdots\geq N_s\geq N_{s+1} = \set{e}
  \end{align*}
  a normal series with $N_i/N_{i+1}$ cyclic for each $i=1,\dots,s$. We will show that each subgroup $H\leq G$ is generated by at most $s$ elements.

  If $s = 1$, then $G$ is cyclic and so the statement necessarily holds. Assume the statement holds for all groups with normal series with cyclic quotients of length $\left( s-1 \right)$. Since $G/N_2$ is cyclic, so is the subgroup $HN_2/N_2$. By the second isomorphism theorem, this is isomorphic to $H/\left( H\cap N_2 \right)$. Since $H\cap N_2$ is a subgroup of $N_2$, the induction hypothesis gives that $H$ is generated by at most $\left( s-1 \right)$ elements, whence $H$ is generated by at most $s$ elements.
\end{proof}
...
\begin{proposition}
  The subgroup $F = \left\langle a,b \right\rangle\subseteq \SL_2\left( \Z \right) $ is finite index.

  Specifically, considering subgroups
  \begin{align*}
    G &= \set{ \begin{pmatrix}2m+1 & 2r \\ 2s & 2n+1\end{pmatrix} : m,n,r,s\in \Z, \det \begin{pmatrix}2m+1 & 2r \\ 2s & 2n+1\end{pmatrix} = 1 }\\
    F' &= \set{ \begin{pmatrix}4m+1 & 2r \\ 2s & 4n+1\end{pmatrix} : m,n,r,s\in \Z, \det \begin{pmatrix}4m+1 & 2r \\ 2s & 4n+1\end{pmatrix} = 1 },
  \end{align*}
  we have $\left[ G:F' \right] = 2$, $\left[ \SL_2\left( \Z \right) : G \right] = 6$, and $F = F'$.
\end{proposition}
\begin{proof}
  First, we show that $\left[ G:F' \right] = 2$. Taking modulo $4$, we have either both $m$ and $n$ are odd or both $m$ and $n$ are even. If $m$ and $n$ are even, then an element $x\in G$ is contained in $F'$, and if $m$ and $n$ are odd, then $-x\in F'$. Therefore, $\set{gF' : g\in G} = F'\sqcup \left( -1 \right)F'$. Therefore, $\left[ G:F' \right] = 2$.

  Next, we show that $\left[ \SL_2\left( \Z \right):G \right] = 6$. Observe that $G$ is the kernel of the reduction modulo $2$ homomorphism $\SL_2\left( \Z \right)\rightarrow \SL_2\left( \Z/2\Z \right)$. Since $\left\vert \SL_2\left( \Z/2\Z \right) = 6 \right\vert$, it follows that $\left[ \SL_2\left( \Z \right):G \right] = 6$.

  Finally, we observe that $F\subseteq F'$ since any word in the matrices $a$ and $b$ is of the given form. Writing
  \begin{align*}
    \begin{pmatrix}x_{11} & x_{12} \\ x_{21} & x_{22}\end{pmatrix} &\in F',
  \end{align*}
  we use induction on $\min\left( \left\vert x_{11} \right\vert,\left\vert x_{12} \right\vert \right)$ to show that $x\in F$.
  \begin{itemize}
    \item If $x-{12} = 0$, then we must have $x_{11} = x_{22}\in \left\langle b \right\rangle\subseteq F$.
    \item If $\left\vert x_{12} \right\vert\geq \left\vert x_{11} \right\vert$, then we will find $k\in \Z$ and $0\leq R \leq 2\left\vert x_{11} \right\vert-1$, with
      \begin{align*}
        x_{12} + \left\vert x_{11} \right\vert &= -2kx_{11} + R.
      \end{align*}
      Then, the matrix
      \begin{align*}
        x' &= x a^{k}\\
           &= \begin{pmatrix}x_{11} & x_{12} + 2kx_{11} \\ x_{21} & x_{22} + 2kx_{21}\end{pmatrix}.
      \end{align*}
      We have that $x'\in xF$, and
      \begin{align*}
        \left\vert x_{12}' \right\vert &= \left\vert x_{12} + 2kx_{11} \right\vert\\
                                       &= \left\vert -\left\vert x_{11} \right\vert-2kx_{11} + R + 2kx_{11} \right\vert\\
                                       &\leq \left\vert x_{11} \right\vert\\
                                       &= \left\vert x_{11}' \right\vert.
      \end{align*}
      By parity, we have that $\left\vert x_{12}' \right\vert \neq \left\vert x_{11}' \right\vert$, so that $\left\vert x_{12}' \right\vert < \left\vert x_{11}' \right\vert$, meaning that $\min\left( \left\vert x_{11}' \right\vert,\left\vert x_{12}' \right\vert \right) < \min\left( \left\vert x_{11} \right\vert,\left\vert x_{12} \right\vert \right)$, whence $x'\in F$, meaning $x\in F$.
    \item Similarly, if $\left\vert x_{12} \right\vert < \left\vert x_{11} \right\vert$, then we will find $k\in \Z$ with $0\leq R \leq \left\vert 2x_{12} \right\vert-1$ such that $x_{11} + \left\vert x_{12} \right\vert = -2kx_{12} + R$.

      Considering the matrix $x' = xb^{k}\in xF$ yields the same result with $\min\left( \left\vert x_{11}' \right\vert,\left\vert x_{12}' \right\vert \right) < \min\left( \left\vert x_{11} \right\vert,\left\vert x_{12} \right\vert \right)$ by a similar process as the previous item.
  \end{itemize}
\end{proof}
We note that it is possible to extend this method to show that, if we let
\begin{align*}
  G_1 &= \left\langle \begin{pmatrix}0 & -1 \\ 1 & 1\end{pmatrix} \right\rangle\\
  G_2 &= \left\langle \begin{pmatrix}0 & -1 \\ 1 & 0\end{pmatrix} \right\rangle\\
  A &= G_1\cap G_2,
\end{align*}
then $\SL_2\left( \Z \right)\cong G_1\ast_{A}G_2$. Since $G\cong \Z/6\Z$, $G_2\cong \Z/4\Z$, and $A\cong \Z/2\Z$, it follows that $\SL_2\left( \Z \right)\cong \Z/6\Z\ast_{\Z/2\Z}\Z/4\Z$.

The next application is about regular graphs of large girth. Recall that if $X$ is a graph, then the girth $g(X)$ is the length of the shortest cycle in $X$. A tree (and/or a forest) has infinite girth.

Finding graphs of large girth, especially those with minimal vertex count, is a very important goal, and the solution can be found through the group-theoretic constructions we are interested in. We will focus on the specific case of $4$-regular graphs.
\begin{theorem}
  Let $N \geq 5$. Then, there exists a graph $X_N$ with the following properties:
  \begin{itemize}
    \item $X_N$ is $4$-regular;
    \item $X_N$ has at most $N^3$ vertices;
    \item the graph $X_N$ satisfies
      \begin{align*}
        g\left( X_N \right) &\geq 2 \frac{\ln\left( N/2 \right)}{\ln\left( 1 + \sqrt{2} \right)} - 1.
      \end{align*}
  \end{itemize}
\end{theorem}
\begin{proof}
  ...
  \begin{align*}
    ...
    b &= \begin{pmatrix}1 & 0 \\ 2 & 1\end{pmatrix}
  \end{align*}
  be the matrices in $\SL_2\left( \Z \right)$ that generate a free subgroup of rank $2$. Then, we consider the homomorphism $\varphi_N\colon \SL_2\left( \Z \right)\rightarrow \SL_2\left( \Z/N\Z \right)$ given by reduction modulo $N$. Let $a_N = \varphi_N\left( a \right)$ and $b_N = \varphi_N\left( b \right)$.

  Let $G_N = \left\langle a_N,b_N \right\rangle$ and $X_N = \operatorname{Cay}\left( G,\set{a_N,b_N} \right)$. Then, $X_N$ is $4$-regular by construction, and $X_N$ has at most $\left\vert \SL_2\left( \Z/N\Z \right) \right\vert\leq N^3$ vertices.

  Thus, our task is to show the lower girth bound. Let $v,w$ be reduced words in the free group $F(a,b)$ with $w\neq v$ and $\varphi_N\left( w \right) = \varphi_N\left( v \right)$. We may assume that the lengths $m$ and $n$ of $w$ and $v$ respectively satisfy
  \begin{align*}
    g\left( X_N \right) &= m + n\\
    \max\left( m,n \right) &\leq \frac{m + n + 1}{2}.
  \end{align*}
  Let $c = w-v\in \Mat_{2}\left( \Z \right)$. Since $w\neq v$, it follows that $c\neq 0$, but $c_N = \varphi_N\left( c \right)$, given by reduction modulo $N$, we have that $c_N = 0$. Therefore, we have $c'\in \Mat_2\left( \Z \right)$ such that $c = Nc'$. Therefore, we have estimates of the operator norm given by
  \begin{align*}
    \norm{w} + \norm{v} &\geq \norm{c}\\
                        &= N\norm{c'}\\
                        &\geq N\max\left( \left\vert c'_{11} \right\vert,\left\vert c'_{12} \right\vert,\left\vert c'_{21} \right\vert,\left\vert c'_{22} \right\vert \right)\\
                        &\geq N.
  \end{align*}
  Yet, we already have that $\norm{a} \leq 1 + \sqrt{2}\eqcolon d$, and $\norm{b}\leq d$ as well. Therefore, we have
  \begin{align*}
    N&\leq \norm{c}\\
     &\leq d^m + d^n\\
     &\leq 2d^{\frac{m + n + 1}{2}}\\
     &= 2d^{\frac{g\left( X_N \right) + 1}{2}},
  \end{align*}
...
\begin{corollary}[Topological Formulation of Milnor--Schwarz]
  Let $G$ be a group acting by isometries on a nonempty proper geodesic metric space $\left( X,d \right)$. Suppose this action is proper and cocompact. Then, $G$ is finitely generated, and for all $x\in X$, the map $g\mapsto g.x$ is a quasi-isometry. That is, $G$ and $X$ are quasi-isometric.
\end{corollary}
...
\begin{corollary}\hfill
  ...
  \end{enumerate}
\end{corollary}
\subsection{Dynamic Criterion for Quasi-Isometry}%
Now, we discuss the dynamic criterion for quasi-isometry.
\begin{definition}
...
\begin{proof}
  ...
  \end{align*}
  First, we show that the map $f$ has quasi-dense image. Let $h\in H$. Since $G.K = X$, we will find some $g\in G$ with $x.h\in g.K$. Since the actions of $G$ and $H$ commute, we have $g^{-1}.x\in K.h^{-1}$. Yet, by the definition of $f$, $g^{-1}.x\in K.f(g)^{-1}$, so $K.h^{-1}\cap K.f(g)^{-1}\neq \emptyset$, meaning $h^{-1}f(g)\in F_H$. In particular, this means $d_T\left( h,f(g) \right)\leq D_TF_H$, where we recall that $F_H$ is the set of all $h\in H$ such that $K.h\cap K\neq \emptyset$.

  Now, we will show that $f$ is a quasi-isometric embedding. Let
  \begin{align*}
    F_H(S) &= \bigcup_{s\in S\cup S^{-1}}F_H(s)\\
    F_H(T) &= \bigcup_{t\in T\cup T^{-1}} F_G(t).
  \end{align*}
  These sets are assumed finite since each of the $F_G(t)$ and $F_H(s)$ are finite. Let $g,g'\in G$.
  \begin{itemize}
    \item Let $n = d_S\left( g,g' \right)$. We start by showing that $K.f(g)^{-1}f(g')\cap K.F_H(S)^{n}$ is nonempty. First, we see that $g^{-1}.x.f(g')\in K.f(g)^{-1}f(g')$ by the construction of $f$. We may write $g^{-1}g' = s_1\cdots s_n$ for some $s_1,\dots,s_n\in S\cup S^{-1}$, so that
      \begin{align*}
        g^{-1}.x.f(g') &= g^{-1}g'.\left( g'^{-1}.x.f(g') \right)\\
                       &\in g^{-1}g'.K.f(g')^{-1}f(g')\\
                       &= g^{-1}g'.K\\
                       &= s_1\cdots s_{n-1}s_n.K\\
                       &\subseteq s_1\cdots s_{n-1}.K.F_H(S)\\
                       &\vdots\\
                       &\subseteq K.F_H(S)^{n}.
      \end{align*}
      Thus, we have $K.f(g)^{-1}f(g')\cap K.F_H(S)^{n}\neq \emptyset$. Notice that all of these calculations use the fact that the actions between $G$ and $H$ commute. By the definition of $F_H$, we have $f(g)^{-1}f(g')\in F_HF_H(S)^{n}$. By the triangle inequality, we have
      \begin{align*}
        d_G\left( f(g),f(g') \right) &= d_G\left( e,f(g)^{-1}f(g') \right)\\
                                     &\leq D_T\left( F_HF_H(S)^{n} \right)\\
                                     &\leq nD_TF_H(S) + D_TF_H\\
                                     &= d_S\left( g,g' \right)D_TF_H(S) + D_TF_H.
      \end{align*}
      This gives us our desired upper bound.
    \item Using a similar line of reasoning, if $m = D_T\left( f(g),f(g') \right)$, then we may find that $g^{-1}.x.f(g')\in F_G(T)^{m}.K\cap g^{-1}g'.K$. Therefore, this intersection is nonempty, so we may conclude that
      \begin{align*}
        d_S\left( g,g' \right) &\leq D_SF_G(T)d_T\left( f(g),f(g') \right) + D_SF_G.
      \end{align*}
      This gives our desired lower bound.
  \end{itemize}
  Finally, suppose $G$ and $H$ are quasi-isometric. We will determine a locally convex space that allows for a topological coupling.

  Let $S\subseteq G$ and $T\subseteq H$ be finite generating sets for $G$ and $H$. We will show that there is a finite group $F$ and a constant $C > 0$ such that the set $X$ of all functions $f\colon G\rightarrow H\times F$ satisfying
  \begin{itemize}
    \item $f$ has $C$-dense image in $H\times F$
    \item for all $g,g'\in G$,
      \begin{align*}
        \frac{1}{C}d_S\left( g,g' \right) \leq d_{T\times F} \left( f(g),f(g') \right) \leq Cd_S\left( g,g' \right)
      \end{align*}
  \end{itemize}
  is nonempty. Let $f\colon G\rightarrow H$ be a quasi-isometry. Since $f$ is a quasi-isometry, there is some $c > 0$ such that $f$ has $c$-dense image in $H$ --- that is, for all $g,g'\in G$,
  \begin{align*}
    \frac{1}{c}d_S\left( g,g' \right)-c \leq d_T\left( f(g),f(g') \right) \leq cd_S\left( g,g' \right) + c.
  \end{align*}
  Notice that if $g,g'\in G$ satisfy $f(g) = f(g')$, then $d_S\left( g,g' \right)\leq c^2$. If $F$ is a finite group that has more elements than $B_S\left( e_G,c^2 \right)$, then we may construct an injective quasi-isometry $ \overline{f}\colon G\rightarrow H\times F $. Let $ \overline{c} > 0 $ be such that $ \overline{f} $ is a $\left(  \overline{c}, \overline{c} \right)$-quasi-isometric embedding with $ \overline{c} $-dense image.

  Since $ \overline{f} $ is injective, it follows that $ \overline{f} $ satisfies a $\max\left( 2 \overline{c}^2 + \overline{c} \right)$-bilipschitz estimate. Therefore, if we let $C = \max\left( 2 \overline{c}, \overline{c}^2 + \overline{c} \right)$, the group $F$ and the constant $C$ satisfy the desired properties, whence $X$ is nonempty.

  Let $G$ act on $X$ by $\left( g.f \right)\left( x \right) = f\left( g^{-1}.x \right)$ and $H$ on $X$ by $\left( f.h \right)(x) = f(x)\left( h,e \right)$. These actions necessarily commute with each other by the way they're defined.

  Equipping $X$ with the topology of pointwise convergence, the Arzelá--Ascoli theorem yields that $X$ is locally compact with respect to this topology. This follows from the fact that the elements of $X$ have a bilipschitz condition attached to them so that the family is equicontinuous. Using the Arzelá--Ascoli theorem on the actions of $G$ and $H$ on $X$ yields that the actions are proper and cocompact.
\end{proof}
The most important use case for the dynamic criterion is the consideration of discrete subgroups of locally compact groups.
\begin{definition}
  Let $G$ be a locally compact topological group. A \textit{uniform lattice} in $G$ is a discrete subgroup $\Gamma$ of $G$ such that the left translation action (or right translation action) of $\Gamma$ on $G$ is cocompact.
\end{definition}
\begin{corollary}
  Let $G$ be a locally compact topological group. Then, all finitely generated uniform lattices in $G$ are quasi-isometric.
\end{corollary}
\begin{proof}
  Let $\Gamma$ and $\Lambda$ be finitely generated uniform lattices in $G$. Then, the left and right translation actions of $\Gamma$ and $\Lambda$ respectively are continuous (by definition) and commute (since multiplication is associative).

  These actions are cocompact and proper, so $G$ is a topological coupling for $\Gamma$ and $\Lambda$, whence $\Gamma$ and $\Lambda$ are quasi-isometric.
\end{proof}
The best use of the dynamic criterion is that one can apply it to other settings. The measure-theoretic notion is known as measure equivalence, and it plays an essential role in measured group theory.

Now, we will discuss some other quasi-isometry invariants.
\subsection{Asymptotic Cones}%
Let $\left( X,d \right)$ be a metric space, and let $x_0\in X$ be the base point. Let $\omega$ be a free ultrafilter on $\N$.
\begin{definition}
  We say a sequence $\left( x_n \right)_n = x$ is \textit{moderate} if there exists some constant $A$ (dependent on $x$) such that
  \begin{align*}
    d\left( x_n,x_0 \right) &\leq An
  \end{align*}
  for all $n\in \N$. Let $\mathcal{M}\left( X,d \right)$ be the set of all moderate sequences.
\end{definition}
That the sequences in $ \mathcal{M}\left( X,d \right) $ are invariant under basepoint follows from the fact that, whenever $n\geq 1$, we have $An + C \leq \left( A+C \right)n$.

If $x,y\in X^{\N}$ are moderate sequences, we observe that
\begin{align*}
  d\left( x_n,y_n \right) &\leq d\left( x_n,x_0 \right) + d\left( x_0,y_n \right)\\
                          &\leq \left( A_x + A_y \right)n.
\end{align*}
This gives that the sequence $\left( n^{-1}d\left( x_n,y_n \right) \right)_n$ is bounded, so that the ultrafilter limit
\begin{align*}
  \lim_{\omega} \frac{d\left( x_n,y_n \right)}{n}
\end{align*}
always exists. Write $x\sim_{\omega}y$ if $\lim_{\omega}n^{-1}d\left( x_n,y_n \right) = 0$. This is an equivalence relation on $ \mathcal{M}\left( X,d \right) $. Write
\begin{align*}
  K_{\omega}\left( X,d \right) &= \mathcal{M}\left( X,d \right)/\sim_{\omega}
\end{align*}
for the quotient space. We have a well-defined distance metric given by
\begin{align*}
  d_{\omega}\left( \left[ x \right]_{\omega},\left[ y \right]_{\omega} \right) &\coloneq \lim_{\omega} \frac{d\left( x_n,y_n \right)}{n}.
\end{align*}
We call the metric space $\left( K_{\omega}\left( X,d \right),d_{\omega} \right)$ the \textit{asymptotic cone} of the metric space $\left( X,d \right)$ relative to the free ultrafilter $\omega$.

The main asymptotic cone we will be concerned with is the asymptotic cone of a metric group (such as a finitely generated group with the word metric).
\begin{example}
  Let $\left( G,d \right)$ be a group generated by a finite symmetric subset $S\subseteq G$ with the metric $d = d_S$. Let $e_G$ be the basepoint for this metric space.

  Equip $\mathcal{M}\left( G,d \right)$ with the group structure given by pointwise multiplication --- that is, $gh = \left( g_nh_n \right)_n$. Left invariance gives that
  \begin{align*}
    d\left( g_nh_n,e_G \right) &\leq d\left( g_n,e_G \right) + d\left( h_n,e_G \right)\\
                             &\leq \left( A_g + A_h \right)n.
  \end{align*}
  The constant sequence $e = \left( e_G \right)_{n}$ is the neutral element, and inverses are given by $g^{-1} = \left( g_n^{-1} \right)_{n}$. If $\omega$ is a free ultrafilter, and
  \begin{align*}
    N_{\omega} &= \set{g\in \mathcal{M}\left( G,d \right) : \lim_{\omega} n^{-1}d\left( g_n,e_G \right) = 0},
  \end{align*}
  then $N_{\omega}$ is a subgroup of $ \mathcal{M}\left( G,d \right) $, and is such that $g\sim_{\omega}g'$ if and only if $gN_{\omega} = g'n_{\omega}$ for all $g,g'\in \mathcal{M}\left( G,d \right)$. There is then a bijection between the asymptotic cone and the coset space of $N_{\omega}$ in $\mathcal{M}\left( G,d \right)$. That is,
  \begin{align*}
    K_{\omega}\left( G,d \right) &\cong \frac{\mathcal{M}\left( G,d \right)}{N_{\omega}}.
  \end{align*}
  Furthermore, if $d$ is bi-invariant, then $N_{\omega}$ is normal and $K_{\omega}\left( G,d \right)$ equipped with the metric $d_{\omega}$ is a metric group.
\end{example}
Asymptotic cones are the structure that allow us to convert the information in quasi-isometries into bilipschitz equivalences. In a sense, asymptotic cones allow us to ``zoom out'' of a metric space in a well-defined fashion so that we may capture the coarse structure of a quasi-isometry.
\begin{proposition}
  Suppose $\left( X,d \right)$ and $\left( Y,d \right)$ are metric spaces, and $\omega$ is a free ultrafilter on $ \N $. Every quasi-isometry $\Phi\colon X\rightarrow Y$ induces a bilipschitz homeomorphism $\Phi_{\omega}\colon K_{\omega}\left( X,d \right) \rightarrow K_{\omega}\left( Y,\rho \right)$.
\end{proposition}
\begin{proof}
  Let $C \geq 1$ be sufficiently large such that $\Phi$ satisfies
  \begin{align*}
    \frac{1}{C}d\left( x_1,x_2 \right) - C \leq \rho\left( \Phi\left(x_1\right),\Phi\left( x_2 \right) \right) \leq Cd\left( x_1,x_2 \right) + C
  \end{align*}
  and is $C$-quasi-dense. That is, for all $x\in X$, there is $y\in Y$ such that
  \begin{align*}
    \rho\left( \Phi(x),y \right) &\leq C.
  \end{align*}
  Let $x_0$ be a basepoint for $X$, and let $y_0 = \Phi\left( x_0 \right)$ be the corresponding basepoint for $Y$. Then, for any $x\in \mathcal{M}\left( X,d \right)$, we have
  \begin{align*}
    \rho\left( \Phi\left( x_n \right),y_0 \right) &\leq Cd\left( x_n,x_0 \right) + C\\
                                                  &\leq C\left( A_x + 1 \right)n.
  \end{align*}
  Therefore, $\left( \Phi\left( x_n \right) \right)_{n\in \N}$ is a moderate sequence in $Y$. We have
  \begin{align*}
    \lim_{\omega} \frac{\rho\left( \Phi\left( x_n \right),\Phi\left( x_n' \right) \right)}{n} &\leq \lim_{\omega} \frac{Cd\left( x_n,x_n' \right) + C}{n}\\
                                                                                              &= Cd_{\omega}\left( \left[ x \right]_{\omega},\left[ x' \right]_{\omega} \right).
  \end{align*}
  Therefore, the map $\Phi_{\omega}$ given by $\left[ x \right]_{\omega}\mapsto \left[ \left( \Phi\left( x_n \right) \right)_n \right]_{\omega}$ is well-defined. Furthermore, we have
  \begin{align*}
    \rho_{\omega}\left( \Phi_{\omega}\left( \left[ x \right]_{\omega} \right),\Phi_{\omega}\left( \left[ x' \right]_{\omega} \right) \right) &\leq Cd_{\omega}\left( \left[ x \right]_{\omega},\left[ x' \right]_{\omega} \right).
  \end{align*}
  Similarly, we obtain the lower bound given by
  \begin{align*}
    \lim_{\omega} \frac{d\left( \Phi_{\omega}\left( \left[ x \right]_{\omega} \right),\Phi_{\omega}\left( \left[ x' \right]_{\omega} \right) \right)}{n} &\geq \lim_{\omega} \frac{\frac{1}{C}d\left( x_n,x_n' \right) + C}{n}\\
                                                                                           &= \frac{1}{C}d_{\omega}\left( \left[ x \right]_{\omega},\left[ x' \right]_{\omega} \right).
  \end{align*}
  Finally, we only need to show that $\Phi_{\omega}$ is surjective. Let $y\in \mathcal{M}\left( X,d \right)$. Then, for each $n\in \N$, there is $x_n\in X$ such that $\rho\left( \Phi\left( x_n \right),y_n \right)\leq C$. We see that the sequence $x = \left( x_n \right)_n$ satisfies
  \begin{align*}
    d\left( x_n,x_0 \right) &\leq C\rho\left( \Phi\left( x_n, \right),\Phi\left( x_0 \right) \right)\\
                            &\leq C\left( \rho\left( \Phi\left( x_n \right),y_n \right) + \rho\left( y_n,y_0 \right) \right)\\
                            &\leq C\left( C + A_y \right)n,
  \end{align*}
  so it is moderate. Therefore, we have
  \begin{align*}
    \rho_{\omega}\left( \Phi_{\omega}\left( \left[ x \right]_{\omega} \right),\left[ y \right]_{\omega} \right) &= \lim_{\omega} \frac{\rho\left( \Phi\left( x_n \right),y_n \right)}{n}\\
              &\leq \lim_{\omega} \frac{C}{n}\\
              &= 0.
  \end{align*}
\end{proof}
\subsection{Growth Rates and Types}%
\begin{definition}
  Let $G$ be a finitely generated group, and $S\subseteq G$ a finite symmetric generating set for $G$. The growth function $\beta_{G,S}\colon \N\rightarrow \N$ is given by
  \begin{align*}
    r\mapsto \left\vert B_{G,S} \left( e,r \right) \right\vert,
  \end{align*}
  where
  \begin{align*}
    B_{G,S}\left( e,r \right) &= \set{g\in G : d_S\left( g,e \right) \leq r}.
  \end{align*}
\end{definition}
Note that growth functions are dependent on the choice of generating sets. For example, $\beta_{\Z,\set{1}}$ is given by $r\mapsto 2r+1$, while $\beta_{\Z,\set{2,3}}$ is given by
\begin{align*}
...
\end{align*}
\begin{lemma}
  Let $G$ be a finitely generated group, $S\subseteq G$ a finite generating set.
  \begin{enumerate}[(i)]
    \item For all $r,r'\in \N$, we have
      \begin{align*}
        \beta_{G,S} \left( r + r' \right) &\leq \beta_{G,S}\left( r \right)\beta_{G,S}\left( r' \right).
      \end{align*}
    \item Let $G$ be infinite. Then, $\beta_{G,S}$ is strictly increasing, and satisfies $\beta_{G,S}(r) \geq r$ for all $r\in \N$.
    \item For all $r\in \N$, we have $\beta_{G,S}\left( r \right) \leq \beta_{F(S),S}\left( r \right)$.
  \end{enumerate}
\end{lemma}
\begin{proof}\hfill
  \begin{enumerate}[(i)]
    \item We start by noting that if $g\in B_{G,S}\left( r+r' \right)$, then we may write $g = s_1\cdots s_r\cdots s_{r + r'}$, meaning that $g\in B_{G,S}(r)B_{G,S}\left(r'\right)$, while for any elements $g\in B_{G,S}(r)$ and $h\in B_{G,S}(r')$, we have $d\left( e,gh \right)\leq d\left( e,g^{-1} \right) + d\left( e,h \right)$, whence $gh\in B_{G,S}\left( r+r' \right)$. Therefore, we have
      \begin{align*}
        \left\vert B_{G,S}\left( r + r' \right) \right\vert &= \left\vert B_{G,S}\left( r \right)B_{G,S}\left( r' \right) \right\vert\\
                                                            &\leq \left\vert B_{G,S}\left( r \right) \right\vert \left\vert B_{G,S}\left( r' \right) \right\vert.
      \end{align*}
    \item Since $G$ is infinite, for any $r\in \N$ there exists some $g\in G$ such that $d\left( e,g \right) = r$, and furthermore, if $g = s_1\cdots s_r$, then there is some $s\in S$ such that $d\left( e,gs \right) > d\left( e,g \right)$, so that $B_{G,S}\left( e,r \right)\subsetneq B_{G,S}\left( e,r+1 \right)$.
    \item The homomorphism $\varphi\colon F(S)\rightarrow G$ given by $\varphi|_{S} = \operatorname{id}_S$ is contractive with respect to the word metrics given by $S$ on their respective groups (since $G$ possibly admits relations), and $\varphi$ is surjective. Therefore,
      \begin{align*}
        \beta_{G,S}\left( r \right) &= \left\vert B_{G,S}\left( e,r \right) \right\vert\\
                                    &= \left\vert \varphi\left( B_{F(S),S}\left( e,r \right) \right) \right\vert\\
                                    &\leq \left\vert B_{F(S),S}\left( e,r \right) \right\vert\\
                                    &= \beta_{F(S),S}(r).
      \end{align*}
  \end{enumerate}
\end{proof}
Now that we have established some properties of the growth rate, we can abstract away from the growth rate to focus, in a sense, on the ``largest'' term.
\begin{definition}\hfill
  \begin{itemize}
    \item A generalized growth function is an increasing function from $\R_{\geq 0}$ to $\R_{\geq 0}$.
    \item If $f,g$ are generalized growth functions, we say that $g$ \textit{quasi-dominates} $f$ if there exist $b,c\in \R_{> 0}$ such that
      \begin{align*}
        f(r) &\leq cg\left( cr + b \right) + b.
      \end{align*}
      We write $f\preceq g$.
    \item If $f\preceq g$ and $g\preceq f$, then we say $f$ and $g$ are \textit{quasi-equivalent}, and write $f\sim g$.
  \end{itemize}
\end{definition}
\begin{example}\hfill
  \begin{itemize}
    \item If $a \geq 0$, then $x\mapsto x^{a}$ is a generalized growth function, and $\left( x\mapsto x^{a} \right)\preceq \left( x\mapsto x^{a'} \right)$ if and only if $a\leq a'$. Notice that $\left( x\mapsto x^{a} \right) \sim \left( x\mapsto x^{a'} \right)$ if and only if $a = a'$.
    \item If $a > 1$, then $x\mapsto a^{x}$ is a generalized growth function. Notice that $x\mapsto a^{x}$ is quasi-equivalent to $x\mapsto a'^{x}$ for all $a,a' > 1$, since we may substitute $\log_{a}(a')x$ in the first equation to yield the second equation, and vice versa.
  \end{itemize}
\end{example}
Notice that if $G$ is a finitely generated group with finite generating set $S$, then the function $r\mapsto \beta_{G,S}\left( \lceil r \rceil \right)$ is a submultiplicative generalized growth function. In particular, $\beta_{G,S}\left( r \right)\preceq \beta_{H,T}\left( r \right)$ if and only if there exists $b,c\in \N$ such that
\begin{align*}
  \beta_{G,S}\left( r \right) &\leq c\beta_{H,T}\left( cr + b \right) + b
\end{align*}
for all $r\in \N$.

Our primary focus will be on quasi-isometry invariance of the growth rate.
\begin{proposition}
  Let $G$ and $H$ be finitely generated groups with generating sets $S\subseteq G$ and $T\subseteq H$. If there exists a quasi-isometric embedding $\left( G,d_S \right)\rightarrow \left( H,d_T \right)$, then $\beta_{G,S}\preceq \beta_{H,T}$.

  In particular, if $G$ and $H$ are quasi-isometric, then the growth functions $\beta_{G,S}$ and $\beta_{H,T}$ are quasi-equivalent.
\end{proposition}
\begin{proof}
  Let $f\colon G\rightarrow H$ be a quasi-isometric embedding. There exists $c > 0$ such that
  \begin{align*}
    \frac{1}{c}d_S\left( g,g' \right) - c \leq d_T\left( f(g),f(g') \right) \leq cd_S\left( g,g' \right) + c.
  \end{align*}
  Write $e' = f(e)$, and let $r\in \N$. If $g\in B_{G,S}\left( e,r \right)$, then we have
  \begin{align*}
    d_T\left( f(g),e' \right) &\leq cd_S\left( g,e \right) + c\\
                              &\leq cr + c,
  \end{align*}
  so that $f\left( B_{G,S}\left( e,r \right) \right) \subseteq B_{H,T}\left( e',cr+c \right)$. Furthermore, if $f(g) = f(g')$, then
  \begin{align*}
    d_S\left( g,g' \right) &\leq c\left( d_T\left( f(g),f(g') \right) + c \right)\\
                           &= c^2.
  \end{align*}
  Since the metrics are invariant under left translation, we get
  \begin{align*}
    \beta_{G,S}\left( r \right) &\leq \left\vert B_{G,S}\left( e,c^2 \right) \right\vert\left\vert \beta_{H,T}\left( e',cr+c \right) \right\vert\\
                                &= \left\vert B_{G,S}\left( e,c^2 \right) \right\vert\left\vert B_{H,T}\left( e,cr+c \right) \right\vert\\
                                &= \beta_{G,S}\left( c^2 \right)\beta_{H,T}\left( cr+c \right).
  \end{align*}
  Since $c$ is constant, it follows that $\beta_{G,S}\preceq \beta_{H,T}$.
\end{proof}
This allows us a well-defined notion of growth type for a finitely generated group (recall that bilipschitz equivalences are quasi-isometries, so the growth type is invariant with respect to the generating set).
\begin{definition}
  Let $G$ be a finitely generated group.
  \begin{enumerate}[(i)]
    \item The growth type of $G$ is the quasi-equivalence class of a growth function of $G$ with respect to any finite generating set.
    \item The group $G$ is said to be of \textit{exponential growth} if it is of type $x\mapsto e^{x}$.
    \item The group is said to be of \textit{polynomial growth} if it is of type $\left( x\mapsto x^{a} \right)$ for some $a \geq 0$.
    \item The group is said to be of intermediate growth if it is neither exponential nor polynomial growth.
  \end{enumerate}
\end{definition}
This trichotomy emerges from the fact that any group has its growth dominated by the growth of the free group on its generating set, which is exponential.
...
\begin{definition}
  A group $G$ is called sofic if it admits an embedding into a metric ultraproduct of certain symmetric groups. That is, there is some set $A$, finite subsets $\left( F_{\alpha} \right)_{\alpha\in A}$ indexed by $A$, and some nonprincipal ultrafilter $ \mathcal{U} $ on $A$ such that
  \begin{align*}
    G\hookrightarrow \prod_{\mathcal{U}} \left( \sym\left( F_{\alpha} \right),d_{F_{\alpha}} \right)
  \end{align*}
  is a well-defined embedding.
\end{definition}
\subsection{Approximate Homomorphisms}%
The metric ultraproduct definition is equivalent to another definition that shows soficity is a special type of approximation property. This will enable us to establish the soficity of a large class of groups.
\begin{definition}
  Let $G$ be a group, $K\subseteq G$ a finite subset, and $\ve > 0$. A map $\varphi\colon G\rightarrow \sym(F)$ for some finite set $F$ is called a $\left( K,\ve \right)$-almost-homomorphism if it satisfies the following conditions:
  \begin{enumerate}[(i)]
    \item for all $k_1,k_2\in K$ with $k_1k_2\in K$, we have $d_F\left( \varphi\left( k_1k_2 \right),\varphi\left( k_1 \right)\varphi\left( k_2 \right) \right) \leq \ve$;
    \item for all $k_1,k_2\in K$ with $k_1\neq k_2$, we have $d_F\left( \varphi\left( k_1 \right),\varphi\left( k_2 \right) \right) \geq 1-\ve$.
  \end{enumerate}
  Here, $d_F$ denotes the normalized Hamming distance on $\sym(F)$.
\end{definition}
\begin{definition}
  A group $G$ is called sofic if, for every finite subset $K\subseteq G$ and every $\ve > 0$, there is a nonempty finite set $F$ and a $\left( K,\ve \right)$-almost-homomorphism $\varphi\colon G\rightarrow \sym(F)$.
\end{definition}
We will now discuss the equivalence of this definition and the metric ultraproduct definition.
\begin{theorem}
  The metric ultraproduct definition of a sofic group and the almost-homomorphism definition of a sofic group coincide, where we note that we only need to assume that for all $k_1,k_2\in K$ with $k_1\neq k_2$, we have $d_F\left( \varphi\left( k_1 \right),\varphi\left( k_2 \right) \right)\geq 1/4$ in the second item.
\end{theorem}
\begin{proof}
  We will always assume that $G$ is countable. In one direction, we let $ \mathcal{K} $ be the set of finite subsets of $G$ ordered by inclusion.

  Then, we will consider the family of $\left( K,1/n \right)$-homomorphisms $\varphi_{K,1/n}\colon \left( K,1/n \right)\rightarrow \sym\left( F_{K,1/n} \right)$ for every finite set $K\subseteq G$ and strictly positive natural number $n$. Choose a nonprincipal ultrafilter $ \mathcal{U} $ on the set of all such pairs such that $ \mathcal{U} $ contains all pairs of the form $\left( K',1/m \right)$ with $K'\supseteq K$ and $m\geq n$. The map 
  \begin{align*}
    \varphi\colon G&\rightarrow \prod_{\mathcal{U}}\left( \sym\left( F_{K,1/n} \right),d_{F_{K,1/n}} \right)\\
    g &\mapsto \left( \varphi_{K,1/n}(g) \right)_{\mathcal{U}}
  \end{align*}
  is a well-defined group homomorphism satisfying $d\left( \varphi(g),\varphi(h) \right)\geq 1/4$ for any $g\neq h$.

  In the reverse direction, the given embedding $\varphi\colon G\rightarrow \prod_{\mathcal{U}}\sym\left( F_{\alpha} \right)$ yields a family of $\left( K,\ve \right)$-almost-homomorphisms by choosing satisfactory coordinate projections for the given choices of $K$ and $\ve$. For the final condition, we note that choices of coordinate projections may not necessarily yield $d\left( \varphi(g),\varphi(h) \right)\geq 1/4$ whenever $g,h\in K$ with $g\neq h$. To resolve this issue, we amplify as follows.

  Fix a $\left( K,\ve' \right)$-almost-homomorphism $\theta\colon F\rightarrow S_n$ with $\theta(g)\neq \theta(h)$ and $\ve'$ sufficiently small. Let $\delta = d\left( \theta(g),\theta(h) \right)$. We amplify by embedding $K$ diagonally into the group of permutations of the square $[n] \times [n]$ diagonally; that is,
  \begin{align*}
    \theta^{(2)}\left( g \right) &= \left( \theta(g),\theta(g) \right)
  \end{align*}
  acting by double permutations. Then, the normalized counting measure of the set of pairs $\left( i,j \right)$ for which $ \left( \theta(g),\theta(g) \right) $ and $\left( \theta(h),\theta(h) \right)$ are different is strictly larger than $\delta$. Applying this amplification method as many times as necessary yields our desired separation of $1/4$.

  Since we may choose $\delta$ to be arbitrarily close to $d\left( g,h \right)$ as we desire, the amplification level $k$ can be chosen before we choose the $\left( K,\ve' \right)$-almost-homomorphism by letting $\ve'$ be small enough such that $\theta^{(k)}$ remains a $\ve$-almost-homomorphism for prescribed $\ve > 0$.
\end{proof}
We begin establishing the soficity of large classes of groups.
\begin{theorem}
  Locally sofic groups --- that is, groups whose finitely generated subgroups are sofic --- is sofic.
\end{theorem}
\begin{proof}
  Let $K\subseteq G$ be a finite subset, and let $\ve > 0$. Let $H = \left\langle K \right\rangle$ be the finitely generated subgroup of $G$ generated by $K$. Then, since $H$ is sofic, there is a $\left( K,\ve \right)$-almost-homomorphism $\psi\colon H\rightarrow \sym(F)$ for some nonempty finite set $F$. We may extend $\psi$ arbitrarily to a map $\varphi\colon G\to \sym(F)$ setting $\varphi(g) = \id_F$ for all $g\in G\setminus H$, and note that $\varphi$ remains a $\left( K,\ve \right)$-almost-homomorphism.
\end{proof}
In particular, every locally finite group is sofic. This is also established by the following more general proposition.
\begin{proposition}
  Amenable groups are sofic.
\end{proposition}
\begin{proof}
  Let $G$ be amenable, $K\subseteq G$ finite, and $\ve > 0$. Let $S = \left( \set{e_G}\cup K\cup K^{-1} \right)^2$. Since $G$ is amenable, the Følner criterion gives a nonempty finite subset $F\subseteq G$ such that
  \begin{align*}
    \left\vert F\setminus sF \right\vert &\leq \frac{\ve}{\left\vert S \right\vert}\left\vert F \right\vert
  \end{align*}
  for all $s\in S$. Let
  \begin{align*}
    E &= \bigcap_{s\in S} sF.
  \end{align*}
  Note that $E\subseteq F$ since $e_G\in S$. Since $S = S^{-1}$, it follows that $sE\subseteq F$ for all $s\in S$, and we have
  \begin{align*}
    \left\vert F\setminus E \right\vert &= \left\vert \bigcup_{s\in F}\left( F\setminus sF \right) \right\vert\\
                                        &\leq \sum_{s\in S} \left\vert F\setminus sF \right\vert\\
                                        &\leq \ve \left\vert F \right\vert.
  \end{align*}
  Therefore, we have $\left\vert E \right\vert\geq \left( 1-\ve \right)\left\vert F \right\vert$.

  For every $g\in G$, we have $\left\vert F \right\vert = \left\vert gF \right\vert$, so $\left\vert F\setminus gF \right\vert = \left\vert gF\setminus F \right\vert$, and we may find a bijective map $\alpha_g\colon gF\setminus F\rightarrow F\setminus gF$. Let $\varphi\colon G\to \sym(F)$ be defined by
  \begin{align*}
    \varphi(g)(f) &= \begin{cases}
      gf & gf\in F\\
      \alpha_g(gf) & \text{else}
    \end{cases}
  \end{align*}
  ...
  If $k_1,k_2\in K$ and $f\in E$, then we have $k_2,k_1k_2\in S$, meaning $k_2f,k_1k_2f\in F$, meaning $\varphi\left( k_1k_2 \right)(f) = k_1k_2f$, and 
  \begin{align*}
    \left( \varphi\left( k_1 \right)\varphi\left( k_2 \right) \right)(f) &= \varphi\left( k_1 \right)\left( \varphi\left( k_2 \right)(f) \right)\\
                                                                         &= \varphi\left( k_1 \right)\left( k_2f \right) = k_1k_2f.
  \end{align*}
  Therefore, $\varphi\left( k_1k_2 \right)$ and $\varphi\left( k_! \right)\varphi\left( k_2 \right)$ coincide on $E$, so by the definition of $E$ we have
  \begin{align*}
    d_F\left( \varphi\left( k_1k_2 \right),\varphi\left( k_1 \right)\varphi\left( k_2 \right) \right) &\leq \frac{\left\vert F\setminus E \right\vert}{\left\vert F \right\vert}\\
                                          &\leq \ve.
  \end{align*}
  Meanwhile, if $k_1,k_2\in K$ with $k_1\neq k_2$, and $f\in E$, then we have $k_1f,k_2f\in F$, so that $\varphi\left( k_1 \right)(f) = k_1f\neq k_2f = \varphi\left( k_2 \right)f$. Thus, we have
  \begin{align*}
    d_F\left( \varphi\left( k_1 \right),\varphi\left( k_2 \right) \right) &\geq \frac{\left\vert E \right\vert}{\left\vert F \right\vert}\\
                                                                          &\geq 1-\ve.
  \end{align*}
  This gives that $G$ is a sofic group.
\end{proof}
\subsection{Cayley Graphs}%
Given a group $G$, we have established previously that we only need to verify that all its finitely generated subgroups are sofic in order to establish that $G$ is sofic.

This will allow us to consider a classification of soficity of a finitely generated subgroup by its relationship to the Cayley graph.

Let $G$ be a finitely generated group with finite symmetric generating set $S$. For any $r > 0$, denote by $B_S(r)$ the ball of radius $r$ centered at the identity element. We may label the edges of the graph by $S$ --- i.e., labeling by $s$ the edge $\set{g,gs}$. Let $X = \left( V,E \right)$ be an $S$-labeled graph. Given $q\in V$ and $r\in R$, let $B\left( q,r \right)$ be the ball of radius $r$ centered at the vertex $q$ with the induced $S$-labeled graph structure.

Let $Q(r)$ be the set of all $q\in V$ such that there exists an isomorphism of $S$-labeled graphs $\psi_{q,r}\colon B_S(r)\rightarrow B\left( q,r \right)$ satisfying $\psi_{q,r}\left(e_G\right) = q$.

If such a map $\psi_{q,r}$ exists, then it is unique. There is a sequence of inclusions
\begin{align*}
  V \supseteq Q(0)\supseteq Q(1)\supseteq\cdots\supseteq Q(r)\supseteq Q(r+1)\supseteq\cdots,
\end{align*}
where we note that $\operatorname{Cay}\left( G,S \right)$ is edge-symmetric with respect to $s\mapsto s^{-1}$, meaning that we will require $B\left( q,r \right)$ to be edge-symmetric as well (since $\psi_{q,r}$ is an isomorphism of $S$-labeled graphs).
\begin{lemma}
  Let $X = \left( V,E \right)$ be an $S$-labeled graph, $r_0,i\in \N$. Suppose $q_0\in Q\left( \left( i+1 \right)r_0 \right)$. Then, $B\left( q_0,r_0 \right)\subseteq Q\left( ir_0 \right)$.
\end{lemma}
\begin{proof}
  Let $q'\in B\left( q_0,r_0 \right)$. We will show that $q'\in Q\left( ir_0 \right)$. From the triangle inequality, we have that $B\left( q',ir_0 \right)\subsetneq B\left( q_0,\left( i+1 \right)r_0 \right)$. Since $\psi_{q_0,\left( i+1 \right)r_0}$ is isometric, we have that $g = \psi_{q_0,\left( i+1 \right)r_0}^{-1}\left( q' \right)$ is contained in $B_S\left( e,r_0 \right)$. In particular, this means that $gh\in B\left( \left( i+1 \right)r_0 \right)$. Therefore, the map $\psi_{q',ir_0}$ defined by taking $\psi_{q',ir_0}(h) = \psi_{q_0,\left( i+1 \right)r_0}(gh)$ for all $h\in B_S\left( e,ir_0 \right)$ yields an $S$-labeled graph isomorphism with $\psi_{q',ir_0}(e) = q'$. Therefore, $q'\in Q\left( ir_0 \right)$, so that $B\left( q_0,r_0 \right)\subseteq Q\left( ir_0 \right)$.
\end{proof}
\begin{lemma}
  Let $G = \left( V,E \right)$ be an $S$-labeled graph, and let $r_0\in \N$. Let $q_1,q_2\in Q\left( 2r_0 \right)$ be such that $q_1\neq q_2$ and $g\in B_S\left(e, r_0 \right)$. Then,
  \begin{align*}
    \psi_{q_1,2r_0}(g) &\neq \psi_{q_2,2r_0}(g).
  \end{align*}
\end{lemma}
\begin{proof}
  If $g = e_G$, then since $q_1\neq q_2$, we obtain the desired result.

  Suppose $g \neq e_G$, and assume toward contradiction that $\psi_{q_1,2r_0}(g) = \psi_{q_2,2r_0}(g)$. Call this value $q_0$. Since $g\in B_S\left(e, r_0 \right)$, there is $1\leq r' \leq r_0$ and elements $s_1,\dots,s_{r'}$ in $S$ such that $g = s_1s_2\cdots s_{r'}$. Let
  \begin{align*}
    \pi &= \left[ e_G,s_1,s_1s_2,\dots,s_1\cdots s_{r'-1},g \right]
  \end{align*}
  be a path traversing from $e_G$ to $g$. Notice that $\pi$ is contained in $B_S\left(e, r_0 \right)$. The path $\pi$ is mapped by $\psi_{q_1,2r_0}$ and $\psi_{q_2,2r_0}$ to paths $\pi_1$ and $\pi_2$ with initial vertices $\pi_1^{-1} = q_1$ and $\pi_2^{-} = q_2$ with common terminal vertex $\pi_{1}^{+} = \pi_2^{+} = q_0$. Since $\psi_{q_1,2r_0}$ and $\psi_{q_2,2r_0}$ are isometric, $\pi_1$ and $\pi_2$ are contained in $B\left( q_0,r_0 \right)$.

  Moreover, since $\psi_{q_1,2r_0}$ and $\psi_{q_2,2r_0}$ are label-preserving, it follows that $\pi_1$ and $\pi_2$ have the same labels; namely, their edges are labeled by $s_1\cdots s_{r'}$.

  The inverse images of $\pi_1$ and $\pi_2$ under the $S$-labeled graph isomorphism $\psi_{q_0,r_0}\colon B_S\left(e, r_0 \right)\rightarrow B\left( q_0,r_0 \right)$ are equal to $\pi$. Yet, this means that $\pi_1 = \pi_2$, so $\pi_1^{-} = \pi_2^{-}$, or that $q_1 = q_2$, which is a contradiction. Thus, $\psi_{q_1,2r_0} \neq \psi_{q_2,2r_0}$.
\end{proof}
\begin{lemma}
  Let $G = \left( V,E \right)$ be an $S$-labeled graph, and let $r_0\in \N$. Let $h,k\in B_S\left( e,r_0 \right)$, and let $q_0\in Q\left( 2r_0 \right)$. Then, $\psi_{q_0,2r_0}\in Q\left( r_0 \right)$.
\end{lemma}
