...
\begin{proof}
  ...
  \begin{align*}
    \norm{\varphi\left( a \right)}^2 &= \norm{\varphi\left( a^{\ast}a \right)}\\
                                     ...
                                     &= \norm{a}^2.
  \end{align*}
  If $\varphi$ is injective, then since the Gelfand transform commutes with $\ast$-homomorphisms, we have a corresponding map $\pi\colon C(\hat{A})\rightarrow C(\hat{B})$. 
\end{proof}
