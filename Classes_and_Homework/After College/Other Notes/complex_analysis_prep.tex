...
\end{solution}
\begin{problem}[Problem 4]
  Let $\left( f_n \right)_n$ be a sequence of holomorphic functions, and suppose $\left( f_n \right)_n\rightarrow f$ uniformly on compact subsets of $\D$ to a function $f$. Suppose $f$ has no zeros on $\set{z\in \C | \left\vert z \right\vert = \frac{1}{2}}$. Show that there exists an $N\in \N$ such that if $n,m\geq N$, then $f_m$ and $f_n$ have the same number of zeros in $\set{z\in \C | \left\vert z \right\vert < \frac{1}{2}}$, counted with multiplicity.
\end{problem}
\begin{solution}
