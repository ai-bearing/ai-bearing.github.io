...
\end{align*}
The characteristic function of a random variable is the characteristic function of its distribution, i.e. the map
\begin{align*}
...
To see (ii), letting $\left( a_{ij} \right)_{i,j}$ be a positive element of $\M_n\left( L^{\infty}\left( X \right) \right)$, we have, for any $g\in L^{\infty}\left( Y \right)^{\oplus n}$,
\begin{align*}
  \iprod{ \mathbb{E}_Y\left( \left( a_{ij} \right)_{i,j} \right)g }{g} &= \sum_{i,j} \iprod{ \mathbb{E}_Y\left( a_{ij} \right)g_j }{g_i}\\
                                                                       &= \sum_{i,j} \iprod{ \mathbb{E}_Y\left( a_{ij} \right) }{g_i \overline{g_j}}\\
                                                                       &= \sum_{i,j} \iprod{a_{ij}}{g_i \overline{g_j}}\\
                                                                       &= \sum_{i,j} \iprod{a_{ij}g_j}{g_i}\\
                                                                       &\geq 0.
\end{align*}
Approximating yields that positivity holds whenever $g_i\in L^2\left( Y \right)$.

We may then view $L^{\infty}\left( X \right)$ as an $L^{\infty}\left( Y \right)$-module by the multiplication $\left( f,g \right)\mapsto fg$, and define an $L^{\infty}\left( Y \right)$-valued inner product $ \iprod{f}{g}_Y = \mathbb{E}_Y\left( f \overline{g} \right) $. The Hilbert $L^{\infty}\left( Y \right)$-module $L^{2}\left( X|Y \right)$ is the completion of $L^{\infty}\left( X \right)$ with respect to this $L^{\infty}\left( Y \right)$-module. We will let $\norm{f} \coloneq \norm{ \iprod{f}{f}_Y }_{L^{\infty}\left( X \right)}^{1/2}$. Note that, with context, we may drop the label $L^{\infty}\left( X \right)$ on the norm $\norm{\cdot}$, but we will keep the label $ \norm{\cdot}_{L^2\left( X \right)} $.

There are natural inclusions $L^{\infty}\left( X \right) \subseteq L^{2}\left( X|Y \right) \subseteq L^{2}\left( X \right)$. The second inclusion emerges from the fact that
\begin{align*}
  \norm{f}_{L^2(X)}^2 &= \iprod{f}{f}\\
                      &= \int_{X}^{} f \overline{f}\:d\mu\\
                      &= \int_{X}^{} \mathbb{E}_Y\left( f \overline{f} \right)\:d\mu\\
                      &\leq \norm{ \mathbb{E}_Y\left( f \overline{f} \right) }\\
                      &= \norm{ \iprod{f}{f}_Y }.
\end{align*}
This shows that the identity map $L^{\infty}\left( X \right)$ with the Hilbert $L^{\infty}(Y)$-module norm into $L^{2}\left( X \right)$ is contractive, so it extends to $L^2\left( X|Y \right)$. This extension is injective since
\begin{align*}
  \norm{f}_{L^2(X)}^2 &= \iprod{f}{f}\\
                      &= \int_{X}^{} \iprod{f}{f}_Y\:d\mu
\end{align*}
for all $f\in L^{\infty}\left( X \right)$, so it holds for all $f\in L^2\left( X|Y \right)$. This yields $\norm{f}_{L^2(X)} = 0$ implies $ \iprod{f}{f}_Y = 0 $, so $f = 0$.

For context in the following definition, we will let $A\subseteq_{\ve}B$ denote $\ve$-inclusion, meaning that every element of $A$ is a distance less than or equal to $\ve$ from some element of $B$.
\begin{definition}\hfill
  \begin{enumerate}[(i)]
    \item A subset of $L^2\left( X|Y \right)$ is called a finitely generated module zonotope if it is of the form $\sum_{h\in \Omega}h B_{L^{\infty}(Y)}$, where $\Omega\subseteq L^2\left( X|Y \right)$ is finite and $B_{L^{\infty}(Y)}$ denotes the closed unit ball of $L^{\infty}(Y)$.
    \item A set $K\subseteq L^2\left(X|Y\right)$ is called conditionally precompact if, for every $\ve > 0$, there is a set $D\subseteq Y$ with $\nu(D) > 1-\ve$ and a finitely generated module zonotope $Z\subseteq L^{\infty}\left( X|Y \right)$ such that $\chi_D K \subseteq_{\ve}D$.
  \end{enumerate}
\end{definition}
\nocite{kerr_and_li}
\printbibliography
