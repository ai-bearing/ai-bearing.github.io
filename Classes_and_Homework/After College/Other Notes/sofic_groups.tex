...
\end{proposition}
\begin{proof}
  Let $\Gamma$ be a amenable countable discrete group, $F$ a finite subset of $\Gamma$, and $\ve$ a positive real number. Find a $\left( F\cup F^{-1},\ve \right)$-invariant subset $K$ of $\Gamma$, which follows from the Følner condition. If $\gamma\in F$, then define
  \begin{align*}
