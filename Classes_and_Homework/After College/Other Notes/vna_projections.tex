...
\end{proof}
It turns out that the tensor product is part of a general theory of representations of von Neumann algebras.
\begin{lemma}
  Let $M\subseteq B(H)$ be a von Neumann algebra, where $H$ is a separable infinite-dimensional Hilbert space. For any $\omega\in M_{\ast}$, where $M_{\ast}$ denotes the normal ($\sigma$-WOT continuous) linear functionals on $M$, there exist vectors $\xi,\eta\in H\otimes H$ such that
  \begin{align*}
    \omega(x) &= \iprod{\left( x\otimes 1 \right)\xi}{\eta}.
  \end{align*}
\end{lemma}
\begin{proof}
  Let $\set{\ve_n}_{n\in \N}$ be an orthogonal basis for $H$. By the structure of $\sigma$-WOT continuous linear functionals, we have
  \begin{align*}
    \omega &= \sum_{n=1}^{\infty} \theta_{\xi_n,\eta_n},
  \end{align*}
  where
  \begin{align*}
    \theta_{x,y}(z) &= \iprod{z}{y}x
  \end{align*}
  is a rank-one projection, and the sequences $\left( \xi_n \right)_n$ and $\left( \eta_n \right)_n$ are square-summable in norm. If we set
  \begin{align*}
    \xi &= \sum_{n=1}^{\infty} \xi_n\otimes \ve_n\\
    \eta &= \sum_{n=1}^{\infty}\eta_n\otimes \ve_n,
  \end{align*}
  then $\xi,\eta\in H\otimes H$, and satisfies
  \begin{align*}
    \omega(x) &= \sum_{n=1}^{\infty} \iprod{x\xi_n}{\eta_n}\\
              &= \sum_{n,m=1}^{\infty} \iprod{\left( x\otimes 1 \right)\left( \xi_n\otimes\ve_n \right)}{\left( \eta_m\otimes \ve_m \right)}\\
              &= \iprod{\left( x\otimes 1 \right)\xi}{\eta}.
  \end{align*}
\end{proof}
Analogously, any normal homomorphism of a von Neumann algebra can be decomposed into components including a tensor product.
\begin{theorem}
  Let $M_1\subseteq B\left( H_1 \right)$ and $M_2\subseteq B\left( H_2 \right)$ be von Neumann algebras. If $\pi$ is a normal homomorphism from $M_1$ to $M_2$, then there is a Hilbert space $H$, a projection $e'\in M_1'\otimes B\left( H \right)$, and a unitary $U\colon e'\left( H_1\otimes H \right)\rightarrow H_2$ such that
  \begin{align*}
    \pi(x) &= U e'\left( x\otimes 1_{H} \right)e'U^{\ast}
  \end{align*}
  for all $x\in M_1$. That is, every normal homomorphism between $M_1$ and $M_2$ consists of an amplification, a compression, and a spatial isomorphism.
\end{theorem}
\begin{proof}
  Let $\set{\xi_{2,i}}_{i\in I}$ be a maximal family of nonzero vectors in $H_2$ such that the range subspaces $\left[ M_2\xi_{2,i} \right] = H_{2,i}$ are mutually orthogonal. From maximality, we have that
  \begin{align*}
    H_2 &= \bigoplus_{i\in I}H_{2,i}.
  \end{align*}
  Now, let
  \begin{align*}
    H &= \bigoplus_{i\in I}H_i,
  \end{align*}
  where $H_i\cong H_0$ for some fixed separable infinite-dimensional Hilbert space $H_0$. Let $\pi_1$ be the amplification of $M_1$ to $M_1\otimes 1_{H}$. If $\set{\ve_n}_{n\in \N}$ is an orthogonal basis of $H_0$, then by the above lemma, we may find a vector $\xi_{1,i}\in H_1\otimes H_i$ such that
  \begin{align*}
    \iprod{\pi(x)\xi_{2,i}}{\xi_{2,i}} &= \iprod{\pi_1(x)\xi_{1,i}}{\xi_{1,i}}
  \end{align*}
  for each $x\in M_1$.

  Observe that for each $i$, the ranges $ \left[ \left( M_1\otimes \C 1_{H} \right)\xi_{1,i} \right] $ are mutually orthogonal in $H_1\otimes H$. 

  Define $e'$ to be the range projection onto $\bigoplus_{i\in I}\left[ \left( M_1\otimes \C 1_{H} \right)\xi_{1,i} \right]$. Then, we have that $e'\in \left( M_1\otimes \C1_{H} \right)' = M_1'\otimes B\left( H \right)$. Then, for each $i\in I$, we may define
  \begin{align*}
    U_i\pi_1\left( x \right)\xi_{1,i} &= \pi(x)\xi_{2,i}.
  \end{align*}
  By the definition of $\xi_{1,i}$, it follows that $U_i$ may be extended to an isometry of $\left[ \pi_1\left( M_1 \right)\xi_{1,i} \right]$ onto $H_{2,i}$, which we also denote by $U_i$. Define
  \begin{align*}
    U &= \bigoplus_{i\in I}U_i.
  \end{align*}
  If we let $\pi_2(x) = e'xe'$ for any $x\in M_1\otimes \C 1_{H}$, and $\pi_3(x) = UxU^{\ast}$ for any $x\in e'\left( M_1\otimes \C 1_{H} \right)e'$, then we have the desired decomposition of $\pi$.
\end{proof}
...
\begin{proof}
  ...
  Let $\set{\hat{e}_i,\hat{f}_i}_{i\in I}$ be a maximal family with $\set{\hat{e}_i}_{i\in I}$ and $\set{\hat{f}_{i}}_{i\in I}$ are pairwise orthogonal. Suppose $\hat{e}_i\leq \bar{e}$, $\hat{f}_i\leq \bar{f}$, and $\hat{e}_i\sim \hat{f}_i$ with $\left( 1+\ve \right)\varphi\left( \hat{e}_i \right)\leq \mu\varphi\left( \hat{f}_i \right)$. Now, the projections $p = \bar{e}-\sum_{i}\hat{e}_i$ and $q = \bar{f}-\sum_{i}\hat{f}_i$ are equivalent. If $p_1,p_2\leq p$ with $p_1\sim p_2$, then there exists $r\leq q$ such that $r\sim p_1$, giving
  \begin{align*}
    \varphi\left( p_1 \right) &\leq \mu\varphi\left( r \right)\\
                              &\leq \left( 1+\ve \right)\varphi\left( p_2 \right).
  \end{align*}
  Since $\varphi$ is bounded, and every positive operator can be approximated uniformly by the span of its spectral projections, it follows that
  \begin{align*}
    \varphi\left( ux^{\ast}xu \right) &\leq \left( 1+\ve \right)\varphi\left( x^{\ast}x \right)
  \end{align*}
  for each $x\in pMp$ and unitary $u\in U\left( pMp \right)$. If we take the polar decomposition $x^{\ast} = v\left\vert x^{\ast} \right\vert$, then since $pMp$ is finite, $v$ can be extended to a unitary $u^{\ast}$, so $\varphi\left( xx^{\ast} \right)\leq \left( 1+\ve \right)\varphi\left( x^{\ast}x \right)$.
\end{proof}
\begin{lemma}
  Let $M$ be a finite von Neumann algebra, and let $\ve > 0$. Then, there is a normal center-valued state $\varphi$ such that for all $x\in M$, we have
  \begin{align*}
    \varphi\left( xx^{\ast} \right) &\leq \left( 1+\ve \right)\varphi\left( x^{\ast}x \right).
  \end{align*}
\end{lemma}
\begin{proof}
  We only need to show the existence of such a state on $Mz$ for some $z\in P(Z(M))$, as then we may use maximality to resolve this argument.

  From above, we know that there exists a normal center-valued state $\psi$, and there exists some nonzero $p\in P(M)$ such that $\psi\left( xx^{\ast} \right)\leq \left( 1+\ve \right)\psi\left( x^{\ast}x \right)$ for all $x\in pMp$. We may assume that $p$ is monic.

  Let $\set{p_1,\dots,p_n}$ be a finite family of pairwise orthogonal projections with $p_i\sim p$ and $z_0 = \sum_{i=1}^{n}p_i$, with $z_0\in Z(M)$. Let $v_i\in M$ be such that $v_i^{\ast}v_i = p_i$ and $v_iv_i^{\ast} = p$. For any $x\in Mz_0$, define
  \begin{align*}
    \varphi_0(x) &= \sum_{i=1}^{n}\psi\left( v_ixv_i^{\ast} \right).
  \end{align*}
  Then, we have
  \begin{align*}
    0 &\leq \varphi_0\left( xx^{\ast} \right)\\
      &= \varphi_0\left( xz_0x^{\ast} \right)\\
      &= \sum_{j=1}^{n} \varphi_0\left( xp_jx^{\ast} \right)\\
      &= \sum_{i,j=1}^{n} \psi\left( v_ixv_j^{\ast}v_jx^{\ast}v_i^{\ast} \right)\\
      &\leq \left( 1+\ve \right) \sum_{i,j=1}^{n}\psi\left( v_jx^{\ast}v_i^{\ast}v_ixv_j^{\ast} \right)\\
      &= \left( 1+\ve \right)\sum_{i=1}^{n} \varphi_i\left( x^{\ast}p_ix \right)\\
      &= \left( 1+\ve \right)\varphi_0\left( x^{\ast}x \right).
  \end{align*}
  For any $ \bar{z}\in Z(M)z_0 $ and $x\in Mz_0$, the fact that $\psi$ is center-valued gives
  \begin{align*}
    \varphi_0\left( \bar{z}x \right) &= \sum_{i=1}^{n}\psi\left( v_i\bar{z}xv_i^{\ast} \right)\\
                                     &= \bar{z} \varphi_0\left( x \right).
  \end{align*}
  In general, though, we may not have $\varphi_0\left( z_0 \right) = z_0$. Yet, since $\varphi_0\left( z_0 \right) > 0$, we may take a spectral projection $z = \chi_{[\ve,\infty)}\left( \varphi_0\left( z_0 \right) \right)$, so $0 \neq z = y\varphi_0\left( z_0 \right)$ for some $y\geq 0$ and $y\in Z(M)$. Setting $\varphi(x) = y\varphi_0(x)$ gives us our desired result.
\end{proof}
\begin{theorem}
  A von Neumann algebra $M$ is finite if and only if there exists a faithful normal center-valued trace.
\end{theorem}
\begin{proof}
  If $\tau$ is any center-valued trace on $M$, and $p$ is any monic projection, with $z = \sum_{i=1}^{n}p_i$ and each $p_i\sim p$, then
  \begin{align*}
    z &= \tau\left( \sum_{i=1}^{n}p_i \right)\\
      &= n\tau\left( p \right),
  \end{align*}
  so that $\tau(p) > 0$. Then, $\tau(p) > 0$ for any nonzero projection, so $\tau(x) > 0$ for any nonzero positive operator $x\in M$, meaning $\tau$ is faithful.

  Suppose there exists such a trace. If $v\in M$ is an isometry, then $1 = \tau\left( v^{\ast}v \right) = \tau\left( vv^{\ast} \right)$, so $\tau\left( 1-vv^{\ast} \right) = 1$, so $vv^{\ast} = 1$, meaning $M$ is finite.

  Conversely, suppose $M$ is finite. Then, if $\left( a_n \right)_n$ is a sequence of real numbers that converge to $1$, then there exists a sequence of normal center-valued states $\tau_n$ such that $\tau_n\left( xx^{\ast} \right)\leq a_n\tau_n\left( ^{\ast}x \right)$ for each $n\in \N$ and $x\in M$. We claim that if $1\leq m < n$, then $a_m^2 \tau_m - \tau_n$ is a positive linear map. To see this, it is enough to consider projections; since the map is normal, it is enough to consider monic projections. Let $p\sim p_1\sim \cdots \sim p_k$ be nonzero projections such that $z = \sum_{i=1}^{k}p_i$ is a central projection. Then, $\tau_n(p)\leq a_n\tau_n\left( p_i \right)$ and $\tau_m \leq a_m\tau_m\left( p_i \right)$ for each $1\leq i \leq k$. Thus,
  \begin{align*}
    k\tau_n\left( p \right) &\leq a_n\sum_{i=1}^{k}\tau_n\left( p_i \right)\\
                            &= a_n\tau_n\left( z \right)\\
                            &= a_n z\\
                            &= a_n\tau_m\left( z \right)\\
                            &= a_n\sum_{i=1}^{k}\tau_m\left( p_i \right)\\
                            &\leq ka_na_m\tau_m\left( p \right)\\
                            &\leq ka_m^2\tau_m\left( p \right).
  \end{align*}
  Thus, $\norm{a_m^2\tau_m-\tau_n}\leq a_m^2-1$, so there is a bounded linear map $\tau$ such that $\norm{\tau-\tau_m}\rightarrow 0$.

  We have $0\leq \tau\left( xx^{\ast} \right)\leq \tau\left( x^{\ast}x \right)$, but since this holds for all $x\in M$, it follows that we must have equality $\tau\left( x^{\ast}x \right) = \tau\left( xx^{\ast} \right)$. Using the polar decomposition of $x$ gives that $\tau\left( u^{\ast}yu \right) = \tau\left( y \right)$ for all invertible $y\geq 0$. Taking linear combinations shows that $\tau\left( y \right)$ is invariant under unitary conjugation, so $\tau$ is a trace.

  Furthermore, $\tau|_{Z(M)} = \id$, and $\tau\left( zx \right) = z\tau(x)$ for all $z\in Z(M)$ and $x\in M$, so we only need to verify that $\tau$ is normal. Letting $\varphi\in M_{\ast}$ be any normal linear functional, then 
  \begin{align*}
    \norm{\varphi\circ\tau - \varphi\circ\tau_m}\leq \norm{\varphi}\norm{\tau-\tau_m},
  \end{align*}
  so $\varphi\circ \tau$ is in $M_{\ast}$ since $M_{\ast}$ is closed, meaning $\tau$ is normal.
\end{proof}
\begin{proposition}
  Let $M$ be a finite von Neumann algebra with center-valued trace $\tau$. If $p,q\in P(M)$, then $p\preceq q$ if and only if $\tau(p)\leq \tau(q)$.
\end{proposition}
\begin{proof}
  Let $p = v^{\ast}v$ and $vv^{\ast}\leq q$. Then, $\tau(p) = \tau\left( vv^{\ast} \right)\leq \tau(q)$ as $\tau$ is positive.

  Conversely, if $\tau(p)\leq \tau(q)$, then by the comparison theorem, we have $z\in Z(P(M))$ such that $pz\preceq qz$ and $\left( 1-z \right)q\preceq \left( 1-z \right)p$. If $v^{\ast}v = \left( 1-z \right)q$ and $vv^{\ast}\leq \left( 1-z \right)p$, then 
  \begin{align*}
    \tau\left( vv^{\ast} \right) &\leq \tau\left( \left( 1-z \right)p \right)\\
                                 &= \left( 1-z \right)\tau\left( p \right)\\
                                 &\leq \left( 1-z \right)\tau\left( q \right)\\
                                 &= \tau\left( vv^{\ast} \right),
  \end{align*}
  so since $\tau$ is faithful, $vv^{\ast} = \left( 1-z \right)p$, so $\left( 1-z \right)q\sim \left( 1-z \right)p$ and $p\preceq q$.
\end{proof}
Now, we start discussing the special case of $\text{II}_1$ factors.
\begin{proposition}
  Let $M$ be a $\text{II}_1$ factor with trace $\tau$. Then, $p\mapsto \tau(p)$ induces a bijection from the equivalence classes of projections in $M$ onto $[0,1]$.
\end{proposition}
\begin{proof}
  We only need to show that for all $t\in \left( 0,1 \right)$, there is $p\in P(M)$ with $\tau(p) = t$. If $t = \sum_{k}2^{-n_k}$ is the dyadic expansion of $t$, then we may use the mutually orthogonal projections $p_{n_k}$ indexed by dyadic rationals that have $\tau\left( p_{n_k} \right) = 2^{-n_k}$ discussed at the beginning of this section to construct $p = \sum_{k}p_{n_k}$. Since $\tau$ is normal, it is continuous with respect to norm-bounded increasing nets of projections, so $\tau(p) = t$.
\end{proof}
For all $n\in \N$, $\M_n\left( M \right)$ is also a $\text{II}_1$ factor, with its trace given by $\tau_n\left( \left( x_{ij} \right)_{i,j} \right) = \frac{1}{n}\sum_{i=1}^{n}\tau\left( x_{ii} \right)$.

If $0 < t \leq n$, then for any projections $p,q\in P\left( \M_n\left( M \right) \right)$ with $\tau_n\left( p \right) = \tau_n\left( q \right) = t/n$, there is a unitary $u\in U\left( \M_n\left( M \right) \right)$ such that $upMpu^{\ast} = qMq$. In particular, up to isomorphism, the corner $pMp$ only depends on $t$. Furthermore, this is independent of $n$ since any two matrix algebras over $M$ can be embedded into a larger common matrix algebra. The \textit{amplification} of $M$ with respect to this parameter $t$ is the factor $M^{t}\coloneq pMp$, where $\tau(p) = t$.

We have that $\left( M^t \right)^{s} \cong M^{ts}$ for all $t,s > 0$. The fundamental group\footnote{No relation to the topological one.} of $M$ is the set of all $t\in \R_{ > 0 }$ with $M^{t}\cong M$. We have that the fundamental group is an isomorphism class invariant of $M$.
\subsection{Conditional Expectations}%
This subsection is also in the \href{https://ai-bearing.github.io/Classes_and_Homework/After\%20College/Other\%20Notes/completely_positive_maps.pdf}{completely positive maps} notes.
\begin{definition}
...
\end{proof}
\subsection{Semifinite Traces}%
Similar to how, over a compact Hausdorff space $K$, states on $C(K)$ correspond to Radon probability measures on $K$. The notion of weight helps generalize the notion of not-necessarily-finite positive Radon measures.
\begin{definition}
  A \textit{weight} on a $C^{\ast}$-algebra $A$ is a map $\varphi\colon A_{+}\rightarrow [0,\infty]$ satisfying
  \begin{align*}
  ...
  \end{align*}
  for all $x,y\in A_{+}$ and $\lambda > 0$.

  The weight is faithful if $\varphi\left( x \right) \neq 0$ for all $x\neq 0$ and $\varphi$ is tracial if $\varphi\left( x^{\ast}x \right) = \varphi\left( xx^{\ast} \right)$ for all $x\in A$.
  
  If $A$ is a von Neumann algebra, the weight $\varphi$ is semifinite if the set
  \begin{align*}
    \mathfrak{p}_{\varphi} &= \set{x\in A_{+} | \varphi(x) < \infty}
  \end{align*}
  generates $A$ as a von Neumann algebra. We say $\varphi$ is normal if it is a normal map in $A_{+}$.
\end{definition}
\begin{proposition}
  Let $\varphi$ be a weight on a $C^{\ast}$-algebra $A$. Then,
  \begin{enumerate}[(i)]
    \item $\mathfrak{n}_{\varphi} = \set{x\in A | x^{\ast}x\in \mathfrak{p}_{\varphi}}$ is a left ideal of $A$;
    \item $\mathfrak{m}_{\varphi} = \set{\sum_{i=1}^{n}y_i^{\ast}x_i | x_1,\dots,x_n,y_1,\dots,y_n\in \mathfrak{n}_{\varphi}}$ is a $\ast$-subalgebra of $\mathfrak{n}_{\varphi}$ that is the span of $\mathfrak{p}_{\varphi}$.
  \end{enumerate}
\end{proposition}
\begin{proof}
  Since
  \begin{align*}
    \left( x + y \right)^{\ast}\left( x + y \right) &\leq \left( x + y \right)^{\ast}\left( x + y \right) + \left( x-y \right)^{\ast}\left( x-y \right)\\
                                                    &= 2\left( x^{\ast}x + y^{\ast}y \right),
  \end{align*}
  we have that $\mathfrak{n}_{\varphi}$ is a linear subspace. It is a left ideal since $\left( ax \right)^{\ast}\left( ax \right) \leq \norm{a}^2x^{\ast}x$.

  Since $\mathfrak{n}_{\varphi}$ is a linear subspace, we have that $\mathfrak{m}_{\varphi}$ is a $\ast$-subalgebra. We have that $\mathfrak{m}_{\varphi}$ contains $\mathfrak{p}_{\varphi}$, and since
  \begin{align*}
    4y^{\ast}x &= \sum_{k=0}^{3}i^{k}\left( x + i^{k}y \right)^{\ast}\left( x + i^{k}y \right),
  \end{align*}
  that $\mathfrak{m}_{\varphi}$ is equal to the span of $\mathfrak{p}_{\varphi}$ follows.
\end{proof}
We call $\mathfrak{m}_{\varphi}$ the domain of definition for $\varphi$.

If $\varphi\left( 1 \right) < \infty$, then we can extend $\varphi$ to all of $A$, and upon rescaling, construct a state.

Fixing a weight $\varphi$ on a $C^{\ast}$-algebra $A$, then the set
\begin{align*}
  N_{\varphi} &= \set{x\in A | \varphi\left( x^{\ast}x \right) = 0}
\end{align*}
is a left ideal of $A$ that is contained in $\mathfrak{n}_{\varphi}$. The quotient space $\mathfrak{n}_{\varphi}/N_{\varphi}$ with quotient map $\eta_{\varphi}\colon \mathfrak{n}_{\varphi}\rightarrow \mathfrak{n}_{\varphi}/N_{\varphi}$ allows us to define a sesquilinear form on $\mathfrak{n}_{\varphi}/N_{\varphi}$ by
\begin{align*}
  \iprod{\eta_{\varphi}(x)}{\eta_{\varphi}(y)} &= \varphi\left( y^{\ast}x \right).
\end{align*}
The space $L_2\left( A,\varphi \right)$ is the Hilbert space completion with respect to this form, and the inequality $\left( ax \right)^{\ast}\left( ax \right)\leq \norm{a}^{2}x^{\ast}x$ extends to a representation $\pi_{\varphi}\colon A\rightarrow B\left( L_2\left( A,\varphi \right) \right)$.

This is essentially a version of the GNS construction with respect to a semifinite weight.
\begin{theorem}
  Let $M$ be a semifinite factor. Then, there exists a unique up to scalar multiplication faithful normal semifinite tracial weight $\Tr\colon M_{+}\rightarrow [0,\infty]$. Furthermore, $p\in P(M)$ is finite if and only if $\Tr(p) < \infty$.
\end{theorem}
\begin{proof}
  Any finite trace is semifinite, so we only need to consider the case when $M$ is properly infinite.

  Let $p_0\in M$ be a nonzero finite projection. Then, there exists an infinite family $\set{p_i}_{i\in I}$ with $\sum_{i\in I}p_i = 1$ and $p_0\sim p_i$ for each $i\in I$. Let $v_i$ be the partial isometries implementing this equivalence, with $v_i^{\ast}v_i = p_0$ and $v_iv_i^{\ast} = p_i$; let $\tau_0$ be the unique tracial state on $p_0Mp_0$.

  Define $\Tr\colon M_{+}\rightarrow [0,\infty]$ by
  \begin{align*}
    \Tr(x) &= \sum_{i\in I} \tau_0\left( v_i^{\ast}xv_i \right).
  \end{align*}
  Then, $\Tr$ is a normal weight on $M$, with normality emerging from the fact that $x\mapsto \tau_0\left( v_i^{\ast}xv_i \right)$ is a normal map for each $i$.

  Now, we have $v_i^{\ast}Mv_i\subseteq \mathfrak{m}_{\Tr}$ for each $i\in I$, meaning that $\mathfrak{m}_{\Tr}$ is weakly dense in $M$, so $\Tr$ is semifinite.

  For each $x\in M$, we use Fubini's Theorem to find
  \begin{align*}
    \Tr\left( xx^{\ast} \right) &= \sum_{i,j\in I} \tau_0\left( v_i^{\ast}xv_jv_j^{\ast}x^{\ast}v_i \right)\\
                                &= \sum_{i,j\in I}\tau_0\left( v_jx^{\ast}v_iv_i^{\ast}xv_j^{\ast} \right)\\
                                &= \Tr\left( x^{\ast}x \right).
  \end{align*}
  Now, if $p\in P(M)$ is finite, then so is the supremum $p_0\vee p$. There is a nonzero monic subprojection $q\leq p_0$ in $\left( p_0\vee p \right)M \left( p_0\vee p \right)$ as this is a finite corner. We must have $0 < \Tr\left( q \right)$, and $\Tr\left( p_0\vee p \right) < \infty$, so $\Tr$ is faithful and finite on finite projections.

  Conversely, if $p$ is infinite, then there is a subprojection $q\leq p$ such that $q\sim p$ and $q\sim p-q$, meaning that $\Tr\left( p \right) = \Tr\left( p-q \right) = \Tr\left( q \right)$, so $\Tr\left( p \right) = 2\Tr\left( p \right)$. Since $\Tr$ is faithful, $\Tr\left( p \right) = \infty$.

  We now show uniqueness. Let $M$ be a semifinite factor and $\omega$ a normal semifinite tracial weight on $M$. Since $\omega$ is semifinite, for each $x\in M_{+}$ with $x\neq 0$, there is $y\in \mathfrak{n}_{\omega}$ such that $x^{1/2}y\neq 0$. Therefore, $x^{1/2}yy^{\ast}x^{1/2}\in \mathfrak{m}_{\omega}$, so $\omega\left( x^{1/2}yy^{\ast}x^{1/2} \right) < \infty$. If $q$ is some nonzero spectral projection of $x^{1/2}yy^{\ast}x^{1/2}$, then there exists some $c > 0$ such that $cq \leq x^{1/2}yy^{\ast}x^{1/2}\leq \norm{y}^2x$.

  From this, it follows that $q$ is a finite projection, since $\omega(q) < \infty$, so $\omega(q)\neq 0$. In particular, this means that $\omega(x)\neq 0$, so $\omega$ is faithful. By a similar argument as with the case of $\Tr$, it follows that $\omega(p)$ is finite if and only if $p$ is finite. In particular, this means that $\omega|_{pMp}$ defines a tracial positive linear functional, so it is a scalar multiple of the trace on $pMp$. Since $M$ is semifinite, every projection is an increasing limit of finite projections, so $\Tr$ is unique up to a scalar multiple.
\end{proof}
\begin{proposition}
  Let $M$ be a countably decomposable semifinite factor. If $p,q\in P(M)$, then $p\preceq q$ if and only if $\Tr(p)\leq \Tr(q)$.
\end{proposition}
\begin{proof}
  If $p$ and $q$ are finite, then restricting $\Tr$ to the finite corner $\left( p\vee q \right)M\left( p\vee q \right)$ gives us a scalar multiple of the trace on $\left( p\vee q \right)M\left( p\vee q \right)$, so the conclusion follows from the case of finite von Neumann algebras.

  Meanwhile, we have $\Tr\left( p \right) = \infty$ if and only if $p$ is not finite, so this follows from the fact that any two infinite projections are equivalent in a semifinite factor.
\end{proof}
\nocite{davidson_functional_analysis,conway_operator_theory,takesaki_1,ii1_factors_houdayer,ii1_factors_anantharaman_popa,goals_vna,von_neumann_algebras_peterson}
\printbibliography
\end{document}
