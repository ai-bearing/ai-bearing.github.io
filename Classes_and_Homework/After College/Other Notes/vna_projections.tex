...
\end{theorem}
\begin{proof}
  Start by considering the case that $A$ is a von Neumann algebra and $B$ is a von Neumann subalgebra. Then, for any $p\in P(B)$ and $x\in A$, we have $\left( 1-p \right)E\left( px \right) = E\left( \left( 1-p \right)E\left( px \right) \right)$, so for all $t > 0$,
  \begin{align*}
