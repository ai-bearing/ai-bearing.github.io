...
\begin{proof}
  ...
  \end{align*}
  Now, if $p\in P(M)$ is finite, then so is their supremum $p_0\vee p$. There is a nonzero monic subprojection $q\leq p_0$ in $\left( p_0\vee p \right)M \left( p_0\vee p \right)$. We must have $0 < \Tr\left( q \right)$, and $\Tr\left( p_0\vee p \right) < \infty$, so $\Tr$ is faithful and finite on finite projections.
