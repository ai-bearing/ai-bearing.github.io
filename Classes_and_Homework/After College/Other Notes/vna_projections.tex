...
\begin{proof}
  ...
  \begin{align*}
    \Phi(x) &= \frac{1}{n}\sum_{i=1}^{n}x_{ii},
  \end{align*}
  based on the tensor product decomposition $M\cong e_1Me_1 \otimes B\left( \ell_2\left( I \right) \right) \cong Z\otimes B\left( \ell_2\left( I \right) \right)$. Since we have $e_i\sim f_j$, we have $\Phi\left( e_i \right) = \Phi\left( f_j \right)$ for all $i\in I$ and $j\in J$.

  Therefore, we have
  \begin{align*}
    \alpha\Phi\left( e_1 \right) &= \sum_{i\in I} \Phi\left( e_i \right)\\
                                 &= \Phi\left( 1 \right)\\
                                 &= \sum_{j\in J}\Phi\left( f_j \right)\\
                                 &= \beta \Phi\left( f_1 \right),
  \end{align*}
  so that $\alpha = \beta$.

  Now, suppose $\alpha$ is infinite. By symmetry, $\beta$ is infinite. If $\varphi$ is a normal ($\sigma$-WOT continuous) state on $Z$, we have that the map $\sigma_i\colon x\mapsto xe_i$ taking $Z\rightarrow e_iMe_i$ is an isomorphism, so that we may define a normal state $\varphi_i$ on $M$ by $\varphi_i(x) = \varphi\circ \sigma_i^{-1}\left( e_ixe_i \right)$. If $z$ is the support of $\varphi$ in $Z$, then $ze_i$ is the support of $\varphi_i$ for each $i$.

  We let $J_i = \set{j\in J | \varphi_i\left( f_j \right)\neq 0}$. Since $\set{f_j}_{j\in J}$ is orthogonal, it follows that $J_i$ is countable. If $\varphi_i\left( f_j \right) = 0$, then $ze_if_jze_i = 0$, meaning that $ze_if_je_i = 0$, meaning $zf_je_i = 0$. Since
  \begin{align*}
    \sum_{i\in I}zf_je_i &= zf_j\\
                         &\neq 0,
  \end{align*}
  it follows that $\varphi_i\left( f_j \right)\neq 0$ for some $i\in I$. Therefore, we have $J = \bigcup_{i\in I}J_i$, meaning $\beta\leq \alpha \aleph_0$, meaning $\beta\leq \alpha$, and thus $\alpha = \beta$ by symmetry.
\end{proof}
\begin{theorem}
  Let $M$ be a type I von Neumann algebra. Then, there exists a unique family of orthogonal central projections $\set{z_{\alpha}}_{\alpha}$ indexed by cardinal numbers such that $\sum_{\alpha}z_{\alpha} = 1$, and $z_{\alpha}Mz_{\alpha}$ is isomorphic to the tensor product of an abelian von Neumann algebra $A_{\alpha}$ and $B\left( H_{\alpha} \right)$, where $\Dim\left( H_{\alpha} \right) = \alpha$. In particular, this gives
  \begin{align*}
    M &\cong \bigoplus_{\alpha} A_{\alpha}\bar\otimes B\left( H_{\alpha} \right).
  \end{align*}
  Additionally, if $M$ is finite, then each of the $\alpha$ is finite.
\end{theorem}
\begin{proof}
  Call a central projection $z\in M$ $\alpha$-homogeneous if $z$ is the sum of $\alpha$ orthogonal abelian projections with central support equal to $z$.

  Letting $\set{z_i}_{i\in I}$ be an orthogonal family of $\alpha$-homogeneous central projections, then we have that $\sum_{i\in I}z_{i}$ is $\alpha$-homogeneous. 
\end{proof}
