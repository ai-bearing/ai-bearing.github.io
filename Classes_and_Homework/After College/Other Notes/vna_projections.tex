...
\begin{proof}
  ...
  Let $\set{\hat{e}_i,\hat{f}_i}_{i\in I}$ be a maximal family with $\set{\hat{e}_i}_{i\in I}$ and $\set{\hat{f}_{i}}_{i\in I}$ are pairwise orthogonal. Suppose $\hat{e}_i\leq \bar{e}$, $\hat{f}_i\leq \bar{f}$, and $\hat{e}_i\sim \hat{f}_i$ with $\left( 1+\e \right)\varphi\left( \hat{e}_i \right)\leq \mu\varphi\left( \hat{f}_i \right)$. Now, the projections $p = \bar{e}-\sum_{i}\hat{e}_i$ and $q = \bar{f}-\sum_{i}\hat{f}_i$ are equivalent. If $p_1,p_2\leq p$ with $p_1\sim p_2$, then there exists $r\leq q$ such that $r\sim p_1$, giving
  \begin{align*}
...
\begin{proof}
  ...
  We have $0\leq \tau\left( xx^{\ast} \right)\leq \tau\left( x^{\ast}x \right)$, 
\end{proof}
