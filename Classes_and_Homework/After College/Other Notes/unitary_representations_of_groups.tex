...
The most important representation is the representation on $L_2(G)$ induced by the action of $G$ on itself by left-multiplication. This defines $\left( \lambda(g)f \right)(y) = f\left( x^{-1}y \right)$. Similarly, the action of $G$ on itself by right-multiplication defines a representation $\left( \rho(g)f \right)(y) = f\left( yx \right)$. These are the \textit{left-regular} and \textit{right-regular} representations of $G$.

Any unitary representation $\pi$ on $\mathcal{H}_{\pi}$ induces a representation $ \overline{\pi} $ on the dual space $ \overline{\mathcal{H}_{\pi}} $, determined by $ \overline{\pi}\left( x \right) = \iprod{\cdot}{\pi\left( x^{-1} \right)} $. We call $ \overline{\pi} $ the contragradient of $\pi$.
...
\end{proposition}
\begin{proof}
  Define the map $T\colon L_2(G)\rightarrow L_2(G)$ by
  \begin{align*}
  ...
  \end{align*}
\end{proof}
If $ \mathcal{M} $ is a closed subspace of $\mathcal{H}_{\pi}$, then we say $\mathcal{M}$ is invariant for $\pi$ if $\pi(x)\mathcal{M}\subseteq \mathcal{M}$ for all $x\in G$. If $ \mathcal{M} $ is a nontrivial invariant subspace, then $\pi|_{\mathcal{M}}$ defines a representation of $G$ on $ \mathcal{M} $, known as a subrepresentation of $\pi$. If $\pi$ admits a nontrivial invariant subspace, then we say $\pi$ is reducible; else, $\pi$ is irreducible.

If $\set{\pi_i}_{i\in I}$ is a family of unitary representations, their direct sum $\bigoplus_{i\in I}\pi_i$ is the representation on $\mathcal{H} = \bigoplus_{i\in I}\mathcal{H}_i$ given by
\begin{align*}
...
\end{proof}
If $\pi$ is a unitary representation, and $u \in \mathcal{H}_{\pi}$, then the closed linear span of $\set{\pi(x)u | x\in G}$ is called the \textit{cyclic subspace} generated by $u$. Observe that $\mathcal{M}_{u}$ is invariant under $\pi$; if $\mathcal{M}_u = \mathcal{H}_{\pi}$, then $u$ is called a cyclic vector for $\pi$, and $\pi$ is called a cyclic representation if it has a cyclic vector.
\begin{proposition}
  Every unitary representation is a direct sum of cyclic representations.
\end{proposition}
\begin{proof}
  Let $\pi$ be a representation on $\mathcal{H}_{\pi}$. By Zorn's Lemma, there is a maximal collection $\set{\mathcal{M}_{i}}_{i\in I}$ of mutually orthogonal cyclic subspaces of $\mathcal{H}_{\pi}$. If there were some $u\in \mathcal{H}_{\pi}$ orthogonal to all the $\mathcal{M}_i$, then the cyclic subspace generated by $u$ would also be orthogonal to the $\mathcal{M}_i$, which would contradict maximality. Therefore,
  \begin{align*}
    \mathcal{H}_{\pi} &= \bigoplus_{i\in I} \mathcal{M}_i\\
    \pi &= \bigoplus_{i\in I}\pi|_{\mathcal{M}_i}.
  \end{align*}
\end{proof}
\begin{proposition}
  Let $\mathcal{M}$ be a closed subspace of $\mathcal{H}_{\pi}$, and let $P$ be the orthogonal projection onto $\mathcal{M}$. Then, $\mathcal{M}$ is invariant under $\pi$ if and only if $P$ is an intertwiner for $\pi$.
\end{proposition}
\begin{proof}
...
We will show that any unitary representation of $G$ corresponds uniquely with the non-degenerate $\ast$-representations of $L_1(G)$. If $\pi$ is a unitary representation of $G$, it determines a representation of $L_1(G)$ by defining the bounded operator $\pi(f)$ on $\mathcal{H}_{\pi}$ by
\begin{align*}
  \pi(f) &= \int_{}^{} f(x)\pi(x)\:dx
\end{align*}
in the weak sense --- that is, for any $u,v\in \mathcal{H}_{\pi}$, we define $\pi(f)$ to be the operator corresponding to the sesquilinear form
\begin{align*}
...
\end{align*}
\begin{theorem}
  Let $\pi$ be a unitary representation of $G$. Then, the map $f\mapsto \pi(f)$ is a nondegenerate $\ast$-representation of $L_1(G)$ on $\mathcal{H}_{\pi}$. Moreover, for any $x\in G$ and $f\in L_1(G)$,
  \begin{align*}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \pi\left( f^{\ast} \right) &= \int_{}^{} \Delta\left( x^{-1} \right) \overline{f\left( x^{-1} \right)} \pi(x)\:dx\\
                               ...
                               &= \pi\left( f \right)^{\ast},
  \end{align*}
  and the latter two follow from similar manipulations. That these are valid manipulations follows from taking inner products with some $u\in \mathcal{H}_{\pi}$, and then using the fact that vector-valued integration commutes with the application of bounded linear maps.

  To see that $\pi$ is nondegenerate, let $u\neq 0\in \mathcal{H}_{\pi}$. Let $V$ be a compact neighborhood of $1$ in $G$ such that $\norm{\pi(x)u-u} < \norm{u}$ for any $x\in V$, and let $f = \mu(V)^{-1}\chi_V$, so that
  \begin{align*}
...
\end{proof}
\begin{theorem}
  Let $\pi$ be a nondegenerate $\ast$-representation of $L_1(G)$ on $\mathcal{H}$. Then, $\pi$ arises from a unique unitary representation of $G$ on $\mathcal{H}$.
\end{theorem}
\begin{proof}
  We want $\pi(x)$ to be the limit of $\pi(f)$ as $f$ approaches $\delta_x$. Let $\set{\psi_U}$ be an approximate identity in $L_1(G)$. Then, for any $f\in L_1(G)$, we have $\psi_U\ast f \rightarrow f$ in $L_1$, meaning that $\left( L_x\psi_U \right)\ast f\rightarrow L_xf$ in $L_1$ for any $x\in G$, so $\pi\left( L_x\psi_U \right)\pi\left( f \right)v\rightarrow \pi\left( L_xf \right)v$ for all $v\in H$.

  Let $\mathcal{D}$ be the finite linear span of $\set{\pi(f)v | f\in L_1(G),v\in \mathcal{H}}$. Then, $\mathcal{D}$ is dense in $\mathcal{H}$, as if $u\in \mathcal{D}^{\perp}$, then 
  \begin{align*}
  0 &= \iprod{u}{\pi(f)v}\\
  ...
  \end{align*}
  on $\mathcal{D}$, so it holds on $\mathcal{H}$. Next, since $\widetilde{\pi}(1) = I_{\mathcal{H}}$, we have $ \widetilde{\pi} $ is a homomorphism from $G$ to the group of invertible operators on $\mathcal{H}$.
  ...
  Finally, if $\left( x_{i} \right)_i\rightarrow x$ in $G$, then $L_{x_i}f\rightarrow L_xf$ in $L_1$, so $ \widetilde{\pi}\left( x_i \right)\pi(f) \rightarrow \widetilde{\pi}(x)\pi(f) $ in SOT. Therefore, it $\widetilde{\pi}\left( x_i \right)\rightarrow \widetilde{\pi}(x)$ on $\mathcal{D}$, so since $\norm{\widetilde{\pi}\left( x_i \right)} = 1$, it holds on $\mathcal{H}$, so $\widetilde{\pi}$ is continuous.

  Next, we will show that $\pi(f) = \widetilde{\pi}(f)$ whenever $f\in L_1\left( G \right)$, where $ \widetilde{\pi}(f) $ arises from $\widetilde{\pi}$ via
  \begin{align*}
    \iprod{\widetilde{\pi}(f)u}{v} &= \int_{}^{} f(x) \iprod{\widetilde{\pi}(x)u}{v}\:dx.
  \end{align*}
  Yet, if $f,g\in L_1$, we have
  \begin{align*}
  ...
  \begin{align*}
    \pi(f)\pi(g) &= \pi\left( f\ast g \right)\\
                 ...
                 &= \widetilde{\pi}(f)\pi(g),
  \end{align*}
  so $\widetilde{\pi}(f) = \pi(f)$ on $\mathcal{D}$, hence on $\mathcal{H}$.

  Finally, if $ \hat{\pi} $ is another unitary representation of $G$ such that $\hat{\pi}(f) = \pi(f)$ for any $f\in L_1(G)$, so $ \iprod{\hat{\pi}(x)u}{v} = \iprod{\widetilde{\pi}(x)u}{v} $, so $\hat{\pi}(x) = \widetilde{\pi}(x)$ for all $x\in G$.
\end{proof}
