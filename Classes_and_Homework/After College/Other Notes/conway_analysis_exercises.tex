...
\end{solution}
\begin{problem}[{\cite[Exercise IX.2.11]{conway_functional_analysis}}, with some modifications]
  Suppose $H$ is a separable Hilbert space; show that $\sigma_P(N)$ is such that $E\left( \sigma(N)\setminus \sigma_p(N) \right) = 0$ if and only if $N$ is diagonalizable. That is, there is a basis of $H$ consisting of eigenvectors for $N$.
\end{problem}
\begin{solution}
  Consider the internal direct sum space
  \begin{align*}
    X &= \bigoplus_{\lambda\in \sigma_p(N)} \ker\left( N-\lambda I \right).
  \end{align*}
  Since $H$ is separable, it follows that $\sigma_p(N)$ is countable, so this is a countable direct sum. Consider the spectral projection given by
  \begin{align*}
    P &= \sum_{\lambda\in \sigma_p(N)} E\left( \set{\lambda} \right).
  \end{align*}
  Observe then that
  \begin{align*}
    P &= \int_{\sigma_p(N)}^{} \:dE\\
      &= \int_{\sigma_p(N)}^{} \:dE + 0\\
      &= \int_{\sigma_p(N)}^{} \:dE + \int_{\sigma(N)\setminus \sigma_p(N)}^{} \:dE\\
      &= \int_{\sigma(N)}^{} \:dE,
  \end{align*}
...
\begin{solution}
  ...
  \end{align*}
  is invertible for each $N$, with $f_n(N)\rightarrow N$ by dominated convergence.
\end{solution}
\printbibliography
\end{document}
