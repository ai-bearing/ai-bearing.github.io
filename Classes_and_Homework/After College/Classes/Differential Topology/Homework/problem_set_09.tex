\documentclass[10pt]{mypackage}

% sans serif font:
%\usepackage{cmbright}
%\usepackage{sfmath}
%\usepackage{bbold} %better blackboard bold

\usepackage{homework}
%\usepackage{notes}
\usepackage{newpxtext,eulerpx,eucal}
\renewcommand*{\mathbb}[1]{\varmathbb{#1}}

\fancyhf{}
\fancyhead[R]{Avinash Iyer}
\fancyhead[L]{Differential Topology: Problem Set 9}
\fancyfoot[C]{\thepage}

\setcounter{secnumdepth}{0}

\begin{document}
\RaggedRight
\begin{problem}[Problem 1]
  A topological group is a group which is also a Hausdorff topological space where the group operations are continuous.\newline

  Recall the definition of the concatenation operation on the fundamental group. Now, let $G$ be a path-connected topological group, and let $\pi_1\left( G,e \right)$ be the fundamental group of $G$ with base point $e$. Use the Hilton--Eckmann argument to prove that the concatenation operation on the fundamental group is commutative.
\end{problem}
\begin{solution}
  Define two operations, $\ast$ and $\cdot$, on the homotopy-classes of functions $f\colon S^{1}\rightarrow \left( G,e \right)$, where $S^{1}\cong [0,1]/\left(\set{0}\sim \set{1}\right)$ given by
  \begin{align*}
    f\ast g &= \begin{cases}
      f\left( 2t \right) & 0 \leq t \leq 1/2\\
      g\left( 2t-1 \right) & 1/2\leq t\leq 1
    \end{cases}\\
      f\cdot g &= f(t)g(t),
  \end{align*}
  where the latter is multiplication within the group and the former is concatenation. We see that the identity map
  \begin{align*}
    \operatorname{id}\colon S^{1} &\rightarrow \left( G,e \right)\\
    t &\mapsto e
  \end{align*}
  is an identity for both $\ast$ and $\cdot$. Our task now is to show that the Hilton--Eckmann condition holds. That is, let $a,b,c,d\colon S^{1}\rightarrow \left( G,e \right)$ be continuous maps with base point $e$. Then,
  \begin{align*}
    \left( a\ast b \right)\cdot \left( c\ast d \right) &= \left( a\ast b \right)(t)\cdot \left( c\ast d \right)(t)\\
                                                       &= \begin{cases}
                                                         a(2t)c(2t) & 0\leq t \leq 1/2\\
                                                         b(2t-1)d(2t-1) & 1/2\leq t \leq 1
                                                       \end{cases}\\
                                                       &= \left( a\cdot c \right)\ast \left( b\cdot d \right),
  \end{align*}
  whence $\cdot = \ast$ and the concatenation operation is commutative.
\end{solution}
\begin{problem}[Problems 2--4]\hfill
  \begin{description}[font=\normalfont]
    \item[(2)] Let $M$ and $N$ be smooth, orientable, closed manifolds of the same dimension $n$, and let $f\colon M\rightarrow N$ be a smooth function. Show that $f$ induces a map $f^{\ast}\colon H^n_{\operatorname{DR}}\left( N \right)\rightarrow H^{n}_{\operatorname{DR}}\left( M \right)$ which is multiplication by an integer. This is called the degree of $f$ and is written $\deg(f)$.
    \item[(3)] Recall the definition of the degree of $f$ from one of the previous problem sets, counting the sums of signs of determinants of the derivative of $f$ over the preimage of a regular value of $f$. Prove that the two definitions of the degree agree.
    \item[(4)] With the setup of the previous exercises, prove that if $\omega$ is an arbitrary $n$-form on $N$, then
      \begin{align*}
        \int_{M}^{} f^{\ast}\omega &= \deg(f) \int_{N}^{} \omega.
      \end{align*}
  \end{description}
\end{problem}
\begin{solution}
  Letting $\omega\in H^{n}_{\operatorname{DR}}\left( N \right)$ be a nonvanishing top-dimensional form. By the naturality of the de Rham isomorphism, it follows that there is some $\delta\in \R$ such that.
  \begin{align*}
    \int_{M}^{} f^{\ast}\omega &= \delta \int_{N}^{} \omega
  \end{align*}
  Our task now is to show that $\delta \in \Z$. In particular, we will show that $\delta = \deg\left( f \right)$, where $\deg\left( f \right)$ is defined as before.\newline

  Toward this end, let $q$ be a regular value of $f$. We may use a smooth bump function to restrict $\omega$ to a small open neighborhood $V$ of $q$. It follows then that $f^{-1}\left( q \right) = \set{p_1,\dots,p_{\ell}}$ for some $\ell$, with corresponding disjoint open neighborhoods $U_1,\dots,U_{\ell}$ locally diffeomorphic to $V$, whence the support of $f^{\ast}\omega$ is contained in the union of $U_1,\dots,U_{\ell}$. If $f^{-1}\left( q \right) = \emptyset$, then
  \begin{align*}
    \int_{M}^{} f^{\ast}\omega &= \int_{\emptyset}^{} f^{\ast}\omega\\
                               &= \delta \int_{N}^{} \omega\\
                               &= 0,
  \end{align*}
  whence $\delta = 0$. If $f^{-1}\left( q \right)\neq \emptyset$, then we see that
  \begin{align*}
    \int_{M}^{} f^{\ast}\omega &= \sum_{k=1}^{\ell} \int_{U_k}^{} f^{\ast}\omega.
  \end{align*}
  Now, since $f$ is a local diffeomorphism on each of the $U_k$, it follows that
  \begin{align*}
    \int_{U_k}^{} f^{\ast}\omega &= \sgn\left( \det\left( D_{p_k}f \right) \right) \int_{V}^{} \omega\\
                                 &= \sgn\left( \det\left( D_{p_k}f \right) \right) \int_{N}^{} \omega.
  \end{align*}
  Therefore, we find that
  \begin{align*}
    \int_{M}^{} f^{\ast}\omega &= \sum_{k=1}^{\ell}\sgn\left( \det\left( D_{p_k}f \right) \right) \int_{N}^{} \omega\\
                               &= \deg\left( f \right) \int_{N}^{} \omega,
  \end{align*}
  giving that $\deg\left( f \right)$ as defined via cohomology and as defined via summation over neighborhoods of preimages of a regular value are equal to each other.
\end{solution}
\begin{problem}[Problem 5]
  Prove that for all $d\in \Z$ and all $n$, there exists a smooth map $S^{n}\rightarrow S^{n}$ with degree $d$.
\end{problem}
\begin{solution}
  If $d = 0$, then we may take a constant map to have degree $d$.\newline

  If $d > 0$, select $d$ points, $p_1,\dots,p_d\in S^{n}$ and corresponding disjoint open neighborhoods $U_1,\dots,U_d$. We define $f\colon S^{n}\rightarrow S^{n}$ to be a map that takes $p_i$ to the north pole, a diffeomorphism between $U_i$ and $S^{n}\setminus \set{s}$ (which follows from the coordinate map for $U_i$ composed with the (inverse) south pole stereographic projection), where $s$ is the south pole, and everything outside of $U_1\cup\cdots\cup U_d$ to the south pole.\newline

  Similarly, if $d < 0$, perform the same process, but $f$ should be orientation-reversing on each of the $U_i$. 
\end{solution}
\begin{problem}[Problem 6]
  Let $K$ and $L$ be smooth knots in $\R^{3}$, which is to say smooth embeddings of $S^{1}$ into $\R^{3}$. Assume that $K$ and $L$ are disjoint subsets of $\R^{3}$. Choosing an orientation on $S^{1}$ orients $K$ and $L$. Write $K = f(s)$ and $L = g(t)$, where $t,s\in S^{1}$, and let $F\left( s,t \right) = g(t)-f(s)$ as a difference of vectors in $\R^{3}$. Notice that $F$ never takes on the value $0$. Renormalizing the vectors in $\R^{3}\setminus \set{0}$ gives a map $\pi\colon \R^{3}\setminus \set{0}\rightarrow S^{2}$. Write $G = \pi\circ F$. Notice that $G\colon S^{1}\times S^{1}\rightarrow S^{2}$ is a smooth map.\newline

  Define the linking number $\operatorname{lk}\left( K,L \right)\equiv \deg\left( G \right)$. Now, let
  \begin{align*}
    \omega &= \frac{1}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{3/2}} \left( x_1\:dx_2\wedge dx_3 - x_2\:dx_1\wedge dx_3 + x_3\:dx_1\wedge dx_2 \right),
  \end{align*}
  where $x_1,x_2,x_3$ are (Cartesian) coordinates on $\R^{3}$.
  \begin{enumerate}[(a)]
    \item Show that $d\omega = 0$.
    \item Let $\eta = \omega|_{S^{2}}$. Prove that
      \begin{align*}
        \int_{S^{2}}^{} \eta &= 4\pi.
      \end{align*}
      Conclude that
      \begin{align*}
        \operatorname{lk}\left( K,L \right) &= \frac{1}{4\pi} \int_{S^{1}\times S^{1}}^{} F^{\ast}\omega.
      \end{align*}
    \item Compute $F^{\ast}dx_i$ for $i=1,2,3$.
    \item Find an explicit expression for $\operatorname{lk}\left( K.L \right)$ as an integral over $S^{1}\times S^{1}$, in terms of $f$ and $g$.
  \end{enumerate}
\end{problem}
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    \item We compute
      \begin{align*}
        d\omega &= \left( \pd{}{x_1}\left( \frac{x_1}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{3/2}} \right) + \pd{}{x_2}\left( \frac{x_2}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{3/2}} \right) + \pd{}{x_3}\left( \frac{x_3}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{3/2}} \right) \right)dx_1\wedge dx_2\wedge dx_3.
      \end{align*}
      Symmetrically, we observe that
      \begin{align*}
        \pd{}{x_1}\left( \frac{x_1}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{3/2}} \right) &= \frac{1}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{3/2}} - \frac{3}{2}\frac{2x_1\left( x_1 \right)}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{5/2}}\\
                                                                                        &= \frac{-2x_1^2 + x_2^2 + x_3^2}{\left( x_1^2 + x_2^2 + x_3^2 \right)^{5/2}},
      \end{align*}
      whence their sum is zero.
    \item We observe that the restriction to $S^{1}$ yields $x_1^2 + x_2^2 + x_3^2 = 1$, meaning that the restriction is
      \begin{align*}
        \iota^{\ast}\omega &= x_1\:dx_2\wedge dx_3 + x_2\:dx_3\wedge dx_1 + x_3\:dx_1\wedge dx_2,
      \end{align*}
      where $\iota\colon S^{2}\hookrightarrow \R^{3}$ is inclusion. In particular, we observe that this is precisely the area form on $\R^{3}$ with respect to the vector
      \begin{align*}
        v &= \begin{pmatrix}x_1\\x_2\\x_3\end{pmatrix},
      \end{align*}
      whence 
      \begin{align*}
        \int_{S^{2}}^{} \iota^{\ast}\omega &= 4\pi.
      \end{align*}
      Now, since $G^{\ast} = F^{\ast}\circ \pi^{\ast}$, we see that
      \begin{align*}
        \deg\left( G \right) \int_{S^{2}}^{} \omega &= \int_{S^{1}\times S^{1}}^{} G^{\ast}\omega\\
                                                    &= \int_{S^{1}\times S^{1}}^{} F^{\ast}\left( \pi^{\ast}\omega \right)\\
                                                    &= \int_{S^{1}\times S^{1}}^{} F^{\ast}\omega,
      \end{align*}
      where the latter equality emerges from the fact that $\pi|_{S^{2}} = \operatorname{id}$.
    \item We write
      \begin{align*}
        F(t,s) &= \begin{pmatrix}f_1(t) - g_1(s)\\f_2(t) - g_2(s)\\f_3(t)-g_3(s)\end{pmatrix}.
      \end{align*}
      Then,
      \begin{align*}
        F^{\ast}dx_i &= d\left( x_i\circ F \right)\\
                     &= d\left( f_i(t)-g_i(s) \right)\\
                     &= \diff{f_i}{t} dt - \diff{g_i}{s}ds.
      \end{align*}
    \item Inserting these expressions into $\omega$ and using the natural symmetry therein, we find
      \begin{align*}
        F^{\ast}\omega &= \frac{1}{\norm{F}^{3}} \left( f_1(t)-g_1(s) \right)\left( \diff{f_3}{t}\diff{g_2}{s} - \diff{f_2}{t}\diff{g_3}{s} \right)\\
                       &+ \frac{1}{\norm{F}^3}\left( f_2(t)-g_2(s) \right)\left( \diff{f_1}{t}\diff{g_3}{s} - \diff{f_3}{t}\diff{g_1}{s} \right)\\
                       &+ \frac{1}{\norm{F}^3}\left( f_3(t)-g_3(s) \right)\left( \diff{f_2}{t}\diff{g_1}{s}-\diff{f_1}{t}\diff{g_2}{s} \right)
      \end{align*}
  \end{enumerate}
\end{solution}
\end{document}
