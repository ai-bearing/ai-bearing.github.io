...
Furthermore, we define the \textit{volume form} to be $\ast\left( 1 \right)$, and observe that $\ast\left( \theta_1\wedge\cdots\wedge \theta_n \right) = 1$.\newline
...
\end{align*}
Using this bilinear form, and the property that $\omega\wedge\ast\eta = \ega\wedge\ast\omega$, we observe that the operator $\ast$ is then an isometry between $\mathcal{A}^{k}\left( M \right)$ and $\mathcal{A}^{n-k}\left( M \right)$.
\section{Notations}%
\begin{itemize}
