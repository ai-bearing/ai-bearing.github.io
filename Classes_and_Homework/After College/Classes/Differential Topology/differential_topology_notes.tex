...
\end{align*}
In other words, the family of $\Lambda^{k}\left( \R^{n} \right)$ are alternating and multilinear. In fact, the exterior algebra is the ``universal'' algebra for alternating multilinear maps. The multiplication in $\Lambda\left( \R^{n} \right)$ is given by the exterior product,
\begin{align*}
...
\end{align*}
We then consider differential $k$-forms as $C^{\infty}$ maps from $\R^{n}$ to $\Lambda^{k}\left( \R^{n} \right)$. We call this space $\mathcal{A}^{k}$,
\begin{align*}
  \mathcal{A}^{k}\coloneq \set{\omega\colon \R^{n}\rightarrow \Lambda^{k}\left( \R^{n} \right) | \omega\text{ is }C^{\infty}}.
\end{align*}
Note that we can express $\mathcal{A}^{k} = C^{\infty}\left( \R^{n} \right) \otimes \Lambda^{k}\left( \R^{n} \right)$. Elements of $\mathcal{A}^{k}$ are of the form
\begin{align*}
  \omega &= \sum_{I} f_{I} dx_{i_1}\wedge\cdots\wedge dx_{i_k},
\end{align*}
where $I$ is an indexing set. Notice that $\mathcal{A}^{0}\left( \R^{n} \right) = C^{\infty}\left( \R^{n} \right)$. 
\section{Notations}%
\begin{itemize}
