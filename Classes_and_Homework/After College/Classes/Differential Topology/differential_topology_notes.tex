...
\begin{proof}
  ...
  \end{align*}
  has nullity $\mu$. Considering the line $\ell = q + tv$ for $v\in N_qM$, we find that this is equal to $\left( I_k - tv\cdot \ell_{ij} \right)_{i,j}$, which is singular if and only if $\left( \frac{1}{t}I_k-v\cdot \ell_{ij} \right)_{i,j}$ is singular, whence $\frac{1}{t}$ is a principal curvature of $M$ at $q$ in the direction of $v$.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  Therefore, if $q$ is a critical point for $f$, then $q$ is degenerate if and only if $p$ is a focal point for $M$ at $q$. By Sard's Theorem, this implies that for almost every $p\in \R^{n}$, $L_p$ is Morse.
\end{proof}
\section{Notations}%
\begin{itemize}
