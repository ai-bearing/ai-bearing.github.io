...
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    ...
    \item We view $S^{\infty}\subseteq \R^{\infty}$, where the latter denotes finitely supported sequences in $\R$ endowed with the disjoint union topology. Then, for an element $\left( x_n \right)_n\in S^{\infty}$, we have that $\left( x_n \right)_n$ is finitely supported. Furthermore, from the definition of the union topology, we have that the homotopy
      \begin{align*}
        H\colon [0,1]\times S^{\infty}&\rightarrow S^{\infty}\\
        \left( t,\left( x_n \right)_n \right) &\mapsto \left( 1-t \right)\left( x_n \right)_n + t \left( x_{n-1} \right)_n
      \end{align*}
      is necessarily continuous, as in each coordinate it is simply a straight line in $\R^{n}$. Letting $f_t$ denote the latter function, we observe that
      \begin{align*}
        f_t &= \left( 1-t \right)\id + t V,
      \end{align*}
      where $V$ denotes the left unilateral shift. Now, we will define a homotopy on $S^{\infty}$, $G\colon [0,1]\times S^{\infty}\rightarrow S^{\infty}$ by taking
      \begin{align*}
        G\left(t,x\right) &= \begin{cases}
          H(2t,x) & t\in [0,1/2]\\
          H(2(2t-1),x) & t\in [1/2,3/4]\\
          H(4(2t-1),x) & t\in [3/4,7/8]\\
          \vdots
        \end{cases}.
      \end{align*}
      By this arbitrary composition of left unilateral shifts, it follows that every element $x\in S^{\infty}$ will eventually homotope to $0$, so that the identity map on $S^{\infty}$ is null-homotopic.
  \end{enumerate}
\end{solution}
