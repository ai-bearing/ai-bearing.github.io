...
\end{problem}
\begin{solution}
  Let $ \overline{g_1} $ denote the path $g_1$ ran in reverse. Then, since concatenation is associative, we have that
  \begin{align*}
    \left( f_0\cdot g_0 \right)\cdot \overline{g_1} &\simeq \left( f_1\cdot g_1 \right) \cdot \overline{g_1}\\
    f_0 \cdot \left( g_0\cdot \overline{g_1} \right) &\simeq f_1\cdot \left( g_1\cdot \overline{g_1} \right)\\
                                                     &\simeq f_1.
  \end{align*}
  Therefore, it is our task to show that $f_0\cdot \left( g_0 \cdot \overline{g_1} \right)$ is homotopic to $f_0$. Yet, this follows from the fact that $g_0\cdot \overline{g_1}\simeq c$, where $c$ is a constant path, so we have $f_0\cdot \left( g_0\cdot \overline{g_1} \right) \simeq f_0\cdot c\simeq f_0$, so $f_0\simeq f_1$ since ``is homotopic to'' is an equivalence relation.
\end{solution}
\begin{problem}
