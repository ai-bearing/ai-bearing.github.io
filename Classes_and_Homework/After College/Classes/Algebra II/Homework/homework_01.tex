...
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    ...
    \item We consider the direct product
      ...
      \end{align*}
      we have that this is not a zero module, since by using the same element $\left( 1,1,\dots \right)$, the Archimedean property implies that for any $\frac{a}{b}$ in lowest terms in $\Q$, there is some $k$ such that $2^k > a$. (Alternatively, we may use 8(d) to establish a similar result.) Yet, since each of the individual $\Z/2^{i}\Z$ has torsion, it would follow that
      \begin{align*}
...
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    \item By the well-definedness of addition in $R/I$, we have that $\varphi$ is additive in each variable. Now, letting $p(x)\in R$ and $a(x),b(x)\in I$ be defined by
      ...
      \begin{align*}
        \varphi\left( p(x)a(x),b(x) \right) &= \varphi\left( p_0a_0 + O(x),b_0 + b_1 x + \cdots \right)\\
...
\end{problem}
\begin{solution}
  Using the fact that $R[x]$ is a free $R$-module with basis $\set{1,x,x^2,\dots}$, we only need to define an $R$-bilinear map on the basis elements. Consider the map $\phi\colon S\times R[x]\rightarrow S[x]$ given by
  \begin{align*}
    \phi\left( s,x^m \right) &= sx^{m}.
  \end{align*}
  This is an $R$-bilinear map, so it extends uniquely to a homomorphism on the tensor product
  \begin{align*}
    \varphi\colon S\otimes R[x]&\rightarrow S[x]
    s\otimes x^m &\mapsto sx^m.
  \end{align*}
  Furthermore, this homomorphism has an inverse given by $\psi\left( sx^m \right) = s\otimes x^m$, once again extended linearly since $S[x]$ is a free $S$-module. Thus, we have that $S\otimes R[x]\cong S[x]$.
\end{solution}
\end{document}
