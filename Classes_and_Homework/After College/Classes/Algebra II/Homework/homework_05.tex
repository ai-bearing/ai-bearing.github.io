...
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    ...
    \item Let $\omega(w)$, the weight of $w$, denote the number of nonzero coordinates of $w$. Observe that if $\omega(w) > 2$, then there is a transposition $\tau\in S_n$ that leaves at least one coordinate invariant; therefore, we would necessarily have $\omega\left( \rho(\tau)w-w \right)$ has weight smaller than $w$.

      Now, suppose $w\in X\leq W$ is a nonzero $S_n$-invariant subspace. Either the weight of $w$ is $1$, $2$, or greater than $2$. If the weight of $w$ is $1$, then we may choose a transposition in $S_n$ exchanging the index of this nonzero element with some index with a zero element; then, $\omega\left( \rho\left( \tau \right)w-w \right)$ has weight $2$, and $\rho(\tau)w-w$ is contained in $X$ by the assumption of $S_n$-invariance. Meanwhile, if $w$ has weight greater than $2$, we may inductively apply the procedure discussed above to find another vector in $X$ with weight $2$. In particular, this means that any $S_n$-invariant subspace of $W$ contains a vector of weight $2$.
  \end{enumerate}
\end{solution}
\end{document}
