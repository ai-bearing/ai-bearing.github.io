...
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    ...
    \item If $g$ is $0$, or $f$ is $0$, we are done. Else, assume that $g$ and $f$ are not identically zero. Observe that if $g$ is everywhere non-vanishing, then the function $ \frac{f(z)}{g(z)} $ is entire, and satisfies
      ...
      If $k(z) = \frac{f(z)}{g(z)}$ is such that $g(z)$ admits zeros, then they must be isolated zeros, or else by the identity theorem, $g$ would be identically zero everywhere. Let $a$ be one of these zeros for $g$. If $\ve$ is small, we observe that for $ 0 < \left\vert z-a \right\vert < \ve $, $k(z)$ is bounded (as it is bounded everywhere except for the singularities). If we let $M$ be this bound, then we observe that the value
      \begin{align*}
