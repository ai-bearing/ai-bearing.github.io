...
\begin{solution}
  ...
  \begin{align*}
    \left\vert f'(z) \right\vert &\leq \frac{1}{r}\sup_{\left\vert \xi - z \right\vert = r} \left\vert f(z) \right\vert\\
                                 &= \frac{1}{r} \sup_{z\in S\left( z,r \right)} \left\vert f(z) \right\vert\\
                                 &\leq \frac{m}{r},
  \end{align*}
  ...
  \begin{align*}
    \left\vert f_n(z) - g(z) \right\vert &= \left\vert \int_{0}^{1} z \left( f_n'(tz)- g'(tz)\right)\:dt \right\vert\\
                                         &\leq \sup_{w\in K_m} \left\vert w \right\vert \left\vert f_n'(w) - g'(w)  \right\vert\\
                                         &\leq \sup_{w\in K_m} \left\vert f_n'(w) - g'(w) \right\vert\\
                                         &= \norm{f_n' - g'}_{K_m},
  \end{align*}
...
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    \item For each $\rho \leq t \leq r$, we parametrize $S\left( z_0,t \right)$ as $\gamma(t) = z_0 + te^{i\theta}$. In particular, by Cauchy's Integral Formula, we get
      \begin{align*}
        f(z) &= \frac{1}{2\pi i} \int_{S\left( z_0,t \right)}^{} \frac{f\left( w \right)}{w - z}\:d\theta\\
      ...
      \begin{align*}
        f(z) &= \frac{1}{2\pi \left( r - \rho \right)} \int_{0}^{2\pi} \int_{\rho}^{r} \frac{f\left( z_0 + te^{i\theta} \right) te^{i\theta}}{\left( z_0-z \right) + te^{i\theta}}\:dt\:d\theta
      \end{align*}
      Estimating the integral, we find that for $\rho \leq t \leq r$,
      \begin{align*}
        \left\vert \frac{1}{\left( z_0-z \right) + te^{i\theta}} \right\vert &\leq \frac{1}{\left\vert t \right\vert - \left\vert \left\vert z_0 \right\vert-\left\vert z \right\vert \right\vert}\\
                                                                             &\leq \frac{1}{\left\vert t \right\vert - \left\vert z_0 - z \right\vert}\\
                                                                             &\leq \frac{1}{\left\vert t \right\vert - \rho}.
      \end{align*}
      Since this holds for any $\rho < t \leq r$, it holds for $t = r$, whence using this estimate and the triangle inequality gives
      \begin{align*}
