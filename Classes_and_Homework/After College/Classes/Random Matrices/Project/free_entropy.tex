...
\begin{align*}
  \chi(x) &= \int_{}^{} \ln\left\vert s-t \right\vert\:d\mu(s)\mu(t) + \frac{3}{4} + \frac{1}{2}\ln\left( 2\pi \right),
\end{align*}
...
\begin{itemize}
  ...
  \item if $\delta_0\left( a_1,\dots,a_n \right) > -\infty$, then $\delta_0\left( a_1,\dots,a_n \right) = n$.
\end{itemize}
...
For this, given a (compact) metric space $\left( X,d \right)$, define $P_{\ve}\left( X,d \right)$ to be the maximum number of disjoint open balls contained in $X$, and let $K_{\ve}\left( X,d \right)$ be the minimum number of open balls necessary to cover $X$. We have the inequality $P_{\ve}\left( X,d \right) \geq K_{2\ve}\left( X,d \right)\geq P_{4\ve}\left( X,d \right)$. 
\section{Applications: Structural Properties of Free Group Factors}%
The primary hope with the value $\chi$ is that it will provide for some kind of invariant of a von Neumann algebra. For this, we consider properties as follows: say $M$ has property $P$ if $W^{\ast}\left( x_1,\dots,x_n \right) = -\infty$ for all generating sets $\set{x_1,\dots,x_n}$ of $M$. In particular, if we can find some generating set of $M$ with finite free entropy, then $M$ cannot have this property.
\printbibliography
\end{document}
