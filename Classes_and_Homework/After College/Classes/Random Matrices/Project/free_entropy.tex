...
\end{align*}
It turns out that the value of $R$ has minor influence, and we only need to choose a fixed $R$ greater than the norm of each $x_i$. 
...
\end{align*}
It turns out that this question is equivalent to a different problem: one concerning a special kind of von Neumann algebra. Here, we will elaborate on the relation between this \textit{microstate conjecture} and the Connes Embedding Problem.
\printbibliography
\end{document}
