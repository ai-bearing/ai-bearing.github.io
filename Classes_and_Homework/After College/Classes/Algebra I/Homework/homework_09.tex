...
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    ...
    \item By the orbit-stabilizer theorem, and since the action of $\GL_n\left( F \right)$ on $T(m)$ is transitive, we know that
      ...
      Thus,
      \begin{align*}
        \left\vert T(m) \right\vert &= \frac{\prod_{i=0}^{n-1}\left( q^{n}-q^{i} \right)}{\left( \prod_{i=0}^{m-1}\left( q^{m}-q^{i} \right) \right) \left( \prod_{i=0}^{n-m-1}\left( q^{n-m} -q^{i}\right) \right)\left( q^{m\left( n-m \right)} \right)}.
      \end{align*}
  \end{enumerate}
\end{solution}
...
\begin{problem}[Problem 8]
  ...
  \begin{enumerate}[(a)]
    ...
    \item Show that $\aut\left( S_3 \right)\cong S_3$, and hence every automorphism of $S_3$ is inner.
  \end{enumerate}
\end{problem}
\begin{solution}\hfill
  \begin{enumerate}[(a)]
    \item To start, we observe that the transpositions do not commute with each other. Letting $a,b,c$ be arbitrary letters, then upon rearrangement we observe that, without loss of generality, $\left( a,b \right)\left( a,c \right) = \left( a,c,b \right)$, while $\left( a,c \right)\left( a,b \right) = \left( a,b,c \right)$.\newline

      Similarly, a cycle does not commute with any transposition. This can be seen, again without loss of generality, by computing $\left( a,b,c \right)\left( a,b \right) = \left( a,c \right)$, while $\left( a,b \right)\left( a,b,c \right) = \left( b,c \right)$.\newline

      Since the center of $S_3$ is trivial, it follows that conjugation is always a nontrivial automorphism on $S^{3}$, so the inner automorphisms $\iota_g(k) = gkg^{-1}$ are in one-to-one correspondence with the elements of $S_3$ by mapping $g\mapsto \iota_g$, or that $\operatorname{inn}\left( S_3 \right) \cong S_3$.
    \item Let $T = \set{\tau_1,\tau_2,\tau_3}\subseteq S_3$ denote the transpositions, and let $\aut\left( S_3 \right)$ act on $T$ by permutation. We claim that for any $\sigma\in \aut\left( S_3 \right)$, the action of $\sigma$ on $S_3$ is entirely determined by its action on $T$. This follows from the fact $\sigma\left( e \right) = e$ by definition, and that the two $3$-cycles in $S_3$ are built from $2$-cycles,
      \begin{align*}
        \left( 1,2,3 \right) &= \left( 1,3 \right)\left( 1,2 \right)\\
        \left( 1,3,2 \right) &= \left( 1,2 \right)\left( 1,3 \right).
      \end{align*}
      The orbit of $\tau_1$ is the entirety of $T$, since we may select $\sigma$ to be the automorphism that exchanges $\tau_1$ with either $\tau_2$ or $\tau_3$ respectively. Since the permutations $\sym(T)$ are in one-to-one correspondence with permutations in $S_3$ (take the permutation in $S_3$ to be that of the indices in $T$), it follows that the permutation representation $\pi\colon \aut\left( S_3 \right)\rightarrow \sym(T)$ is faithful, since the only way for $\sigma\cdot \tau_i = \tau_i$ for each $\tau_i$ to hold is if $\sigma = \id$.\newline

      Using the identification $\sym\left( T \right)\cong S_3$, we thus get an injective map $\pi\colon \aut\left( S_3 \right)\rightarrow S_3$. Yet, since $\operatorname{inn}\left( S_3 \right)\subseteq \aut\left( S_3 \right)$ is isomorphic to $S_3$, it follows that $\pi$ is necessarily a surjection, so that $\operatorname{inn}\left( S_3 \right)\cong \aut\left( S_3 \right)$.
  \end{enumerate}
\end{solution}
\end{document}
