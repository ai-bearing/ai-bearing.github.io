...
\end{problem}
\begin{solution}
  Let $M_1 = \varphi_1(G)$ and $M_2 = \varphi_2(G)$. We start with the case that $G$ is an infinite cyclic group, operating under the assumption that $\varphi_1$ and $\varphi_2$ are injective. It then follows that $M_1 = \left\langle \varphi_1\left( x \right) \right\rangle$ and $M_2 = \left\langle \varphi_2\left( x \right) \right\rangle$ by injectivity. Since $M_1$ and $M_2$ are conjugate, it follows that there is some $g\in \aut(H)$ such that $gM_1g^{-1} = M_2$. Conjugation is an isomorphism, so this means that $g\varphi_1\left( x \right)g^{-1} = \varphi_2(x)$ as generators are mapped to generators under isomorphism.\newline
  ...
  \begin{align*}
    \psi\left( \left( x_1,y_1 \right)\left( x_2,y_2 \right) \right) &= \psi\left( x_1\varphi_1\left( y_1 \right)\left( x_2 \right),y_1y_2 \right)\\
                                                                    ...
                                                                    &= \left( g\left( x_1 \right),y_1 \right)\left( g\left( x_2 \right),y_2 \right).
  \end{align*}
  Therefore, we only need to show the case for when $G$ is of finite order. 
\end{solution}
