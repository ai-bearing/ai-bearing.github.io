...
\end{problem}
\begin{solution}
  We notice that, since $0$ is an eigenvalue of $A$, as can be seen by plugging in $0$ for the characteristic polynomial of $A$, the minimal polynomial $m_A(t)$ factors as $m_A(t) =t p(t)$ (by Cayley--Hamilton) for some monic polynomial $p\in \R\left[ t \right]$ with strictly lesser degree than $m_A(t)$. Since $m_A(t)$ is the minimal polynomial of $A$, it must mean that $p(A) \neq 0$, so by setting $B = p(A)$, we see that $AB = BA = 0$.
\end{solution}
